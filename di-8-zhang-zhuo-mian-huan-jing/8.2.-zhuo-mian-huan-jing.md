...
KDE Plasma 是一款易于使用的桌面环境。这个桌面提供了一套具有一致外观和感觉的应用程序，标准化的菜单和工具栏，键绑定，颜色方案，国际化以及一个集中化、基于对话框的桌面配置。有关 KDE 的更多信息，请访问[KDE 主页](https://kde.org/)。有关 FreeBSD 的特定信息，请查阅[KDE 的 FreeBSD 主页](https://freebsd.kde.org/)。
...
KDE Plasma 使用 [dbus-daemon(1)](https://man.freebsd.org/cgi/man.cgi?query=dbus-daemon&sektion=1&format=html) 作为消息总线和硬件抽象。此应用程序会自动作为 KDE Plasma 的一个依赖项安装。
...
GNOME 使用 [dbus-daemon(1)](https://man.freebsd.org/cgi/man.cgi?query=dbus-daemon&sektion=1&format=html) 作为消息总线和硬件抽象。此应用程序会自动作为 GNOME 的一个依赖项安装。
...
XFCE 使用 [dbus-daemon(1)](https://man.freebsd.org/cgi/man.cgi?query=dbus-daemon&sektion=1&format=html) 作为消息总线和硬件抽象。此应用程序会自动作为 XFCE 的一个依赖项安装。
...
MATE 使用 [dbus-daemon(1)](https://man.freebsd.org/cgi/man.cgi?query=dbus-daemon&sektion=1&format=html) 作为消息总线和硬件抽象。此应用程序会自动作为 MATE 的一个依赖项安装。
...
Cinnamon 使用 [dbus-daemon(1)](https://man.freebsd.org/cgi/man.cgi?query=dbus-daemon&sektion=1&format=html) 作为消息总线和硬件抽象。此应用程序会自动作为 Cinnamon 的一个依赖项安装。
...
LXQT 使用 [dbus-daemon(1)](https://man.freebsd.org/cgi/man.cgi?query=dbus-daemon&sektion=1&format=html) 作为消息总线和硬件抽象。此应用程序会自动作为 LXQT 的一个依赖项安装。
