...
Eclipse 平台是一个开放的、可扩展的 IDE，适用于各种用途。Eclipse 平台提供了构建和运行集成软件开发工具的基础模块和平台。Eclipse 平台能让工具开发者独立开发与他人工具集成的工具。
...
Neovim 是对 [editors/vim](https://cgit.freebsd.org/ports/tree/editors/vim/) 的一次彻底重构，具有合理的默认设置、内置终端仿真器、异步插件架构和强大的 API，旨在提升速度和可扩展性。兼容绝大多数 Vim 插件和脚本。
...
GNU Emacs 是一款可扩展、可定制的自由文本编辑器。其核心是一个 Emacs Lisp 解释器，这是 Lisp 编程语言的一种方言，带有支持文本编辑的扩展。
