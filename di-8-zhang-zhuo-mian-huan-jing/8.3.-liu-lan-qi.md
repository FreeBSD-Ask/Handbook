...
Iridium 是基于 Chromium 代码库的一个免费、开源、自由的浏览器修改版，在多个关键领域增强了隐私保护。部分查询、关键词和指标向中央服务的自动传输被禁止，仅在用户同意的情况下才会进行。
...
Falkon 是一款较新的、非常快速的 QtWebEngine 浏览器。它旨在成为一个轻量级的 web 浏览器，适用于所有主流平台。Falkon 拥有用户期待的标准浏览器功能，包括书签、历史记录（也可以在侧边栏显示）和标签。除此之外，AdBlock 插件可以阻止广告，Click2Flash 可以阻止 Flash 内容，SSL Manager 可以编辑本地 CA 证书数据库。
