...
下面介绍如何设置 ATM 上的 PPP（PPPoA）。PPPoA 是欧洲 DSL 供应商中的一个流行选择。

28.5.1. 使用 mpd
mpd 应用程序可以用来连接各种服务，特别是 PPTP 服务。它可以使用二进制包或 ports [net/mpd5](https://cgit.freebsd.org/ports/tree/net/mpd5/pkg-descr) 来安装。许多 ADSL 调制解调器要求在调制解调器和计算机之间建立一个 PPTP 隧道。
...
②ADSL调制解调器的 IP 地址。Alcatel SpeedTouch™ Home 默认为 `10.0.0.138`。

可以通过以 `root` 身份发布以下命令来轻松初始化连接。
...
一个 **tun** 虚拟隧道设备将被创建，用于 pptp 和 ppp 进程之间的交互。一旦提示返回，或者 pptp 进程确认了连接，检查隧道：
