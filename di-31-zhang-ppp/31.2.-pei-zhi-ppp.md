...
FreeBSD 提供对拨号 PPP 连接的内建支持，可以通过 [ppp(8)](https://man.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&format=html) 管理。默认的 FreeBSD 内核已支持 **tun**，用于与调制解调器硬件交互。配置工作通过编辑至少一个配置文件完成，并已提供包含示例的配置文件。最后，使用 `ppp` 命令启动和管理连接。
...
>
> 当编辑配置文件时，缩进非常重要。以 `:` 结尾的行必须置于第一列开头，其他行则需如示例中所示，使用空格或 Tab 缩进。
...
**第 2 行**：启用详细日志参数，用于测试连接。配置正常工作后，可简化为：

```sh
set log phase tun
```

**第 3 行**：向连接另一端的 PPP 软件显示 [ppp(8)](https://man.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&format=html) 的版本号。
...
注意此命令为便于阅读而换行。**ppp.conf** 中的任何命令都可以在行末加 `\` 来换行。

**第 8 行**：设置链路空闲超时（秒）。

**第 9 行**：指示对端确认 DNS 设置。如果本地网络已有 DNS 服务器，应将本行注释掉（加 `#`），或直接删除。

**第 10 行**：空行，仅用于可读性，[ppp(8)](https://man.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&format=html) 会忽略空行。
...
如果以手动或自动方式运行 [ppp(8)](https://man.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&format=html)，可能还需要创建 **/etc/ppp/ppp.linkup** 文件，内容如下。若以 `-auto` 模式运行 `ppp`，该文件是必需的。它在连接建立之后使用，此时 IP 地址已分配，可以添加路由表项。创建此文件时，*provider* 必须与 **ppp.conf** 第 11 行中所用值一致。
...
若希望让这些用户能访问所有条目，则应将该语句放入 `default` 部分。
...
这会告知客户端主、备名称服务器地址和 NetBIOS 名称服务器主机。

在 2 及以上版本中，如果省略 `set dns` 行，则 PPP 会使用 **/etc/resolv.conf** 中的值。

### 31.2.2.1. PAP 与 CHAP 认证
...
以明确表达使用的认证类型，但默认情况下 PAP 与 CHAP 都接受。

**第 15 行**：当使用 PAP 或 CHAP 时，ISP 通常不需要登录服务器，因此请禁用“set login”字符串。

### 31.2.2.2. 使用 PPP 的网络地址转换功能

PPP 能使用内部 NAT，而无需依赖内核的 divert 支持。可以通过在 **/etc/ppp/ppp.conf** 中加入以下行启用此功能：

```sh
nat enable yes
```

或者，也可以通过命令行选项 `-nat` 启用 NAT。此外，**/etc/rc.conf** 中有名为 `ppp_nat` 的选项，默认是启用的。

在使用此功能时，可以加入如下选项以启用入站连接的端口转发（位于 **/etc/ppp/ppp.conf**）：
...
虽然 `ppp` 完成配置，但仍需对 **/etc/rc.conf** 做一些编辑。
...
如果 ISP 提供了静态 IP 地址和主机名，请使用此名称作为主机名。
...
> **注意**
>
> `ifconfig_tun0` 变量应为空，并应创建名为 **/etc/start_if.tun0** 的文件。该文件应包含以下一行：
>
> ```sh
> ppp -auto mysystem
> ```
...
这一点很重要，因为不应启动 `routed` 守护进程，否则 `routed` 会删除由 `ppp` 创建的默认路由表项。

建议确保 `sendmail_flags` 行中不包含 `-q` 选项，否则 `sendmail` 会定期做网络查询，可能会触发系统拨号。你可以尝试：

```sh
sendmail_flags="-bd"
```

这样做的缺点是，每当 PPP 链路建立时，`sendmail` 将被迫重新检查邮件队列。要实现自动化，可以在 **ppp.linkup** 中包含 `!bg`：
...
另一种方法是设置“dfilter”阻止 SMTP 流量。详见示例文件。
...
然后输入 `dial provider` 启动 PPP 会话，或者，如果想让 `ppp` 在有出站流量时自动建立连接，并且 **start_if.tun0** 不存在，可输入：

```sh
# ppp -auto provider
```

当 `ppp` 在后台运行时，可以与其通信，但前提是设置了合适的诊断端口。为此，在配置中添加以下行：

```sh
set server /var/run/ppp-tun%d DiagnosticPassword 0177
```

这会让 PPP 监听指定的 UNIX® 域套接字，在允许访问之前，要求客户端提供指定的密码。名称中的 `%d` 由使用中的 **tun** 设备号替代。

套接字建立后，[pppctl(8)](https://man.freebsd.org/cgi/man.cgi?query=pppctl&sektion=8&format=html) 程序就可以在需要操作正在运行的程序的脚本中使用。

## 31.2.5. 配置拨入服务

[“拨入服务”](../di-30-zhang-chuan-hang-tong-xin/30.4.-bo-ru-fu-wu.md) 提供了启用拨号服务的良好说明，使用的是 [getty(8)](https://man.freebsd.org/cgi/man.cgi?query=getty&sektion=8&format=html)。

除了 `getty`，还有更智能的替代方案 [comms/mgetty+sendfax](https://cgit.freebsd.org/ports/tree/comms/mgetty+sendfax/) Ports，它是专为拨号线路设计的 `getty` 改进版本。

使用 `mgetty` 的优点在于它能主动与调制解调器“对话”，意味着如果在 **/etc/ttys** 中关闭了端口，那么调制解调器将不会接听电话。

从 0.99beta 版本起，`mgetty` 还支持自动检测 PPP 流，这样客户端无需脚本即可访问服务器。

关于 `mgetty` 的更多信息，请参阅：[http://mgetty.greenie.net/doc/mgetty_toc.html](http://mgetty.greenie.net/doc/mgetty_toc.html)。

默认情况下，[comms/mgetty+sendfax](https://cgit.freebsd.org/ports/tree/comms/mgetty+sendfax/) Ports 启用了 `AUTO_PPP` 选项，让 `mgetty` 能检测 PPP 连接的 LCP 阶段，并自动启动 ppp shell。然而由于此过程中不会经过常规的登录/密码步骤，因此需要通过 PAP 或 CHAP 方式认证用户。

本节假设用户已成功编译并安装了 [comms/mgetty+sendfax](https://cgit.freebsd.org/ports/tree/comms/mgetty+sendfax/) Ports。
...
创建名为 **/etc/ppp/ppp-pap-dialup** 的可执行文件，内容如下：
