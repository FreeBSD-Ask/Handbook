...
虽然 `ppp` 已经完成配置，但仍需对 **/etc/rc.conf** 做一些编辑。
...
默认情况下，[comms/mgetty+sendfax](https://cgit.freebsd.org/ports/tree/comms/mgetty+sendfax/) Ports 启用了 `AUTO_PPP` 选项，使 `mgetty` 能够检测 PPP 连接的 LCP 阶段，并自动启动 ppp shell。然而由于此过程中不会经过常规的登录/密码步骤，因此需要通过 PAP 或 CHAP 方式认证用户。
