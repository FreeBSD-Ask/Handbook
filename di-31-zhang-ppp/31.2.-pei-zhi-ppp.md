...
除了 `getty`，还有更智能的替代方案 Port [comms/mgetty+sendfax](https://cgit.freebsd.org/ports/tree/comms/mgetty+sendfax/)，它是专为拨号线路设计的 `getty` 改进版本。
...
默认情况下，Port [comms/mgetty+sendfax](https://cgit.freebsd.org/ports/tree/comms/mgetty+sendfax/) 启用了 `AUTO_PPP` 选项，让 `mgetty` 能检测 PPP 连接的 LCP 阶段，并自动启动 ppp shell。然而由于此过程中不会经过常规的登录/密码步骤，因此需要通过 PAP 或 CHAP 方式认证用户。

本节假设用户已成功编译并安装了 Port [comms/mgetty+sendfax](https://cgit.freebsd.org/ports/tree/comms/mgetty+sendfax/)。
