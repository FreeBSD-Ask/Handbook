...
FreeBSD 提供对拨号 PPP 连接的内建支持，可以通过 [ppp(8)](https://man.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&format=html) 管理。默认的 FreeBSD 内核已支持 **tun**，用于与调制解调器硬件交互。配置工作通过编辑至少一个配置文件完成，并已提供包含示例的配置文件。最后，使用 `ppp` 命令来启动和管理连接。
