...
将所需的用户添加到此机器并重启。为了进行测试，尝试在两个控制台上以不同用户身份登录。运行 `ps aux` 查看是否可以看到其他用户的进程。验证运行 [ls(1)](https://man.freebsd.org/cgi/man.cgi?query=ls&sektion=1&format=html) 是否无法访问其他用户的主目录。
