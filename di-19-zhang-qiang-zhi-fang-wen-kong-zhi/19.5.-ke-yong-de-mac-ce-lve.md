...
* 保密性，或防止不当泄露数据。
* 为处理具有多个敏感级别的数据的系统提供了设计基础，确保秘密与机密信息之间不发生信息泄漏。
...
除了三个基本标签选项外，管理员还可以根据需要将用户和组分组，以阻止它们之间的信息流动。使用描述性词语（如 `Confidential`、`Secret` 和 `Top Secret`）来查看许可级别中的信息可能更容易。一些管理员会根据项目级别创建不同的组。无论采用何种分类方法，都必须在实施限制性策略之前制定出合理的计划。
...
在 Biba 环境中，每个主体或对象都会设置一个"完整性"标签。这些标签由分层的等级和非分层的隔离区组成。随着等级的升高，其完整性也会提高。
