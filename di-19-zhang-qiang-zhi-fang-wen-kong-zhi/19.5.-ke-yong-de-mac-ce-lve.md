...
启用此策略时，用户只能看到自己的进程以及自己所在分区内的进程，无法使用超出该分区范围的工具。例如，属于 `insecure` 分级的用户将无法访问 `top`，以及许多其他必须生成进程的命令。

以下示例将 `top` 命令添加到 `insecure` 分级用户的标签集中。所有由 `insecure` 分级用户生成的进程将保持在 `partition/13` 标签中：
...
* 保密性，或防止不当泄露数据。
* 为处理具有多个敏感级别的数据的系统提供了设计基础，确保机密与秘密信息之间不发生信息泄漏。
