...
打开计算机并引导操作系统这个过程本身就提出了一个有趣的难题。根据定义，在操作系统引导之前，计算机本身什么也不会做，包括无法从磁盘运行程序。如果计算机在没有操作系统的情况下无法从磁盘运行程序，而操作系统的程序又正好存放在磁盘上，那操作系统究竟是如何被启动的呢？
...
> **注意**
>
> FreeBSD 支持从旧的 MBR 标准和较新的 全局唯一标识分区表（GPT）引导。GPT 分区方式通常用于带有统一可扩展固件接口（UEFI）的计算机。不过，即使是在仅有传统 BIOS 的机器上，FreeBSD 也可以使用 [gptboot(8)](https://man.freebsd.org/cgi/man.cgi?query=gptboot&sektion=8&format=html) 从 GPT 分区引导。目前正在开发直接从 UEFI 引导的支持。
