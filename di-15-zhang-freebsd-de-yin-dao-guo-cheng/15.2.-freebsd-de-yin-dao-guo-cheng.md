...
待内核被 loader 或跳过 loader 的 boot2 加载，它会检查所有引导标志（boot flags），并据此调整其行为。[引导时内核交互](./15.2.-freebsd-de-yin-dao-guo-cheng.md) 中列出了常用的引导标志。更多启动标志的详细信息请参见 [boot(8)](https://man.freebsd.org/cgi/man.cgi?query=boot&sektion=8&format=html)。
...
内核完成启动后，将控制权交给用户进程 [init(8)](https://man.freebsd.org/cgi/man.cgi?query=init&sektion=8&format=html)，该进程位于 **/sbin/init**，或由 `loader` 中 `init_path` 变量指定的程序路径。这是整个引导过程的最后阶段。
