...
FAT 文件系统是简单可靠的文件系统。尽管它在性能、可靠性和可扩展性方面不如现代替代方案，但由于它在多种操作系统上都可用，仍然是设备间数据交换的常见选择。

要访问 FAT 文件系统，指定其 FreeBSD 分区名和一个已存在的挂载点，即可挂载该 FAT 卷。以下示例将 **/dev/ada0s1** 挂载到 **/mnt**：
...
exFAT（扩展文件分配表）是为闪存设备（如 USB 驱动器和 SD 卡）优化的轻量级文件系统。它支持大容量文件，并在多个平台上广泛使用，因而非常适用于外部存储设备。
...
通过指定其 FreeBSD 分区名和一个已存在的挂载点，即可挂载 exFAT 卷。以下示例将 **/dev/ada0s1** 挂载到 **/mnt**：
...
NTFS 是 Microsoft® 开发的健壮文件系统，常用于 Windows 操作系统。FreeBSD 通过 [filesystems/ntfs](https://cgit.freebsd.org/ports/tree/filesystems/ntfs/) 软件包提供对 NTFS 的完整读写支持，便于访问和修改 NTFS 格式的存储设备。
...
通过指定其 FreeBSD 分区名和一个已存在的挂载点，即可挂载 NTFS 卷。以下示例将 **/dev/ada0s1** 挂载到 **/mnt**：
