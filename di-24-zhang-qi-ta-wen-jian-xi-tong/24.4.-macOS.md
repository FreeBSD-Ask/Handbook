...
HFS/HFS+ 是 MacOS 在 APFS 问世前的主要文件系统，常见于较旧的 Mac 设备和外部驱动器。FreeBSD 通过 [filesystems/hfsfuse](https://cgit.freebsd.org/ports/tree/filesystems/hfsfuse/) 软件包提供了对 HFS/HFS+ 的只读支持。
...
通过指定其 FreeBSD 分区名和一个已存在的挂载点，即可挂载 HFS/HFS+ 卷。以下示例将 **/dev/ada0s1** 挂载到 **/mnt**：
