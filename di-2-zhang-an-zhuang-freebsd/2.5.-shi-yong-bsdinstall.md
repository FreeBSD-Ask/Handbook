...
从 FreeBSD 15.0 开始，安装程序提供了两种安装和更新基本系统的方法：
...
- **Distribution Sets（发行版集合）**：经典安装方式。基本系统从压缩 tar 包中解压。更新使用 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html)。此方法"完全受支持、稳定，且在 15.x 系列中仍是大多数安装介质的默认选项。"
- **Packages（软件包，技术预览）**：将基本系统作为独立软件包从 FreeBSD-base 仓库安装。基本系统和第三方软件均可通过一条 `pkg upgrade` 命令统一升级。此方法（pkgbase）将系统管理统一到 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 之下。
