...
无线网络由服务集标识符 (SSID) 标识; 为每个网络指定的简短、独特名称。扫描期间找到的 SSID 列出，然后列出该网络可用的加密类型的描述。如果所需的 SSID 不出现在列表中，请选择重新扫描以再次扫描。如果所需网络仍未显示，请检查天线连接是否存在问题，或尝试将计算机移动到靠近访问点的位置。在进行每次更改后重新扫描。
...
接下来，输入连接到所选无线网络的加密信息。强烈建议使用 WPA2 加密，而不是较旧的加密类型，如 WEP，后者提供了很少的安全性。如果网络使用 WPA2，请输入密码，也称为预共享密钥（PSK）。出于安全原因，输入框中输入的字符显示为星号。
...
IPv4 配置有两种方法。如果网络提供 DHCP 服务器，则 DHCP 将自动正确配置网络接口，并应使用。否则，需要手动输入地址信息作为静态配置。

|  | 请勿输入随机网络信息，否则将无法正常工作。如果没有 DHCP 服务器，请向网络管理员或互联网服务提供商获取“所需网络信息”中列出的信息。 |
| -- | ------------------------------------------------------------------------------------------------------------------------------------ |

如果存在 DHCP 服务器，请在下一个菜单中选择是，以自动配置网络接口。安装程序将似乎暂停一分钟左右，因为它找到 DHCP 服务器并获取系统的寻址信息。
...
* IP Address - 分配给此计算机的 IPv4 地址。地址必须是唯一的，并且本地网络上的其他设备尚未使用。
* Subnet Mask - 网络的子网掩码。
* Default Router - 网络默认网关的 IP 地址。

下一个屏幕将询问接口是否应配置为 IPv6。如果 IPv6 可用并且需要，选择“是”以选择它。
...
如果有 IPv6 路由器可用，请在下一个菜单中选择“是”，以自动配置网络接口。安装程序将似乎在找到路由器并获取系统的寻址信息时停顿一分钟左右。
...
* IPv6 Address - 分配给此计算机的 IPv6 地址。该地址必须是唯一的，不得已被本地网络上的另一设备使用。
* Default Router - 网络默认网关的 IPv6 地址。

最后一个网络配置菜单用于配置域名系统（DNS）解析器，该解析器将主机名转换为网络地址，并从网络地址转换回主机名。如果使用 DHCP 或 SLAAC 自动配置网络接口，则 Resolver Configuration 值可能已经填写。否则，在 Search 字段中输入本地网络的域名。 DNS #1 和 DNS #2 是 DNS 服务器的 IPv4 和/或 IPv6 地址。至少需要一个 DNS 服务器。
...
一旦界面配置完成，请选择一个位于与安装 FreeBSD 的计算机相同地区的镜像站点。当镜像站点靠近目标计算机时，可以更快地检索文件，从而减少安装时间。
...
* ntpdate - 在启动时启用自动时钟同步。请注意，此程序的功能现在已经包含在 ntpd(8) 守护程序中，ntpdate(8) 实用程序将很快被弃用。
* ntpd - 用于自动时钟同步的网络时间协议（NTP）守护程序。如果您希望将系统时钟与远程时间服务器或时间池同步，请启用此服务。
* powerd - 用于电源控制和节能的系统电源控制实用程序。
* dumpdev - 在调试系统问题时，崩溃转储非常有用，因此鼓励用户启用它们。
...
* hide_jail - 隐藏在监狱中运行的进程。这可以防止非特权用户看到在监狱中运行的进程。
...
* enable_aslr - 启用地址布局随机化。有关地址布局随机化的更多信息，可以查阅维基百科文章。

### 2.8.6. 添加用户

下一个菜单提示至少创建一个用户帐户。建议使用用户帐户登录系统，而不是 root 。以 root 登录时，实际上可以做任何事情，没有任何限制或保护。以普通用户身份登录更安全更可靠。
...
按照提示输入用户帐户所需的信息。在“输入用户信息”中显示的示例创建 asample 用户帐户。
...
* Username - 用户将输入的名称。通常的约定是使用名字的第一个字母与姓氏结合在一起作为登录名，只要系统中每个用户名是唯一的。用户名区分大小写，不应包含任何空格。
* Full name - 用户的全名。这可以包含空格，并被用作用户帐户的描述。
* Uid - 用户 ID。通常会留空，这样系统会自动分配一个值。
* Login group - 用户组。通常会保留为空以接受默认值。
* Invite *user* into other groups? - 用户将被添加为成员的其他组。如果用户需要管理访问权限，请在此处输入 wheel 。
* Login class - 通常保持默认值为空。
* Shell - 输入列表中的一个值以为用户设置交互式 shell。有关 shell 的更多信息，请参阅 Shells。
...
如果有更多用户要添加，请用 Add another user? 答复问题 yes 。输入 no 完成添加用户并继续安装。
...
当 FreeBSD 启动时，会显示信息提示。系统启动完成后，会显示登录提示符。在 login: 提示符下，输入安装过程中添加的用户名。避免以 root 登录。请参阅超级用户帐户，了解在需要管理访问权限时如何成为超级用户的说明。
...
FreeBSD 不会默认安装图形环境。有关安装和配置图形窗口管理器的更多信息，请参阅 X 窗口系统。

对 FreeBSD 计算机进行适当关机有助于保护数据和硬件免受损坏。在系统正确关机之前，请勿关闭电源！如果用户是 wheel 组的成员，请在命令行上输入 su 以成为超级用户，并输入 root 密码。然后，键入 shutdown -p now ，系统将干净地关闭，如果硬件支持，会自动关闭。
