...
下个界面用于配置系统启动时将启动哪些系统服务。所有这些服务都是可选的。只启动系统正常运行所需的服务。
...
当 FreeBSD 启动时，会显示启动信息。系统启动完成后，将显示登录提示。在 `login:` 提示符下，输入安装过程中添加的用户名。避免以 `root` 身份登录。有关如何在需要管理访问时成为超级用户的说明，请参见[超级用户账户](../di-3-zhang-freebsd-ji-chu/3.3.-yong-hu-he-ji-ben-zhang-hu-guan-li.md)。
...
如果在[选择要启用的附加服务](./2.1.-gai-shu.md)中启用了 sshd，则第一次启动可能会稍慢，因为系统正在生成 SSH 主机密钥。后续启动会更快。然后将显示密钥的指纹，如以下示例所示：
