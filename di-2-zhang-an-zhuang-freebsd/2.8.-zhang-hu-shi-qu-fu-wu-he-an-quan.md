...
如果选择以太网接口，安装程序将跳转到菜单[选择 IPv4 网络](./2.8.-zhang-hu-shi-qu-fu-wu-he-an-quan.md)。如果选择无线网络接口，系统将扫描无线接入点：

![显示无线网络扫描的菜单](https://docs.freebsd.org/images/books/handbook/bsdinstall/bsdinstall-configure-wireless-scan.png)

**图 36. 扫描无线接入点**

无线网络通过服务集标识 (SSID) 识别；每个网络都有一个简短、独特的名称。在扫描过程中会列出找到的 SSID，然后提供该网络的可用加密类型的概述。如果所需 SSID 未出现在列表中，可选择 **Rescan** 重新扫描。若所需网络仍未出现，请检查天线连接是否存在问题，或尝试将计算机移近接入点。每次更改后都应重新扫描。

![显示不同无线网络供连接的菜单](https://docs.freebsd.org/images/books/handbook/bsdinstall/bsdinstall-configure-wireless-accesspoints.png)

**图 37. 选择无线网络**

接下来，输入连接到选定无线网络的加密信息。强烈建议使用 WPA2 加密，而不是几乎不提供安全性的旧加密类型，如 WEP。若网络使用 WPA2，请输入密码，也称为预共享密钥 (PSK)。出于安全原因，输入框中输入的字符将以星号显示。
...
> **注意**
>
> 请不要随意输入网络信息，因为那样将无法正常工作。若没有 DHCP 服务器，请从网络管理员或互联网服务提供商处获取[所需网络信息](./2.3.-an-zhuang-qian-de-zhun-bei-gong-zuo.md)中列出的信息。
...
接下来的界面将询问是否应为接口配置 IPv6。如果 IPv6 可用且需要，请选择 **Yes**。
...
在接口配置完成后，选择一个位于与 FreeBSD 正在安装的计算机相同区域的镜像站。当镜像站靠近目标计算机时，可以更快地下载文件，从而减少安装时间。

> **技巧**
>
> 选择 `http://download.freebsd.org Main Site (GeoDNS, HTTP)` 或 `ftp://download.freebsd.org Main Site (GeoDNS, FTP)` 将自动路由到最近的镜像。
...
下个界面用于配置系统引导时将启动哪些系统服务。所有这些服务都是可选的。只启动系统正常运行所需的服务。
...
* `sshd`——安全 shell（SSH）守护进程用于通过加密连接远程访问系统。应仅在能远程登录系统时才启用此服务。
* `ntpd`——网络时间协议（NTP）守护进程，用于自动时钟同步。如果希望将系统时钟与远程时间服务器或池同步，请启用此服务。
* `ntpd_sync_on_start`——启用后，ntpd 在引导时以 `-g` 参数运行，在开机时立即执行一次性时钟同步。
...
* `disable_ddtrace`——DTrace 可以以影响运行内核的模式运行。除非明确启用，否则不能使用破坏性操作。在使用 DTrace 时，请使用 `-w` 来启用此选项。有关更多信息，请参见 [dtrace(1)](https://man.freebsd.org/cgi/man.cgi?query=dtrace&sektion=1&format=html)。
...
按照提示输入用户账户的所需信息。示例中所示[输入用户信息](./2.8.-zhang-hu-shi-qu-fu-wu-he-an-quan.md)创建 `asample` 用户账户。
...
* `Shell`——输入所列值之一以设置用户的交互式 shell。有关 shell 的更多信息，请参见 [Shell](../di-3-zhang-freebsd-ji-chu/3.9.-shell.md)。
...
* `Enter password again`——必须再次输入密码以验证。
* `Lock out the account after creation?`——通常选择 `no`，以便用户可以登录。

输入所有详细信息后，将显示摘要以供检查。如果发现错误，输入 `no` 更正。一切正确后，输入 `yes` 创建新用户。
...
有关添加用户和用户管理的更多信息，请参见[用户和基本账户管理](../di-3-zhang-freebsd-ji-chu/3.3.-yong-hu-he-ji-ben-zhang-hu-guan-li.md)。

## 2.8.7. 最终配置

在所有内容安装和配置完成后，将提供最后一次修改设置的机会。

![显示在完成安装前执行的不同操作的菜单。例如：添加用户](https://docs.freebsd.org/images/books/handbook/bsdinstall/bsdinstall-finalconfiguration.png)

**图 58. 最终配置**

使用此菜单做任何更改或额外配置，然后完成安装。

* `Add User`—— [添加用户](./2.8.-zhang-hu-shi-qu-fu-wu-he-an-quan.md#id-2.8.6.-tian-jia-yong-hu)中概述。
* `Root Password`——在[设置 `root` 密码](./2.8.-zhang-hu-shi-qu-fu-wu-he-an-quan.md#id-2.8.1.-she-zhi-root-mi-ma)中概述。
* `Hostname`——在[设置主机名](./2.5.-shi-yong-bsdinstall.md)中概述。
* `Network`——在[配置网络接口](./2.8.-zhang-hu-shi-qu-fu-wu-he-an-quan.md#id-2.8.2.-pei-zhi-wang-luo-jie-kou)中概述。
* `Services`——在[启用服务](./2.8.-zhang-hu-shi-qu-fu-wu-he-an-quan.md#id-2.8.4.-qi-yong-fu-wu)中概述。
* `System Hardening`——在[启用安全加固选项](./2.8.-zhang-hu-shi-qu-fu-wu-he-an-quan.md#id-2.8.5.-qi-yong-an-quan-jia-gu-xuan-xiang)中概述。
* `Time Zone`——在[设置时区](./2.8.-zhang-hu-shi-qu-fu-wu-he-an-quan.md#id-2.8.3.-she-zhi-shi-qu)中概述。
* `Firmware`——下载并安装系统组件（如 Wi-Fi 或显卡）所需的固件。
* `Handbook`——下载、安装 FreeBSD 手册。

配置完成后，选择 **Exit**。

![显示安装已完成的菜单，并询问是否要打开 shell 做手动更改](https://docs.freebsd.org/images/books/handbook/bsdinstall/bsdinstall-final-modification-shell.png)

**图 59. 手动配置**

bsdinstall 会提示需要在重启新系统之前完成的任何额外配置。选择 **Yes** 以退出到新系统中的 shell，或选择 **No** 以继续安装的最后一步。

![显示安装已完成并询问是否重启系统或访问 Live CD 的菜单](https://docs.freebsd.org/images/books/handbook/bsdinstall/bsdinstall-mainexit.png)

**图 60. 完成安装**

如果需要进一步的配置或特殊设置，选择 **Live CD** 以将安装媒体引导为 Live CD 模式。

如果安装完成，选择 **Reboot** 以重启计算机，引导新的 FreeBSD 系统。不要忘记移除 FreeBSD 安装媒体，否则计算机可能会再次从中引导。

当 FreeBSD 引导时，会显示引导信息。系统引导完成后，将显示登录提示。在 `login:` 提示符下，输入安装过程中添加的用户名。避免以 `root` 身份登录。有关如何在需要管理访问时成为超级用户的说明，请参见[超级用户账户](../di-3-zhang-freebsd-ji-chu/3.3.-yong-hu-he-ji-ben-zhang-hu-guan-li.md)。

在引导过程中出现的信息可以通过按 **Scroll-Lock** 来打开回滚缓冲区查看。可以使用 **PgUp**、**PgDn** 和方向键向上滚动查看信息。完成后，再次按 **Scroll-Lock** 解锁显示并返回控制台。要在系统运行一段时间后查看这些信息，可以从命令提示符下输入 `less /var/run/dmesg.boot`。查看后按 **q** 返回命令行。

如果在[选择要启用的附加服务](./2.8.-zhang-hu-shi-qu-fu-wu-he-an-quan.md)中启用了 sshd，则第一次引导可能会稍慢，因为系统正在生成 SSH 主机密钥。后续引导会更快。然后将显示密钥的指纹，如以下示例所示：
...
有关指纹和 SSH 的更多信息，请参见 [OpenSSH](../di-16-zhang-an-quan/16.7.-openssh.md)。

FreeBSD 默认不安装图形环境。有关安装和配置图形窗口管理器的更多信息，请参见 [X Window 系统](../di-5-zhang-xwindow-xi-tong/5.1.-gai-shu.md)。

正确关闭 FreeBSD 计算机有助于保护数据和硬件免受损坏。*在系统没有正确关闭之前，请勿关闭电源！* 如果用户是 `wheel` 组的成员，可以在命令行输入 `su`，再输入 `root` 密码以成为超级用户。然后，输入 `shutdown -p now`，系统可安全关闭，在硬件支持的情况下会自动关机。
