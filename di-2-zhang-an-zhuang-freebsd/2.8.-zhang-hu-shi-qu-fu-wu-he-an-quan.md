...
- `ntpdate` - 在启动时启用自动时钟同步。请注意，此程序的功能现在已经在[ntpd(8)](https://man.freebsd.org/cgi/man.cgi?query=ntpd&sektion=8&format=html)守护程序中实现，并且[ntpdate(8)](https://man.freebsd.org/cgi/man.cgi?query=ntpdate&sektion=8&format=html)实用程序将很快被淘汰。
...
- `disable_syslogd` - 禁用 syslogd 网络套接字的打开。默认情况下，FreeBSD 以安全方式使用`-s`运行 syslogd。这会阻止守护程序在端口 514 上监听传入的 UDP 请求。启用此选项后，syslogd 将以`-ss`运行，这会阻止 syslogd 打开任何端口。有关更多信息，请参阅[syslogd(8)](https://man.freebsd.org/cgi/man.cgi?query=syslogd&sektion=8&format=html)。
...
- `Full name` - 用户的全名。可以包含空格，用作用户帐户的描述。
- `Uid` - 用户 ID。通常留空，以便系统自动分配一个值。
- `Login group` - 用户的组。通常留空以接受默认值。
- `Invite<span> </span><em>user</em><span> </span>into other groups?` - 将用户添加为成员的其他组。如果用户需要管理员访问权限，在此处键入`wheel`。
