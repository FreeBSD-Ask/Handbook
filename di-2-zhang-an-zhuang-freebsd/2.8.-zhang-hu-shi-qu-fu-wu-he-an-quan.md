...
如果选择以太网接口，安装程序将跳转到菜单[选择 IPv4 网络](./2.1.-gai-shu.md)。如果选择无线网络接口，系统将扫描无线接入点：
...
> **注意**
>
> 请不要随意输入网络信息，因为那样不会起作用。若没有 DHCP 服务器，请从网络管理员或互联网服务提供商处获取[所需网络信息](./2.1.-gai-shu.md)中列出的信息。
...
按照提示输入用户账户的所需信息。示例中所示[输入用户信息](./2.1.-gai-shu.md)创建 `asample` 用户账户。
...
* `Add User`—— [添加用户](./2.1.-gai-shu.md)中概述。
* `Root Password`——在[设置 `root` 密码](./2.1.-gai-shu.md)中概述。
* `Hostname`——在[设置主机名](./2.1.-gai-shu.md)中概述。
* `Network`——在[配置网络接口](./2.5.-shi-yong-bsdinstall.md)中概述。
* `Services`——在[启用服务](./2.1.-gai-shu.md)中概述。
* `System Hardening`——在[启用安全加固选项](./2.1.-gai-shu.md)中概述。
* `Time Zone`——在[设置时区](./2.1.-gai-shu.md)中概述。
...
如果在[选择要启用的附加服务](./2.1.-gai-shu.md)中启用了 sshd，则第一次引导可能会稍慢，因为系统正在生成 SSH 主机密钥。后续引导会更快。然后将显示密钥的指纹，如以下示例所示：
