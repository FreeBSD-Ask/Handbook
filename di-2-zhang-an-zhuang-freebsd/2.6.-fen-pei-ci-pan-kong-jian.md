...
* `Auto (ZFS)` 分区将创建一个使用 ZFS 作为 root 分区的系统，并为_启动环境_提供可选的 GELI 加密支持。
...
| BSD |                                    无 MBR 的 BSD 标签，有时也称作_危险专用模式_，因为非 BSD 磁盘工具可能无法识别它                                   |
...
可以用常见的缩写来输入 `Size`：_K_ 代表千字节，_M_ 代表兆字节，_G_ 代表千兆字节。

> **技巧**
>
> 正确的扇区对齐才能提供最好的性能，使分区大小为 4K 字节的偶数倍有助于确保在具有 512 字节或 4K 字节扇区的设备上对齐。一般来说，使用 1M 或 1G 的偶数倍的分区大小是确保每个分区从 4K 的偶数倍开始的最简单方法。有一个例外：由于目前启动代码的限制，_freebsd-boot_ 分区不应大于 512K。
