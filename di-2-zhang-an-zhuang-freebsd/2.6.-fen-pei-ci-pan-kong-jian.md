...
在配备多块 SCSI 磁盘或多个在不同控制器上运行的 IDE 磁盘的大型机上，建议为每个驱动器都配置交换分区，最多可达四个驱动器。交换分区的大小应大致相同。内核可以处理任意大小，但内部数据结构的规模为最大交换分区的 4 倍。保持交换分区接近相同大小能让内核在磁盘上优化交换空间的条带化。较大的交换分区可能会引发内核警告信息——关于总配置交换分区。通过增加用于跟踪交换分配的内存量，可以提高此限制，如警告信息所示。在强制重启之前，可能更容易从失控程序中恢复。
