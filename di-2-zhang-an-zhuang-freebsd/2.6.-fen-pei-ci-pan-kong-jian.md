...
* `mirror`——镜像在每块磁盘上存储所有数据的完整副本。镜像提供良好的读取性能，因为数据是从所有磁盘并行读取的。写入性能较慢，因为数据必须写入池中的所有磁盘。能让除了一块磁盘外的所有磁盘故障。此选项至少需要两块磁盘。
* `raid10`——条带镜像。提供最佳性能，但存储容量最少。此选项需要偶数块磁盘，至少四块磁盘。
* `raidz1`——单重冗余 RAID。能有一块磁盘同时故障。此选项至少需要三块磁盘。
* `raidz2`——双重冗余 RAID。能有两块磁盘同时故障。此选项至少需要四块磁盘。
* `raidz3`——三重冗余 RAID。能有三块磁盘同时故障。此选项至少需要五块磁盘。
