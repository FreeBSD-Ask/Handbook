...
要继续安装过程，请转到[获取分发文件](./2.7.-huo-qu-an-zhuang-wen-jian.md)。
...
创建自定义分区后，选择 **Finish** 可继续安装，并转到 [获取发行版文件](./2.7.-huo-qu-an-zhuang-wen-jian.md)。
...
* `Encrypt Disks?`——加密磁盘允许用户使用 GELI 对磁盘进行加密。有关磁盘加密的更多信息，请参见 [“使用 geli 进行磁盘加密”](../di-21-zhang-cun-chu/21.12.-jia-mi-ci-pan-fen-qu.md)。按 **回车键** 选择是否激活此选项。
