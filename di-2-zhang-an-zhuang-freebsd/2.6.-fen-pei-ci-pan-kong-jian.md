...
|缩写|说明||:----:|:----||APM|Apple 分区表，供 PowerPC® 使用。||BSD|无 MBR 的 BSD 标签，有时称为 *危险专用模式*，因为非 BSD 磁盘工具可能无法识别它。||GPT|[GUID 分区表](https://en.wikipedia.org/wiki/GUID_Partition_Table)。||MBR|[主引导记录](https://en.wikipedia.org/wiki/Master_boot_record)。|
