...
* `Manual` 分区能让专业用户从菜单选项中创建自定义分区。
* `Shell` 将打开命令行提示符，专业用户可使用命令行工具（如 [gpart(8)](https://man.freebsd.org/cgi/man.cgi?query=gpart&sektion=8&format=html)、[fdisk(8)](https://man.freebsd.org/cgi/man.cgi?query=fdisk&sektion=8&format=html) 和 [bsdlabel(8)](https://man.freebsd.org/cgi/man.cgi?query=bsdlabel&sektion=8&format=html)）创建自定义分区。
...
> **注意**
>
> 有时，**/var/tmp** 需要大量磁盘空间。安装新软件时，打包工具会在 **/var/tmp** 下解压缩软件包的临时副本。如果 **/var/tmp** 下没有足够的磁盘空间，在安装大型软件包（如 Firefox/LibreOffice）时可能会出现问题。
...
作为经验法则，交换分区的大小应约为物理内存（RAM）大小的两倍。内存较少的系统在配置更大的交换分区时可能表现更好（在较大内存配置下交换分区可以更小）。配置过小的交换分区可能会导致虚拟内存页面扫描代码效率低下，并在增加更多内存时可能出现问题。

在配备多块 SCSI 磁盘或多个在不同控制器上运行的 IDE 磁盘的大型系统上，建议为每个驱动器都配置交换分区，最多可达四个驱动器。交换分区的大小应大致相同。内核可以处理任意大小，但内部数据结构的规模为最大交换分区的 4 倍。保持交换分区大小相近将使内核能够在磁盘之间最优地划分交换空间。较大的交换空间可能会引发内核关于总配置交换的警告信息。如警告信息所示，通过增加用于跟踪交换分区分配的内存量，可以提高此限制。在强制重启之前，可能更容易从失控程序中恢复。
...
下一个菜单显示可用分区方案类型的列表。对于 amd64 计算机，GPT 通常是最合适的选择。对于不兼容 GPT 的旧计算机，应使用 MBR。其他分区方案通常用于不常见或旧计算机。更多信息请参见 [分区方案](./2.6.-fen-pei-ci-pan-kong-jian.md)。
...
磁盘配置完成后，下一菜单提供最后一次机会在格式化选定驱动器之前做更改。如果需要更改，请选择 **Back** 返回到主分区菜单。选择 **Revert & Exit** 将在不对驱动器做任何更改的情况下退出安装程序。选择 **Commit** 可开始安装过程。
...
要继续安装过程，请转到[获取安装文件](./2.7.-huo-qu-an-zhuang-wen-jian.md)。
...
| GPT | [全局唯一标识分区表](https://en.wikipedia.org/wiki/GUID_Partition_Table)。 |
...
* `freebsd-boot`/`efi`——存放 FreeBSD 引导代码。
* `freebsd-ufs`——FreeBSD UFS 文件系统。
* `freebsd-zfs`——FreeBSD ZFS 文件系统。更多有关 ZFS 的信息，请参见 [Z 文件系统 (ZFS)](../di-23-zhang-z-wen-jian-xi-tong-zfs/23.1.-shi-shi-mo-shi-zfs-yu-zhong-bu-tong.md)。
...
> **技巧**
>
> 注意，**/tmp** 可以稍后作为基于内存的文件系统（[tmpfs(5)](https://man.freebsd.org/cgi/man.cgi?query=tmpfs&sektion=4&format=html)）添加，前提是系统内存足够。
...
> **技巧**
>
> 正确的扇区对齐能提供最佳性能，将分区大小设置为 4K 字节的整数倍，有助于确保在使用 512 字节或 4K 字节扇区的磁盘上实现对齐。通常，将分区大小设置为 1M 或 1G 的整数倍是最简单的方法，以确保每个分区都从 4K 的整数倍位置开始。有一个例外：用于 BIOS 引导的 *freebsd-boot* 分区由于传统引导代码的限制，其大小不应超过 512K。此限制不适用于 UEFI 引导。
...
创建自定义分区后，选择 **Finish** 可继续安装，并转到 [获取安装文件](./2.7.-huo-qu-an-zhuang-wen-jian.md)。
...
* `Install`——使用选定的选项继续安装。
* `Pool Type/Disks`——配置 `Pool Type` 和将构成池的磁盘。自动 ZFS 安装程序目前仅支持创建一个顶级 vdev，除非在条带模式下。要创建更复杂的池，请参阅 [Shell 模式分区](./2.6.-fen-pei-ci-pan-kong-jian.md#id-2.6.5.-shell-mo-shi-fen-qu) 中的说明。
...
* `Encrypt Disks?`——加密磁盘能让用户使用 GELI 加密磁盘。有关磁盘加密的更多信息，请参见 [“使用 geli 进行磁盘加密”](../di-21-zhang-cun-chu/21.12.-jia-mi-ci-pan-fen-qu.md)。按 **回车键** 选择是否激活此选项。
* `Partition Scheme`——选择分区方案。大多数情况下推荐使用 GPT。按 **回车键** 在不同选项之间选择。
* `Swap Size`——确定交换空间的大小。
* `Mirror Swap?`——是否在磁盘之间镜像交换。请注意，启用镜像交换将破坏崩溃转储。按 **回车键** 激活或不激活此选项。
* `Encrypt Swap?`——是否加密交换。每次系统引导时将使用临时密钥加密交换，并在重启时丢弃。按 **回车键** 选择是否激活此选项。有关交换加密的更多信息，请参见 [“加密交换分区”](../di-21-zhang-cun-chu/21.13.-jia-mi-jiao-huan-fen-qu.md)。
* `ZFS Pool Options`——指定创建 ZFS 根池时要应用的附加池属性。有关选项的更多信息，请参见 [zpool-create(8)](https://man.freebsd.org/cgi/man.cgi?query=zpool-create&sektion=8&format=html) 和 [zpoolprops(7)](https://man.freebsd.org/cgi/man.cgi?query=zpoolprops&sektion=7&format=html)。
...
* `mirror`——镜像在每块磁盘上存储所有数据的完整副本。镜像提供良好的读取性能，因为数据是从所有磁盘并行读取的。写入性能较慢，因为数据必须写入池中的所有磁盘。可容忍除了一块磁盘外的所有磁盘故障。此选项至少需要两块磁盘。
* `raid10`——条带镜像。提供最佳性能，但存储容量最少。此选项需要偶数块磁盘，至少四块磁盘。
* `raidz1`——单重冗余 RAID。可容忍一块磁盘同时故障。此选项至少需要三块磁盘。
* `raidz2`——双重冗余 RAID。可容忍两块磁盘同时故障。此选项至少需要四块磁盘。
* `raidz3`——三重冗余 RAID。可容忍三块磁盘同时故障。此选项至少需要五块磁盘。
...
如果列表中缺少一块或多块磁盘，如果在引导安装程序后新加了磁盘，请选择 `Rescan Devices` 可刷新可用磁盘的列表。
...
安装过程将正常继续。要继续安装，请访问[获取安装文件](./2.7.-huo-qu-an-zhuang-wen-jian.md)。

## 2.6.5. Shell 模式分区

在高级安装时，bsdinstall 的分区菜单可能无法提供所需的灵活性。专业用户可以从分区菜单中选择 **Shell** 选项，以手动对驱动器分区，创建文件系统，填写 **/tmp/bsdinstall_etc/fstab**，并在 **/mnt** 下挂载文件系统。在完成后，输入 `exit` 可返回 bsdinstall 继续安装。
