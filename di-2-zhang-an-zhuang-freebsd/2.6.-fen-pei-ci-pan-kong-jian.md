...
选择磁盘后，下一菜单提示要么安装到整个磁盘，要么使用空闲空间创建分区。如果选择 **Entire Disk**，则会自动创建填充整个磁盘的一般分区布局。选择 **Partition** 则会从磁盘上的未使用空间创建分区布局。
...
创建分区布局后，请检查以确保满足安装需求。选择 **Revert** 将重置分区为其原始值。按 **Auto** 将重新创建自动 FreeBSD 分区。分区也可以手动创建、修改或删除。当分区正确时，选择 **Finish** 继续安装。
...
选择并创建分区方案后，再次选择 **Create** 以创建分区。使用 **Tab** 可以将焦点移到字段（在循环浏览 **<OK>**、**<Options>** 和 **<Cancel>** 后）。
...
>**技巧**
>
>正确的扇区对齐可以提供最佳性能，且将分区大小设为 4K 字节的偶数倍有助于确保在 512 字节/4K 字节扇区的驱动器上保持对齐。一般来说，使用 1M/1G 的偶数倍分区大小是确保每个分区从 4K 的偶数倍开始的最简单方法。有一个例外：由于当前引导代码的限制，*freebsd-boot* 分区的大小不得超过 512K。 
...
创建自定义分区后，选择 **Finish** 以继续安装，并转到 [获取发行版文件](https://docs.freebsd.org/en/books/handbook/bsdinstall/#bsdinstall-fetching-distribution)。
...
* `Disk Info` - 此菜单可用于检查每个磁盘，包括其分区表以及设备型号和序列号等其他信息（如果可用）。
...
* `stripe` - 条带化提供所有连接设备的最大存储，但没有冗余。如果仅有一个磁盘故障，池中的数据将永久丢失。
* `mirror` - 镜像在每个磁盘上存储所有数据的完整副本。镜像提供良好的读取性能，因为数据是从所有磁盘并行读取的。写入性能较慢，因为数据必须写入池中的所有磁盘。允许除了一个磁盘外的所有磁盘故障。此选项需要至少两个磁盘。
...
选择了 `Pool Type` 后，将显示可用磁盘的列表，系统会提示用户选择一个或多个磁盘以构成池。然后将验证配置，以确保选择了足够的磁盘。如果验证失败，请选择 **<Change Selection>** 返回磁盘列表，或选择 **<Back>** 更改 `Pool Type`。
...
在进行高级安装时，bsdinstall 分区菜单可能无法提供所需的灵活性。高级用户可以从分区菜单中选择 **Shell** 选项，以手动对驱动器进行分区，创建文件系统，填充 `/tmp/bsdinstall_etc/fstab`，并在 `/mnt` 下挂载文件系统。完成后，输入 `exit` 可返回 bsdinstall 并继续安装。
