...
* `Encrypt Disks?`——加密磁盘能让用户使用 GELI 对磁盘进行加密。有关磁盘加密的更多信息，请参见 [“使用 geli 进行磁盘加密”](../di-21-zhang-cun-chu/21.12.-jia-mi-ci-pan-fen-qu.md)。按 **回车键** 选择是否激活此选项。
...
安装过程将正常进行。要继续安装，请访问[获取安装文件](./2.7.-huo-qu-an-zhuang-wen-jian.md)。

## 2.6.5. Shell 模式分区

在进行高级安装时，bsdinstall 的分区菜单可能无法提供所需的灵活性。专业用户可以从分区菜单中选择 **Shell** 选项，以手动对驱动器进行分区，创建文件系统，填写 **/tmp/bsdinstall_etc/fstab**，并在 **/mnt** 下挂载文件系统。在完成后，输入 `exit` 可返回 bsdinstall 继续安装。
