...
可能需要在此文件中修改其他配置设置，例如社区名称和访问列表。有关更多信息，请参见 [bsnmpd(1)](https://www.freebsd.org/cgi/man.cgi?query=bsnmpd\&sektion=1\&format=html) 和 [snmp\_bridge(3)](https://www.freebsd.org/cgi/man.cgi?query=snmp\_bridge\&sektion=3\&format=html)。一旦这些编辑被保存，将此行添加到 **/etc/rc.conf**：
