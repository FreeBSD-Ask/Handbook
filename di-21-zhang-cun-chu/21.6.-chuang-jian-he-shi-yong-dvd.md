...
* **DVD-RAM**：这是一个可重写格式，可以视为可移动硬盘。然而，这种介质与大多数 DVD-ROM 驱动器和 DVD-Video 播放器不兼容，因为只有少数 DVD 写入器支持 DVD-RAM 格式。有关 DVD-RAM 使用的更多信息，请参阅 [使用 DVD-RAM](./20.6.-chuang-jian-he-shi-yong-dvd.md)。
...
这些工具使用 SCSI 子系统访问设备，因此必须加载或静态编译 [ATAPI/CAM 支持](./20.5.-chuang-jian-he-shi-yong-cd.md) 到内核中。如果刻录机使用 USB 接口，则无需此支持。有关 USB 设备配置的更多详细信息，请参阅 [USB 存储设备](./20.4.-usb-cun-chu-she-bei.md)。
...
由于 [growisofs(1)](https://man.freebsd.org/cgi/man.cgi?query=growisofs&sektion=1&format=html) 是 [mkisofs](./20.5.-chuang-jian-he-shi-yong-cd.md) 的前端，它将调用 [mkisofs(8)](https://man.freebsd.org/cgi/man.cgi?query=mkisofs&sektion=8&format=html) 来创建文件系统布局并执行 DVD 刻录过程。这意味着在刻录之前不需要创建数据的镜像。
