...
可以在不更改现有数据的情况下增加磁盘容量。这在虚拟机中很常见，当虚拟磁盘空间不足时，通常会扩大磁盘容量。有时，磁盘镜像被写入 U 盘，但未使用完整容量。在这里，我们将介绍如何调整或 *扩展* 磁盘内容，以充分利用增加的容量。
...
分区只能向连续的空闲空间扩展大小。在这里，磁盘上的最后一个分区是交换分区，但需要调整的是第二个分区。交换分区只包含临时数据，因此可以安全地卸载、删除，然后在调整第二个分区后重新创建第三个分区。
...
> **警告**
>
> 修改已挂载文件系统的分区表存在数据丢失的风险。最好在使用 Live CD-ROM 或 USB 设备运行的情况下，对未挂载的文件系统执行以下步骤。然而，在绝对必要的情况下，可以在禁用 GEOM 安全功能后对已挂载的文件系统进行调整：
>
> ```sh
