...
除了物理磁盘，FreeBSD 还支持创建和使用内存盘。内存盘的一个可能用途是访问 ISO 文件系统的内容，而无需先将其刻录到 CD 或 DVD，然后再挂载 CD/DVD 介质。
...
FreeBSD 还支持内存盘，其使用的存储可以从硬盘或内存区域分配。第一种方法通常称为基于文件的文件系统，第二种方法称为基于内存的文件系统。两种类型都可以使用 `mdconfig` 创建。

要创建一个新的基于内存的文件系统，请指定 `swap` 类型和要创建的内存盘大小。然后，格式化内存盘并像平常一样挂载。此示例创建了一个大小为 5MB 的内存盘，单元编号为 `1`。该内存盘随后被格式化为 UFS 文件系统，然后挂载：
...
要创建一个新的基于文件的内存盘，首先从磁盘中分配一个区域。此示例创建了一个名为 `newimage` 的空白 5MB 文件：
...
使用 `mdconfig` 创建基于文件或内存的文件系统需要几个命令。FreeBSD 还配有 `mdmfs`，它可以自动配置内存盘，格式化为 UFS 文件系统，并挂载它。例如，在使用 `dd` 创建 *newimage* 后，下面这个命令相当于运行上面显示的 `bsdlabel`、`newfs` 和 `mount` 命令：

```sh
# mdmfs -F newimage -s 5m md0 /mnt
```

如果要使用 `mdmfs` 创建一个新的基于内存的内存盘，请使用这个命令：
