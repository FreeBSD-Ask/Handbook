...
高可用性是关键业务应用的主要要求之一，而高可用性存储是此类环境中的关键组成部分。在 FreeBSD 中，**高可用存储**（HAST）框架允许通过 TCP/IP 网络将相同数据透明地存储在几台物理分离的机器上。HAST 可以理解为基于网络的 RAID1（镜像），类似于 GNU/Linux® 平台上的 DRBD® 存储系统。结合 FreeBSD 的其他高可用性功能，如 CARP，HAST 使得构建一个能够抵御硬件故障的高可用存储集群成为可能。
