...
* 将整个系统的档案备份到永久的离线存储设备。这可以防止上述所有问题，但恢复过程较慢且不便，尤其对于非特权用户来说。
...
通常，使用多种备份技术的组合。例如，可以创建一个计划，自动进行每周的完整系统备份，并将备份离线存储，此外，使用每小时的 ZFS 快照来补充此备份。另外，在进行文件编辑或删除之前，也可以手动备份单个目录或文件。

本节说明了可以用来在 FreeBSD 系统上创建和管理备份的一些工具。
...
> **技巧**
>
> 使用 [Z 文件系统（ZFS）](../di-23-zhang-z-wen-jian-xi-tong-zfs/23.1.-shi-shi-mo-shi-zfs-yu-zhong-bu-tong.md)的系统可以使用 [zfs(8)](https://man.freebsd.org/cgi/man.cgi?query=zfs&sektion=8&format=html) 来创建快照，并使用 [发送和接收](../di-23-zhang-z-wen-jian-xi-tong-zfs/23.4.-zfs-guan-li.md)功能向远程系统发送快照或从远程系统接收快照。
...
FreeBSD Ports 提供了许多第三方工具，可以用来调度备份的创建、简化磁带备份，并使备份变得更加方便和便捷。许多这些应用程序基于客户端/服务器架构，可以用来自动化单一系统或网络中所有计算机的备份。
...
将这些打印副本和安装盘的副本存储在一个安全的位置。如果需要进行紧急恢复，请引导进入安装盘并选择 `Live CD` 以访问恢复 shell。此恢复模式可用于查看系统的当前状态，如果需要，可以重新格式化磁盘并从备份中恢复数据。

接下来，测试恢复 shell 和备份。记录下操作流程，并将这些笔记与安装盘、打印副本和备份一起存储。这些笔记可以在进行紧急恢复时避免因压力过大而不小心销毁备份。
