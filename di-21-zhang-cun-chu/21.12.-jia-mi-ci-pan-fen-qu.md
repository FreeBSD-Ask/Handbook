...
[gbde(4)](https://man.freebsd.org/cgi/man.cgi?query=gbde&sektion=4&format=html) 工具的目标是为攻击者提供极大的挑战，防止其访问 *冷存储* 设备的内容。但是，如果计算机在存储设备已附加时被入侵，并且攻击者拥有有效的密码短语，它将不会保护存储设备的内容。因此，提供物理安全性是非常重要的，同时也需要保护加密机制所使用的密码短语。
...
1. **添加新硬盘**  
   按照 [添加磁盘](./20.2.-tian-jia-ci-pan.md) 中的说明将新硬盘安装到系统中。对于本示例，已添加新硬盘分区 **/dev/ad4s1c**，**/dev/ad0s1\*** 代表现有的标准 FreeBSD 分区。
...
另一种可用的加密 GEOM 类是 `geli`。该控制工具提供了一些功能，并使用不同的加密方案进行工作。它提供以下特性：
...
* 允许加密根分区。在系统引导时，将要求输入用于访问加密根分区的密码。
* 允许使用两个独立的密钥。
...
以下示例说明了如何生成一个密钥文件，该文件将作为加密提供者的主密钥的一部分，挂载在 **/private** 下。该密钥文件将提供一些随机数据，用于加密主密钥。主密钥也将由密码保护。提供者的扇区大小将设置为 4kB。该示例说明了如何附加到 `geli` 提供者，如何在其上创建文件系统、挂载、操作并最终如何卸载它。
...
这将配置 **/dev/da2** 作为具有主密钥 **/root/da2.key** 的 `geli` 提供者。在系统关闭之前，系统将自动将提供者从内核中分离。在引导过程中，脚本将提示输入密码，然后附加提供者。引导过程中的其他内核消息可能会在密码提示之前或之后显示。如果引导过程似乎停滞，请仔细查看其他消息中的密码提示。输入正确的密码后，提供者将被附加。然后，文件系统将挂载，通常通过 **/etc/fstab** 中的条目进行挂载。有关如何配置文件系统以在引导时挂载的说明，请参考 [“挂载和卸载文件系统”](../di-3-zhang-freebsd-ji-chu/3.7.-wen-jian-xi-tong-de-gua-zai-yu-xie-zai.md)。
