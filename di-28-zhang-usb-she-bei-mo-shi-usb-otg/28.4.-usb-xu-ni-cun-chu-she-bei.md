...
与串口和网络功能不同，模板不应在 **/boot/loader.conf** 中设置为 0 或 10，因为在设置模板之前，LUN 必须先设置。**cfumass** 启动脚本会在引导时自动设置正确的模板编号。

## 28.4.2. 使用其他方式配置 USB 存储

本章其余部分详细说明了如何在不使用 cfumass rc 文件的情况下设置目标。例如，如果想提供可写的 LUN，就必须使用这种方式。
