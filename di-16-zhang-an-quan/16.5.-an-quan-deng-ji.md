...
* **1** 安全模式 - 系统不可变和系统仅追加标志不能关闭；不能为挂载的文件系统、**/dev/mem** 和 **/dev/kmem** 打开写入；**/dev/io**（如果平台支持）不能被打开；不能加载或卸载内核模块（见 [kld(4)](https://man.freebsd.org/cgi/man.cgi?query=kld&sektion=4&format=html)）。不能使用 `debug.kdb.enter` sysctl 进入内核调试器。无法通过 `debug.kdb.panic`、`debug.kdb.panic_str` 等 sysctl 强制触发 panic 或陷阱。
