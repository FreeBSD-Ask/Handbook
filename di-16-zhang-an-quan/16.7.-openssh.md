...
当数据以未加密的形式通过网络发送时，任何位于客户端和服务器之间的网络嗅探器都可能窃取用户/口令信息或会话中传输的数据。OpenSSH 提供多种身份验证和加密方法，以防止这种情况发生。

关于 OpenSSH 的更多信息，请访问 [官方网站](https://www.openssh.com/)。

本节概述了内置的客户端工具，用于安全访问其他系统并安全传输文件。然后说明了如何在 FreeBSD 系统上配置 SSH 服务器。
...
由于之前已验证该主机的指纹，因此在提示输入用户口令之前，服务器的密钥会自动进行检查。
...
与使用口令不同，客户端可以配置为使用密钥连接到远程机器。出于安全原因，这是首选方法。
...
此时，所有具有用户名和口令的用户应该能够访问 sshd。

## 16.7.5. 配置公钥认证方法

配置 OpenSSH 使用公钥认证增强了安全性，利用非对称加密进行身份验证。此方法消除了与口令相关的风险，例如弱口令或传输过程中被拦截的风险，同时抵御了各种基于口令的攻击。然而，确保私钥的安全非常重要，以防止未经授权的访问。
