...
虽然此时不会有 Kerberos 守护进程在运行，但可以通过获取刚刚创建的主体的凭据来确认 KDC 是否正常工作：
...
* 使用 Heimdal 或 MIT Kerberos 的 Port 时，确保 `PATH` 列表中的客户端应用程序版本优先于系统版本。
