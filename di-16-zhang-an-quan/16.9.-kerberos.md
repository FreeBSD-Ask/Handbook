...
* 使用 MIT Kerberos 时，要允许主体的票证生命周期超过默认的十小时生命周期，请在 [kadmin(8)](https://man.freebsd.org/cgi/man.cgi?query=kadmin&sektion=8&format=html) 提示符下使用 `modify_principal` 修改相关主体和 `krbtgt` 主体的 `maxlife`。然后，主体可以使用 `kinit -l` 请求具有更长生命周期的票证。
