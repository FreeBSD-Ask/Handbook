...
密钥分发中心（KDC）是 Kerberos 提供的集中式身份验证服务，是系统中的“可信第三方”。它是发放 Kerberos 凭据的计算机，客户端用它来进行服务器身份验证。由于 KDC 被所有其他计算机视为可信，因此对它的安全要求更高。应限制对 KDC 的直接访问。
...
虽然此时不会有支持 Kerberos 的守护进程在运行，但可以通过获取刚刚创建的主体的凭据来确认 KDC 是否正常工作：
...
通过使用 `kinit`、`klist` 和 `kdestroy` 在客户端进行测试，以获取、显示并删除一个现有主体的凭据。Kerberos 应用程序也应能够连接到启用 Kerberos 的服务器。如果这无法正常工作，但获取凭据成功，则问题可能出在服务器，而非客户端或 KDC。对于 Kerberized 的 [ssh(1)](https://man.freebsd.org/cgi/man.cgi?query=ssh&sektion=1&format=html)，默认情况下 GSS-API 是禁用的，因此可以使用 `ssh -o GSSAPIAuthentication=yes <hostname>` 来进行测试。
...
* 使用 MIT Kerberos 时，要允许主体的凭据生命周期超过默认的十小时生命周期，请在 [kadmin(8)](https://man.freebsd.org/cgi/man.cgi?query=kadmin&sektion=8&format=html) 提示符下使用 `modify_principal` 修改相关主体和 `krbtgt` 主体的 `maxlife`。然后，主体可以使用 `kinit -l` 请求具有更长生命周期的凭据。
...
* 主机主体可以有更长的凭据生命周期。如果用户主体的生命周期为一周，但连接的主机的生命周期为九小时，则用户缓存将包含过期的主机主体，凭据缓存将无法按预期工作。
...
由于 Kerberos 是一种“全有或全无”的方法，网络上的每个启用的服务必须要么修改以与 Kerberos 一起工作，要么采取其他措施来防止网络攻击。这是为了防止用户凭据被窃取并被重新使用。一个例子是，当所有远程 shell 启用了 Kerberos，但非 Kerberized 的 POP3 邮件服务器仍以明文发送密码。
