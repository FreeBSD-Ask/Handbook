...
密钥分发中心（KDC）是 Kerberos 提供的集中式身份验证服务，是系统中的“可信第三方”。它是发放 Kerberos 票据的计算机，客户端用它来进行服务器身份验证。由于 KDC 被所有其他计算机视为可信，因此对它的安全要求更高。应限制对 KDC 的直接访问。
...
创建主密钥后，应初始化数据库。Kerberos 管理工具 [kadmin(8)](https://man.freebsd.org/cgi/man.cgi?query=kadmin&sektion=8&format=html) 可以在 KDC 上直接对数据库操作，而无需使用 [kadmind(8)](https://man.freebsd.org/cgi/man.cgi?query=kadmind&sektion=8&format=html) 网络服务，使用 `kadmin -l` 运行。这解决了在数据库创建之前尝试连接到数据库的先有鸡还是先有蛋的问题。在 `kadmin` 提示符下，使用 `init` 创建领域的初始数据库：
...
虽然此时不会有支持 Kerberos 的守护进程在运行，但可以通过获取刚刚创建的主体的票据来确认 KDC 是否正常工作：
...
通过使用 `kinit`、`klist` 和 `kdestroy` 在客户端进行测试，以获取、显示并删除一个现有主体的票据。Kerberos 应用程序也应能够连接到启用 Kerberos 的服务器。如果这无法正常工作，但获取票据成功，则问题可能出在服务器，而非客户端或 KDC。对于 Kerberized 的 [ssh(1)](https://man.freebsd.org/cgi/man.cgi?query=ssh&sektion=1&format=html)，默认情况下 GSS-API 是禁用的，因此可以使用 `ssh -o GSSAPIAuthentication=yes <hostname>` 来进行测试。
...
在一个领域内，用户的 Kerberos 主体通常会映射到本地用户账户。有时，需要为没有匹配 Kerberos 主体的用户授予访问本地用户账户的权限。例如，`tillman@EXAMPLE.ORG` 可能需要访问本地用户账户 `webdevelopers`。其他主体也可能需要访问该本地账户。
...
* 领域中的所有主机必须能够在 DNS 中进行正向和反向解析，或者至少存在于 **/etc/hosts** 中。CNAME 记录可以使用，但 A 和 PTR 记录必须正确并已设置。无法解析主机时，错误信息并不直观：`Kerberos5 refuses authentication because Read req failed: Key table entry not found`。
* 一些作为客户端连接到 KDC 的操作系统未将 `ksu` 的权限设置为 setuid `root`。这意味着 `ksu` 无法正常工作。这是权限问题，而不是 KDC 错误。
* 使用 MIT Kerberos 时，要让主体的票据生命周期超过默认的十小时生命周期，请在 [kadmin(8)](https://man.freebsd.org/cgi/man.cgi?query=kadmin&sektion=8&format=html) 提示符下使用 `modify_principal` 修改相关主体和 `krbtgt` 主体的 `maxlife`。然后，主体可以使用 `kinit -l` 请求具有更长生命周期的票据。
...
* 主机主体可以有更长的票据生命周期。如果用户主体的生命周期为一周，但连接的主机的生命周期为九小时，则用户缓存将包含过期的主机主体，票据缓存将无法按预期工作。
...
由于 Kerberos 是一种“全有或全无”的方法，网络上的每个启用的服务必须要么修改以与 Kerberos 一起工作，要么采取其他措施来防御网络攻击。这是为了防止用户凭据被窃取并被重新使用。一个例子是，当所有远程 shell 启用了 Kerberos，但非 Kerberized 的 POP3 邮件服务器仍以明文发送密码。
...
Kerberos 能让用户、主机和服务彼此之间进行身份验证，但没有机制可以验证 KDC 对用户、主机或服务的身份。这意味着一个被篡改的 `kinit` 可能记录所有用户名和密码。像 [security/tripwire](https://cgit.freebsd.org/ports/tree/security/tripwire/) 这样的文件系统完整性检查工具可以缓解这个问题。
