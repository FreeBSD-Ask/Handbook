...
该协议的当前版本是版本 5，描述在 RFC 4120 中。可用的该协议的几个免费实现覆盖了各种操作系统。MIT 一直在开发他们的 Kerberos 软件包。它在美国作为一种密码学产品通常被使用，并且在历史上受到美国出口管制的限制。在 FreeBSD 中，MITKerberos 可作为安全/krb5 包或port 使用。Heimdal Kerberos 实现是明确在美国以外开发的，以避免出口管制。Heimdal Kerberos 发行版包含在基本的 FreeBSD 安装中，另一个具有更多可配置选项的发行版可在Ports 集合中的安全/heimdal 中找到。
...
领域内的用户通常将其 Kerberos 主体映射到本地用户帐户。偶尔，需要将对本地用户帐户的访问权限授予没有匹配的 Kerberos 主体的人。例如， tillman@EXAMPLE.ORG 可能需要访问本地用户帐户 webdevelopers 。其他主体也可能需要访问该本地帐户。

放置在用户的主目录中的.k5login 和.k5users 文件可用于解决此问题。例如，如果将以下.k5login 放置在 webdevelopers 的主目录中，则列出的两个主体将能够访问该帐户，而无需共享密码：
