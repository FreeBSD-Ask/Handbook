...
密码是技术领域中一种不可避免的弊端。当必须使用密码时，应确保其复杂性，并使用强大的哈希机制对存储在密码数据库中的版本进行加密。FreeBSD 支持多种算法，包括 SHA256、SHA512 和 Blowfish 哈希算法，详细信息请参见 [crypt(3)](https://man.freebsd.org/cgi/man.cgi?query=crypt&sektion=3&format=html)。
