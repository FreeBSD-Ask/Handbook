...
到目前为止，安全章节已经涉及允许授权用户访问和试图防止未经授权访问。一旦授权用户可以访问系统资源，另一个问题就出现了。在许多情况下，一些用户可能需要访问应用程序启动脚本，或者一组管理员需要维护系统。传统上，标准用户和组、文件权限，甚至 su(1)命令都会管理这种访问。随着应用程序需要更多的访问权限，越来越多的用户需要使用系统资源，需要更好的解决方案。目前最常用的应用程序是 Sudo。
