...
Sudo 允许管理员配置更严格的访问控制系统命令，并提供一些高级日志记录功能。作为一个工具，它可以从 Ports Collection 安装，路径为 [security/sudo](https://cgit.freebsd.org/ports/tree/security/sudo/)，或使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 工具。
