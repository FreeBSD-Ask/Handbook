...
密码是技术领域中一种不可避免的弊端。当必须使用密码时，应确保其复杂性，并使用强大的哈希机制对存储在密码数据库中的版本加密。FreeBSD 支持多种算法，包括其 `crypt()` 库中的 SHA256、SHA512 和 Blowfish 哈希算法，详细信息请参见 [crypt(3)](https://man.freebsd.org/cgi/man.cgi?query=crypt&sektion=3&format=html)。
...
Sudo 能让管理员对系统命令配置更严格的访问控制，并提供一些高级日志记录功能。作为工具，它可以从 Ports Collection 获取，路径为 [security/sudo](https://cgit.freebsd.org/ports/tree/security/sudo/)，或使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 工具。
