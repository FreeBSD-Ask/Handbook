...
* 安全公告的名称始终以 `FreeBSD-SA-`（FreeBSD 安全公告）开头，接着是两位数的年份格式（`23:`），然后是该年度的公告编号（`07.`），最后是受影响的应用程序或子系统的名称（例如 `bhyve`）。
* `Topic` 字段简要说明了漏洞。
...
* `Solution` 字段提供了修补受影响系统的步骤。这是一个经过测试和验证的分步方法，帮助用户修补系统并确保其安全。
