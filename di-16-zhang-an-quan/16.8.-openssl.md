...
重要的是，[fips_module(7)](https://man.freebsd.org/cgi/man.cgi?query=fips_module&sektion=7&format=html) 受到额外的安全措施保护，禁止在未通过完整性检查的情况下使用该模块。此检查可以由本地系统管理员进行设置，让每个 OpenSSL 3 的用户能加载该模块。如果配置不正确，FIPS 模块将按以下方式失败：
...
可以通过创建 **/etc/ssl/fipsmodule.cnf** 文件来配置检查，然后在 OpenSSL 的主配置文件 **/etc/ssl/openssl.cnf** 中引用该文件。OpenSSL 提供了 [openssl-fipsinstall(1)](https://man.freebsd.org/cgi/man.cgi?query=openssl-fipsinstall&sektion=1&format=html) 工具来帮助完成此过程，可以按如下方式使用：
...
每当修改 FIPS 模块时，例如在进行系统更新或应用影响基本系统中 OpenSSL 的安全修复后，都必须重复此过程。
