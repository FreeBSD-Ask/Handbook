...
传统方法通过编辑 **/etc/login.conf** 来定义登录分级。虽然这种方法仍然受支持，但任何更改都需要多步过程：编辑此文件、重建资源数据库、对 **/etc/master.passwd** 进行必要更改，并重建密码数据库。根据需要配置的用户数量，这可能会变得耗时。
...
在传统方法中，登录分级及应用于登录分级的资源限制定义在 **/etc/login.conf** 中。每个用户账户可以分配给一个登录分级，`default` 是默认的登录分级。每个登录分级都有一组关联的登录能力。登录能力是一个 `name=value` 的对，其中 *name* 是一个已知的标识符，*value* 是一个任意字符串，根据 *name* 的不同，*value* 会被相应地处理。
...
然后找到要修改的登录分级部分。在这个示例中，假设登录分级名为 `limited`，如果它不存在，可以创建它：
...
可以使用 [chpass(1)](https://man.freebsd.org/cgi/man.cgi?query=chpass&sektion=1&format=html) 命令更改用户的分级，执行以下命令：
...
[rctl(8)](https://man.freebsd.org/cgi/man.cgi?query=rctl&sektion=8&format=html) 系统提供了一种更精细化的方式来为单个进程和用户设置和管理资源限制。它能让你动态地为特定进程或用户分配资源限制，无论其属于哪个登录分级。
