...
内置的 [mtree(8)](https://man.freebsd.org/cgi/man.cgi?query=mtree&sektion=8&format=html) 工具可用于生成目录内容的规格。种子值或数字常量用于生成规格，并且需要用来检查规格是否发生了变化。这样可以判断文件或二进制文件是否已修改。由于攻击者无法知道种子值，因此伪造或检查文件的校验和值将非常困难甚至不可能。
...
一个 mtree 文件由一系列行组成，每行提供单一文件系统对象的信息。系统总是忽略行首的空白字符。
