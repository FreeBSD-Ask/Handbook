...
[软件包](https://docs.freebsd.org/en/articles/linux-users/#software) 中启用了大量的默认选项。通过运行 `pkg info openldap-server` 来查看它们。如果这些选项不够（例如需要 SQL 支持），请考虑使用适当的 [框架](https://docs.freebsd.org/en/books/handbook/ports/#ports-using) 重新编译 Port。
