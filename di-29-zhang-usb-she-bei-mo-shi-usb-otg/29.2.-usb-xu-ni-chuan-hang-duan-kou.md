...
## 29.2.1. 配置 USB 设备模式串行端口

虚拟串行端口支持由模板 3、8 和 10 提供。注意，模板 3 可以在不需要特殊驱动程序和 INF 文件的情况下与 Microsoft Windows 10 配合使用。其他主机操作系统与所有三个模板兼容。必须加载 [usb_template(4)](https://man.freebsd.org/cgi/man.cgi?query=usb_template&sektion=4&format=html) 和 [umodem(4)](https://man.freebsd.org/cgi/man.cgi?query=umodem&sektion=4&format=html) 内核模块。
...
## 29.2.2. 从 FreeBSD 连接到 USB 设备模式串行端口

要连接到配置为提供 USB 设备模式串行端口的板子，请将 USB 主机（如笔记本电脑）连接到板子的 USB OTG 或 USB 客户端端口。使用 `pstat -t` 命令在主机上列出终端线路。在列表接近末尾的位置，你应该会看到一个 USB 串行端口，例如“ttyU0”。要打开连接，请使用：
...
## 29.2.3. 从 macOS 连接到 USB 设备模式串行端口
...
## 29.2.4. 从 Linux 连接到 USB 设备模式串行端口
...
## 29.2.5. 从 Microsoft Windows 10 连接到 USB 设备模式串行端口
