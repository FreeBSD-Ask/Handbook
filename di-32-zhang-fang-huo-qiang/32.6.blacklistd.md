...
FreeBSD 中有一些软件包可以利用 Blacklistd 的功能。两个最突出的是 [ftpd(8)](https://www.freebsd.org/cgi/man.cgi?query=ftpd\&sektion=8\&format=html) 和 [sshd(8)](https://www.freebsd.org/cgi/man.cgi?query=sshd\&sektion=8\&format=html)，用于阻止过多的连接尝试。要激活 SSH 守护程序中的 Blacklistd，请将以下行添加到 **/etc/ssh/sshd\_config**：
