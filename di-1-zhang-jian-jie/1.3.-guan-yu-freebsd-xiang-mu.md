...
几乎每一个 port 都以预编译的“包”形式提供，那些不希望从源代码编译自己的 port 的人可以通过简单的命令（pkg install）来安装。关于包和 port 的更多信息可以在安装应用程序中找到：包和 port。
