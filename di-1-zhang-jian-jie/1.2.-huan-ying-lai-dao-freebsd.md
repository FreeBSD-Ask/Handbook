...
* _简单而一致的资源库结构和编译系统_ - FreeBSD 的所有组件，包括内核和用户空间，都使用单一的资源库。这一点，加上一个统一的、易于定制的编译系统，以及经过深思熟虑的开发过程，使得将 FreeBSD 与您自己的产品的编译基础结构整合起来变得很容易。
...
* _桌面：_ FreeBSD 是低成本桌面解决方案中一种不错的选择，您可以选择使用免费的 X11 服务器。FreeBSD 提供了许多开源的桌面环境，包括标准的 GNOME 和 KDE 图形用户界面。FreeBSD 甚至可以从中央服务器上“无盘”启动，使单个工作站更便宜，更容易管理。
...
* [Sophos](http://www.sophos.com/)——Sophos Email Appliance 产品基于经过强化的 FreeBSD，扫描入站邮件中的垃圾邮件和病毒，同时也监控出站邮件中的恶意软件以及敏感信息的意外丢失。
* [Spectra Logic](http://www.spectralogic.com/)——nTier 系列档案级存储设备运行于 FreeBSD 和 OpenZFS。
* [Stormshield](http://www.spectralogic.com/)——Stormshield 网络安全设备是一个基于强化的 FreeBSD 版本。BSD 许可证允许他们将自己的知识产权与系统集成，同时将大量有趣的开发成果返还给社区。
