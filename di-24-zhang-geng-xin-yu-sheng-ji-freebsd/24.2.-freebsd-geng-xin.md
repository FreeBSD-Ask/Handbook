...
这份输出是删节缩短后的，其实是有更多的文件。其中有些文件并非人为修改，比如 **/etc/passwd** 被修改是因为添加了用户进系统。在某些情况下，还有另外的一些文件，诸如内核模块与 `freebsd-update` 的不同是因为它们被更新过了。为了把指定的文件或目录排除在外，把它们加到 **/etc/freebsd-update.conf** 的 `IDSIgnorePaths` 选项中。
