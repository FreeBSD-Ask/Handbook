...
对多个 jail 的管理可能会成为问题，因为每个 jail 在升级时都必须从头开始重新编译。如果要创建并手动更新许多 jail，这可能会非常耗时且无聊。

本节演示了一种解决此问题的方法，方法是使用 [mount_nullfs(8)](https://www.freebsd.org/cgi/man.cgi?query=mount_nullfs&sektion=8&format=html) 只读挂载在 jail 之间安全地共享尽可能多的内容，以便让更新更简单。这使得将单个服务（如 HTTP，DNS 和 SMTP）放入单独的 jail 更具吸引力。此外，它还提供了一种增加、删除和升级 jail 的简单方法。
...
- 轻松增加新的 jail 或删除已有的 jail。
- 轻松更新或升级已有 jail。
- 使运行定制的 FreeBSD 分支成为可能。
- 对安全问题持偏执态度，尽可能地减少妥协的可能性。
- 尽可能节省空间和节点。

这种设计依赖于一个单一的、只读的主模板，它被安装到每个 jail 中，每个 jail 有一个读写设备。设备可以是一个单独的物理磁盘，一个分区，或者一个虚拟节点支持的内存设备。这个例子使用了可读写的 nullfs 挂载。

文件系统布局如下：

- jail 位于 **/home** 分区下。
- 每个 jail 都将挂载在 **/home/j** 目录下。
- **/home/j/mroot** 是每个 jail 共用的模板，对所有 jail 来说都是只读分区。
- 每个 jail 都有一个对应的空目录在 **/home/j** 中。
- 每个 jail 都有一个 **/s** 目录，该目录将链接到系统的可读写部分。
- 每个 jail 都有自己的可读写系统，该系统基于 **/home/j/skel**。
- 每个 jail 的可读写部分创建在 **/home/js** 中。
...
>1. 首先，为只读文件系统创建一个目录结构，它将包含 jail 的 FreeBSD 二进制文件。然后，将切换到 FreeBSD 源代码所在目录，并将只读文件系统安装到 jail 模板中：
>
>```
...
>```
>
>这使得在每个 jail 中编译 FreeBSD ports 成为可能。请记住，ports 目录是只读系统的一部分。`WRKDIRPREFIX` 的自定义路径允许在每个 jail 的可读写部分进行编译。
...
>```
>
>为了防止 fsck 在引导期间检查 nullfs 挂载，并防止转储备份 jail 的只读 nullfs 挂载，将最后两列都设置为 `0`。
>
...
>```
>
>*jailnamerootdir* 变量被设置为 **/usr/home** 而不是 **/home**，因为在默认的 FreeBSD 安装中，**/home** 的物理路径是 **/usr/home**。*jailnamerootdir* 变量不能被设置为包含符号链接的路径，否则 jail 将拒绝启动。
>
...
现在 jail 应该已经运行了。要检查它们是否已正确启动，请使用 `jls`。其输出应类似于以下内容：
...
在这时，应该可以登录到每一个 jail，添加新的用户，或者配置守护程序。`JID` 列表示每个运行中的 jail 的标识号。使用下面的命令来执行 JID 为 `3` 的 jail 的管理任务：
...
>```
>
>5. 移动旧的只读文件系统，并将其替换为新的文件系统。如果出现问题，这将用作旧的只读文件系统的备份和存档。此处使用的命名约定对应于创建新的只读文件系统时。将原来的 FreeBSD ports 移到新的文件系统上 以节省一些空间和节点：
>
>```
># cd /home/j
