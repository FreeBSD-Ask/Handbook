...
这个例子在不安全类的用户上增加 `top` 的标签设置。所有由不安全类中的用户产生的进程将停留在分区/13标签中。
...
- 固定规则是 `no write up, no read down` 与 MLS 相反。主体可以对其自身级别或更低级别的对象具有写入权限，但不能具有更高级别的对象。同样，主体可以对其自身级别或更高级别（但不能低于）的对象具有读取访问权限。
- 通过防止对数据进行不当修改来保持完整性。
- 完整性级别，而不是 MLS 敏感度级别。
