...
FreeBSD 包含一组策略，这些策略可满足大多数安全要求。每项政策总结如下。最后三个策略支持整数设置来代替三个默认标签。
...
- `security.mac.portacl.rules` 以` rule[,rule,…]`形式的文本字符串来指定策略，规则数量不限，每个规则的形式为`idtype:id:protocol:port`。id 类型是`uid`或`gid`。协议参数可以是`tcp`或`udp`。端口参数是允许指定的用户或组绑定的端口号。用户ID、组ID和端口参数只能使用数字值。

默认情况下，小于 1024 的端口只能由以 运行方式的特权进程使用。对于[mac_portacl（4）](https://www.freebsd.org/cgi/man.cgi?query=mac_portacl&sektion=4&format=html) 以允许非`root`特权进程绑定到小于 1024 的端口，请按如下方式设置以下可调参数：
...
在 MLS 环境中，每个主体或物体的标签中都设置了一个“清除”级别，同时还有隔间。由于这些清除级别可以达到超过几千的数字，彻底配置每个主题或对象将是一项艰巨的任务。为了减轻这种管理上的开销，本政策中包括三个标签：`mls/low`、`mls/equal`、`mls/high`，其中。
