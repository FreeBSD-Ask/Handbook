...
[mac_partition（4）](https://www.freebsd.org/cgi/man.cgi?query=mac_partition&sektion=4&format=html) 策略根据进程的 MAC 标签将其放入特定的“分区”。这个策略的大部分配置是通过 [setpmac（8）](https://www.freebsd.org/cgi/man.cgi?query=setpmac&sektion=8&format=html) 完成的。这个策略有一个 sysctl 可调控项。
