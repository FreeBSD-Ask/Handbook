...
默认的 FreeBSD 内核包括选项 MAC。这意味着每一个包含MAC框架的模块都可以用`kldload`作为运行时内核模块加载。在测试完模块后，将模块名称添加到 **/boot/loader.conf**中，这样它就会在启动时加载。每个模块还为那些选择编译自己的自定义内核的管理员提供一个内核选项。

FreeBSD 包含一组策略， 这些策略将满足大多数安全要求。每项政策总结如下。最后三个策略支持整数设置来代替三个默认标签。
...
[mac_seeotheruids（4）](https://www.freebsd.org/cgi/man.cgi?query=mac_seeotheruids&sektion=4&format=html) 模块扩展了 `security.bsd.see_other_uids` 和 `security.bsd.see_other_gids sysctl `可调控项。这个选项不需要在配置前设置任何标签，可以与其他模块透明地运行。

加载模块后，可以使用以下`sysctl`可调参数来控制其功能：

- `security.mac.seeotheruids.enabled`启用该模块并实现默认设置，这些设置拒绝用户查看其他用户拥有的进程和套接字。
- `security.mac.seeotheruids.specificgid_enabled`允许指定的组豁免于此策略。要豁免特定的组，使用 `security.mac.seeotheruids.specificgid=XXX sysctl tunable`，用要豁免的数字组ID代替XXX。
...
[mac_bsdextended（4）](https://www.freebsd.org/cgi/man.cgi?query=mac_bsdextended&sektion=4&format=html)模块执行一个文件系统防火墙。它提供了对标准文件系统权限模型的扩展，允许管理员创建一个类似防火墙的规则集来保护文件系统层次结构中的文件、工具和目录。当试图访问一个文件系统对象时，规则列表被迭代，直到找到一个匹配的规则或到达终点。这种行为可以通过 `security.mac.bsdextended.firstmatch_enabled` 来改变。与 FreeBSD 中的其他防火墙模块类似， 一个包含访问控制规则的文件可以在启动时由系统使用  [rc.conf（5）](https://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&format=html) 变量创建和读取包含访问控制规则的文件。`security.mac.bsdextended.firstmatch_enabled`
...
默认情况下，不定义任何规则，所有内容都完全可访问。要创建阻止用户进行所有访问但不受影响的规则，请以`root`账户执行以下操作：

```
# ugidfw add subject not uid root new object not uid root mode n
```

虽然这个规则实现起来很简单，但却是一个非常糟糕的主意，因为它阻止了所有用户发布任何命令。一个更现实的例子是阻止`user1`对`user2`的主目录的所有访问，包括目录列表。
...
为了对所有用户执行相同的访问限制，可以不使用`uid user2`来代替`user1`。然而，root用户不受这些规则的影响。
...
[mac_ifoff（4）](https://www.freebsd.org/cgi/man.cgi?query=mac_ifoff&sektion=4&format=html) 最常见的用途之一是在引导过程中不应允许网络流量的环境中进行网络监控。另一个用途是编写一个脚本，该脚本使用[诸如security/aide](https://cgit.freebsd.org/ports/tree/security/aide/pkg-descr)之类的应用程序，以便在受保护的目录中找到新的或更改的文件时自动阻止网络流量。
...
[mac_portacl（4）](https://www.freebsd.org/cgi/man.cgi?query=mac_portacl&sektion=4&format=html) 模块 用于限制绑定到本地 TCP 和 UDP 端口， 从而允许非`root`用户绑定到低于 1024 的指定特权端口。
...
- `security.mac.portacl.rules` 以` rule[,rule,…]`形式的文本字符串来指定策略，规则数量不限，每个规则的形式为  `idtype:id:protocol:port`。id类型是`uid`或`gid`。协议参数可以是`tcp`或`udp`。端口参数是允许指定的用户或组绑定的端口号。用户ID、组ID和端口参数只能使用数字值。

默认情况下，低于 1024 的端口只能由以 运行方式的特权进程使用。对于[mac_portacl（4）](https://www.freebsd.org/cgi/man.cgi?query=mac_portacl&sektion=4&format=html) 以允许非`root`特权进程绑定到低于 1024 的端口，请按如下方式设置以下可调参数：
...
允许UID为80的`www`用户绑定到80端口，而不需要`root`权限。
...
当这个策略被启用时，用户将只被允许看到他们的进程，以及他们分区内的任何其他进程，但将不被允许使用这个分区范围以外的实用程序。例如，一个不安全等级的用户将不被允许访问`top`以及许多其他必须生成进程的命令。

这个例子在不安全类的用户上增加了`top`的标签设置。所有由不安全类中的用户产生的进程将停留在分区/13标签中。
...
| 注意                                                         |
| ------------------------------------------------------------ |
| 用户可以在`root` 的标签中看到进程，除非加载了 [mac_seeotheruids（4）](https://www.freebsd.org/cgi/man.cgi?query=mac_seeotheruids&sektion=4&format=html) 策略。 |
...
在MLS环境中，每个主体或物体的标签中都设置了一个 "清除 "级别，同时还有隔间。由于这些清除级别可以达到超过几千的数字，彻底配置每个主题或对象将是一项艰巨的任务。为了减轻这种管理上的开销，本政策中包括三个标签：`mls/low`、`mls/equal`、`mls/high`，其中。

- `mls/low`任何标有的都具有较低的许可级别，并且不允许访问更高级别的信息。此标签还可以防止间隙级别较高的对象写入或将信息传递到较低级别。
- `mls/equal`应放置在应从策略中免除的对象上。
- `mls/high`是可能的最高清关水平。分配此标签的对象将优先于系统中的所有其他对象;但是，它们不允许将信息泄露给较低级别的对象。
...
- `security.mac.mls.ptys_equal`将所有 [pty（4）](https://www.freebsd.org/cgi/man.cgi?query=pty&sektion=4&format=html) 设备标记为`mls/equal`创建过程中。
...
另一种方法是在 **/etc/** 中创建一个主策略文件，该文件指定 MLS 策略信息，并将该文件馈送到`setfmac` 。

使用 MLS 策略模块时，管理员计划控制敏感信息的流。`block read up block write down`默认值将所有内容设置为低状态。一切都可以访问，管理员慢慢地增强了信息的机密性。
...
在 Biba 环境中，每个主题或对象都设置了"完整性"标签。这些标签由分层等级和非分层组件组成。随着等级的上升，其完整性也会上升。

支持的标签是 `biba/low`、`biba/equal` 和 `biba/high`，其中：

- `biba/low`被视为对象或主体可能具有的最低完整性。在对象或主体上设置此设置会阻止它们对标记为 的对象或主体的写入访问，但不会阻止读取访问。`biba/high`
...
- 固定规则是 `no write up, no read down` 与MLS相反。主体可以对其自身级别或更低级别的对象具有写入权限，但不能具有更高级别的对象。同样，主体可以对其自身级别或更高级别（但不能低于）的对象具有读取访问权限。
- 通过防止对数据进行不当修改来保持完整性。
- 完整性级别，而不是 MLS 敏感度级别。
...
- `security.mac.biba.revocation_enabled`如果标签更改为主语，则强制吊销对对象的访问。
...
完整性不同于敏感度，用于保证信息不被不受信任的各方操纵。这包括在主体和对象之间传递的信息。它确保用户只能修改或访问他们已被授予显式访问权限的信息。[mac_biba（4）](https://www.freebsd.org/cgi/man.cgi?query=mac_biba&sektion=4&format=html) 安全策略 模块 允许 管理员 配置 用户 可以看到 和 调用 哪些 文件 和 程序， 同时 确保 系统 为 该 用户 信任 这些 程序 和 文件。
...
低水位线完整性策略的工作方式几乎与 Biba 相同，只是使用浮动标签来支持通过辅助等级隔间降级的受试者。此辅助隔间采用 `[auxgrade]` 的形式 。为策略分配辅助等级时，请使用语法 `lomac/10[2]`，其中是辅助等级`2`。
