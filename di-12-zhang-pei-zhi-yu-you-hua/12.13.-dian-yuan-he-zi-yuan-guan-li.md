...
调试输出在默认情况下是不启用的。要启用它，如果 ACPI 被编译到内核中，请在自定义的内核配置文件中添加 `options ACPI_DEBUG`。在 **/etc/make.conf** 中添加 `ACPI_DEBUG=1` 来全局启用它。如果使用模块而不是自定义内核，只需重新编译 **acpi.ko** 模块，如下所示：
...
* 对错误行为的描述，包括系统类型、型号以及导致错误出现的任何东西。如果是新发生的，尽可能准确地记下该 bug 开始出现的时间。
