...
> **注意**
>
> 扫描仪为即插即用设备，更换连接的 USB 端口后，`usbconfig list` 的输出内容也会改变。
...
若获得了 `idVendor` 和 `idProduct`，需要在 [SANE 支持的设备列表](http://www.sane-project.org/lists/sane-mfgs-cvs.html) 中检查扫描仪是否受支持，可通过 `idProduct` 筛选。

## 9.6.2. SANE 配置

SANE 通过后端提供对扫描仪的访问。为了能在 FreeBSD 上扫描，必须安装 Port [graphics/sane-backends](https://cgit.freebsd.org/ports/tree/graphics/sane-backends/)，可运行以下命令安装：
...
> **技巧**
>
> 某些 USB 扫描仪需要加载固件。例如，上面使用的 HP 扫描仪，需要安装 Port [print/hplip](https://cgit.freebsd.org/ports/tree/print/hplip/)。

安装完必要的包后，必须配置 [devd(8)](https://man.freebsd.org/cgi/man.cgi?query=devd&sektion=8&format=html)，让 FreeBSD 能访问扫描仪。
...
之后，必须运行以下命令重新启动 [devd(8)](https://man.freebsd.org/cgi/man.cgi?query=devd&sektion=8&format=html)：
...
[scanimage(1)](https://man.freebsd.org/cgi/man.cgi?query=scanimage&sektion=1&format=html) 识别扫描仪后，配置就完成了，扫描仪现在可以使用。
...
虽然可以通过命令行使用 [scanimage(1)](https://man.freebsd.org/cgi/man.cgi?query=scanimage&sektion=1&format=html) 执行图像采集，但通常更倾向于使用图形界面扫描图像。

**表 6. 图形扫描程序**

| 名称 | 许可证 | 包 |
| ---- | ------ | -- |
| skanlite | GPL 2.0 | [graphics/skanlite](https://cgit.freebsd.org/ports/tree/graphics/skanlite/) |
| GNOME Simple Scan | GPL 3.0 | [graphics/simple-scan](https://cgit.freebsd.org/ports/tree/graphics/simple-scan/) |
| XSANE | GPL 2.0 | [graphics/xsane](https://cgit.freebsd.org/ports/tree/graphics/xsane/) |
