...
如果不知道系统使用的是哪个声卡，或者使用哪个模块，可执行以下命令加载 `snd_driver` 元驱动：
...
显卡通常带有自己的集成声音设备，可能不清楚哪个设备作为默认设备。运行 `dmesg` 并查看 `pcm` 条目即可确认系统如何枚举输出。执行以下命令：
...
显卡（NVIDIA®）列在声卡（Realtek®）之前，声卡作为 `pcm4` 显示。可执行以下命令将系统配置为使用声卡作为默认设备：
...
如果可能，应在声音设备驱动程序之外调整，但对于无法做到的情况（或硬件重采样能力较差），你可以控制用于重采样的质量。
