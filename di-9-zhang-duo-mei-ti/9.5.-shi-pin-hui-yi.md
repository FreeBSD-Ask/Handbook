...
为了让 FreeBSD 访问摄像头并完成配置，需要安装一些实用工具：
...
> **注意**
>
> 如果这是即插即用的 USB 摄像头，更换其连接的 USB 端口后，`webcamd -l` 的输出（尤其是设备标识符）会发生变化，可能需要同步更新 rc.conf。为避免此问题，可使用设备名称（`-N` 选项）和/或匹配 `webcamd -l` 输出中显示的设备序列号——如果已知的话（`-S` 选项）。对于笔记本内置等固定摄像头，可使用设备标识符（`-d` 选项）。

必须执行以下命令启动 [webcamd(8)](https://man.freebsd.org/cgi/man.cgi?query=webcamd&sektion=8&format=html) 服务：
...
可以使用 [multimedia/pwcview](https://cgit.freebsd.org/ports/tree/multimedia/pwcview/) 检查摄像头是否正常工作。可使用以下命令执行 [multimedia/pwcview](https://cgit.freebsd.org/ports/tree/multimedia/pwcview/)：
