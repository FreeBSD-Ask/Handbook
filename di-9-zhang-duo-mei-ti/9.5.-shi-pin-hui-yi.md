...
> **注意**
>
> 注意，如果这是一个即插即用的 USB 摄像头，更改连接的 USB 端口将更改 `webcamd -l` 输出中的内容，特别是设备标识符，并且可能需要更新 rc.conf 中的条目。为避免此问题，请使用设备名称（`-N` 选项）和/或在已知的情况下匹配设备序列号（`-S` 选项），如 `webcamd -l` 输出中所示。对于静态设备（如集成笔记本摄像头），可以使用设备标识符（`-d` 选项）。
