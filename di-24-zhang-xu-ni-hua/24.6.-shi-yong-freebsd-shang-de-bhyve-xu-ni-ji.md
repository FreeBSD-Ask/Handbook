...
|  | 如果你打算在主机上以及一个或多个 jails 上使用 bhyve，请记住 tap 和 nmdm 接口名称将在共享环境中运行。例如，你可以仅将 /dev/nmdmbhyve0 用于主机上的 bhyve 或 jail 中的一个。|
| -- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
这将允许你通过 VNC 以及位于/dev/nmdbbhyve0B 的串行控制台连接到你的虚拟机 bhyvevm0。
