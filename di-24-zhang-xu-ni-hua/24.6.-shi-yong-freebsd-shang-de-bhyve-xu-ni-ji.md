...
此示例假设使用类似 pf 或 ipfw 的防火墙来 NAT 您的jail流量。有关实现此操作的可用选项，请参阅防火墙章节。
...
之后，您可以在jail内创建虚拟机。对于 FreeBSD 客户端，请首先下载安装 ISO：
...
这将允许您通过 VNC 以及位于/dev/nmdbbhyve0B 的串行控制台连接到您的虚拟机 bhyvevm0 。
