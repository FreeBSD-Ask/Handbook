...
Linux 虚拟机可以像其他常规 [基于 UEFI 的虚拟机](./24.7.-shi-yong-freebsd-shang-de-bhyve-xu-ni-ji.md) 一样启动，或者可以使用 Port [sysutils/grub2-bhyve](https://cgit.freebsd.org/ports/tree/sysutils/grub2-bhyve/) 。
...
除了 `bhyveload` 和 `grub-bhyve`，bhyve 还可以使用 UEFI 固件启动虚拟机。这种选项可能支持其他加载器不支持的客体操作系统。
