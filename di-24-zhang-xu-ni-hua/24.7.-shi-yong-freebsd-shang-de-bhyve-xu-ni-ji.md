...
将固件安装好后，在 bhyve 命令行中添加 `-l bootrom,<em>/path/to/firmware</em>` 标志。实际的 bhyve 命令可能如下所示：
...
可以通过添加 `-s 29,fbuf,tcp=<em>0.0.0.0:5900</em>` 标志启用 UEFI-GOP 帧缓存。可以使用 `w=<em>800</em>` 和 `h=<em>600</em>` 配置帧缓存分辨率，并且可以通过添加 `wait` 指示 bhyve 在启动虚拟机之前等待 VNC 连接。帧缓存可以从主机或通过网络通过 VNC 协议进行访问。此外，可以添加 `-s 30,xhci,tablet` 以实现与主机的精确鼠标光标同步。
...
>**警告**
>
>制造商并不支持修改 Windows 安装媒体以在没有 TPM 模块的情况下运行 Windows 虚拟机。实施此类方法前，请考虑您的应用程序和使用案例。 
...
如果您在主机和虚拟机中都使用 ZFS，请记住两者都会缓存虚拟机的内容，可能会造成内存竞争压力。为缓解此问题，考虑将主机的 ZFS 文件系统设置为仅缓存元数据。为此，请对主机上的 ZFS 文件系统应用以下设置，将 `<name>` 替换为特定虚拟机 zvol 数据集的名称：
...
>**注意**
>
> 如果您打算在主机和一个或多个 jails 中使用 bhyve，请记住，**tap** 和 **nmdm** 接口名称将在共享环境中操作。例如，您只能将 **/dev/nmdmbhyve0** 用于主机或 jail 中的 bhyve。
...
然后，在 **/etc/jail.conf** 或 **/etc/jail.conf.d** 中为新的 jail 添加定义。将接口编号 **\$if** 和 IP 地址替换为您的个人变体。
...
此示例假设使用类似 `pf` 或 `ipfw` 的防火墙来 NAT 您的 jail 流量。有关实现此功能的更多详细信息，请参见 [防火墙](https://docs.freebsd.org/en/books/handbook/firewalls/#) 章节。
...
>**注意**
>
> 如果您之前将 **/etc/devfs.rules** 中的 devfs 规则集 ID 100 替换为您自己的唯一数字，请记得在 **jails.conf** 中也替换相应的数字 ID。
...
之后，您可以在 Jail 内创建虚拟机。首先，下载 FreeBSD 虚拟机操作系统的安装 ISO 文件：
...
如果您希望使用 UEFI 虚拟机操作系统，请首先在 Jail 内安装所需的固件包 [sysutils/bhyve-firmware](https://cgit.freebsd.org/ports/tree/sysutils/bhyve-firmware/)：
...
这能让您通过 VNC 连接到虚拟机 `bhyvevm0`，并通过 **/dev/nmdbbhyve0B** 使用串行控制台。
...
**nmdm** 设备路径中的编号必须对每个虚拟机唯一，并且在 bhyve 启动之前，不能被任何其他进程使用。编号可以随意选择，无需来自连续的数字序列。设备节点对（即 **/dev/nmdm0a** 和 **/dev/nmdm0b**）是在 bhyve 连接其控制台时动态创建的，并在 bhyve 关闭时销毁。在创建启动虚拟机的脚本时，请牢记这一点：您需要确保所有虚拟机都分配有唯一的 **nmdm** 设备。
...
1. **/etc/sysctl.conf**
   当使用 **tap** 接口作为网络后端时，您需要手动将每个使用的 **tap** 接口设置为 UP，或者仅需设置以下 sysctl 参数：
...
2. **/etc/rc.conf**
   要通过 **桥接** 将虚拟机的 **tap** 设备连接到网络，您需要在 **/etc/rc.conf** 中持久化设备设置。此外，您可以通过 `kld_list` 配置变量加载必要的内核模块 `vmm` 用于 bhyve 和 `nmdm` 用于 **nmdm** 设备。在配置 `ifconfig_bridge0` 时，确保将 `<ipaddr>/<netmask>` 替换为物理接口（在此示例中为 **igb0**）的实际 IP 地址，并从物理设备中移除 IP 设置。
...
对于一个与网络连接的 *igb0* 接口，其 IP 为 `10.10.10.1`，子网掩码为 `255.255.255.0`，您将使用以下命令：
