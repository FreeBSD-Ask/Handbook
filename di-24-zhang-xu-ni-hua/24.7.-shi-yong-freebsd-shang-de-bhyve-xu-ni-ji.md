...
FreeBSD 附带了示例脚本 `vmrun.sh`，用于在 bhyve 中运行虚拟机。该脚本将引导虚拟机并在一个循环中运行，这样如果虚拟机崩溃，它会自动重新启动。`vmrun.sh` 接受多个选项来控制虚拟机的配置，包括：
...
重启虚拟机。重启虚拟机会导致 bhyve 退出，但 **vmrun.sh** 脚本会在循环中运行 `bhyve` 并自动重新启动。当发生这种情况时，选择引导菜单中的重启选项以跳出循环。现在可以从虚拟磁盘引导虚拟机系统：
...
Linux 虚拟机可以像其他常规 [基于 UEFI 的虚拟机](./24.7.-shi-yong-freebsd-shang-de-bhyve-xu-ni-ji.md) 一样引导，或者可以使用 Port [sysutils/grub2-bhyve](https://cgit.freebsd.org/ports/tree/sysutils/grub2-bhyve/) 。
...
使用 `grub2-bhyve` 引导 Linux 虚拟机是一个两步过程。
...
这将引导 grub。如果安装 CD 包含 **grub.cfg** 文件，将显示一个菜单。如果没有，则必须手动定位并加载 `vmlinuz` 和 `initrd` 文件：
...
系统将引导并开始安装。在虚拟机中安装系统后，重启虚拟机。重启虚拟机会导致 bhyve 退出。虚拟机实例需要被销毁才能重新启动：

```sh
# bhyvectl --destroy --vm=linuxguest
```

现在，可以直接从虚拟磁盘引导虚拟机。加载内核：
...
> **注意**
>
> 某些 Linux 发行版要求使用 UEFI 变量存储其 UEFI 引导文件的路径（例如，使用 `linux64.efi` 或 `grubx64.efi`，而不是 `bootx64.efi`）。因此，建议使用变量文件来避免手动修改引导分区文件。

要查看或修改变量文件的内容，可以使用 [efivar(8)](https://man.freebsd.org/cgi/man.cgi?query=efivar&sektion=8&format=html) 从主机进行操作。

[sysutils/bhyve-firmware](https://cgit.freebsd.org/ports/tree/sysutils/bhyve-firmware/) 还包含支持 CSM 的固件，用于以传统 BIOS 模式引导没有 UEFI 支持的虚拟机：
...
可以通过添加 `-s 29,fbuf,tcp=0.0.0.0:5900` 标志启用 UEFI-GOP 帧缓存。可以使用 `w=800` 和 `h=600` 配置帧缓存分辨率，并且可以通过添加 `wait` 指示 bhyve 在引导虚拟机之前等待 VNC 连接。帧缓存可以从主机或通过网络通过 VNC 协议进行访问。此外，可以添加 `-s 30,xhci,tablet` 以实现与主机的精确鼠标光标同步。
...
> **警告**
>
> 制造商并不支持修改 Windows 安装媒体以在没有 TPM 模块的情况下运行 Windows 虚拟机。采用此类方法前，请评估你的应用程序和使用场景。
...
现代虚拟机监控程序允许用户创建“快照”，该快照包括虚拟机的磁盘、CPU 和内存内容。快照通常可以在虚拟机运行或关闭的情况下创建。然后，可以将虚拟机恢复到快照拍摄时的精确状态。
...
虽然可以在虚拟机运行时对 ZFS 卷进行快照，但请注意，在虚拟机活跃时，虚拟磁盘的内容可能处于不一致的状态。因此，建议在执行此命令之前，先关闭或暂停虚拟机。默认情况下，不支持暂停虚拟机，需要先启用（参见 [内存和 CPU 快照](./24.7.-shi-yong-freebsd-shang-de-bhyve-xu-ni-ji.md)）
...
然而，此功能默认情况下未启用，并且需要从源代码重新编译系统。有关编译内核和自定义选项的详细说明，请参见 [从源代码构建](../di-26-zhang-freebsd-geng-xin-yu-sheng-ji/26.6.-cong-yuan-dai-ma-geng-xin-freebsd.md)。
...
> **注意**
>
> 在执行以下步骤之前，请确保 **/usr/src** 目录是最新的。请参见 [更新源代码](../di-26-zhang-freebsd-geng-xin-yu-sheng-ji/26.6.-cong-yuan-dai-ma-geng-xin-freebsd.md) 以了解详细操作步骤。
...
然后按照 [从源代码更新 FreeBSD 快速入门部分](../di-26-zhang-freebsd-geng-xin-yu-sheng-ji/26.6.-cong-yuan-dai-ma-geng-xin-freebsd.md) 中的步骤构建并安装基本系统和内核。
...
> **注意**
>
> 向 `--suspend` 提供绝对路径和文件名。否则，bhyve 将把快照数据写入引导 bhyve 时所在的目录。
>
...
为了提高安全性并将虚拟机与主机操作系统隔离，可以将 bhyve 运行在 jail 中。有关 jail 及其安全性好处的详细说明，请参见 [Jails](../di-17-zhang-jail/17.1.-gai-shu.md)。
...
如果使用 [ZFS 文件系统](../di-22-zhang-z-wen-jian-xi-tong-zfs/22.1.-shi-shi-mo-shi-zfs-yu-zhong-bu-tong.md)，请使用以下命令：
...
此示例假设使用类似 `pf` 或 `ipfw` 的防火墙来 NAT 你的 jail 流量。有关实现此功能的更多详细信息，请参见 [防火墙](../di-33-zhang-fang-huo-qiang/33.1.-gai-shu.md) 章节。
...
首次引导 Jail 并进行一些额外的配置工作时，输入以下命令：
...
> **注意**
>
> 在 Jail 中引导虚拟机时，可能需要使用 `bhyvectl` 创建虚拟机。跳过此步骤可能会导致引导 `bhyve` 时出现以下错误信息：
>
> ```sh
...
**nmdm** 设备路径中的编号必须对每个虚拟机唯一，并且在 bhyve 引导之前，不能被任何其他进程使用。编号可以随意选择，无需来自连续的数字序列。设备节点对（即 **/dev/nmdm0a** 和 **/dev/nmdm0b**）是在 bhyve 连接其控制台时动态创建的，并在 bhyve 关闭时销毁。在创建引导虚拟机的脚本时，请牢记这一点：你需要确保所有虚拟机都分配有唯一的 **nmdm** 设备。
...
为了在引导时自动引导 bhyve 虚拟机，需要进行一些配置文件的修改。
...
> **注意**
>
> 修改系统的 IP 地址配置可能会导致远程连接中断（例如通过 SSH 连接）！在执行这些命令时，请确保采取必要的预防措施以保持对系统的访问，或在本地终端会话中进行这些修改。
