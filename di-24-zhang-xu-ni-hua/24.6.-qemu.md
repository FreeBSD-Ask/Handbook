...
这将打开一个窗口，QEMU 会尝试从硬盘、软盘、DVD/CD 和 PXE 启动。由于尚未设置任何引导镜像，因此命令会产生一些错误并以 "No bootable device" 结束，如 [图 1](./24.6.-qemu.md) 所示。不过，这表明 QEMU 软件已正确安装。
...
QEMU 将在一个单独的窗口中启动虚拟机，并按 [图 2](./24.6.-qemu.md) 中所示引导 FreeBSD ISO。所有命令选项，如 `-cpu` 和 `-boot`，均在 QEMU 的手册页 [qemu(1)](https://man.freebsd.org/cgi/man.cgi?query=qemu&sektion=1&format=html) 中有详细描述。
...
[安装 Xorg](../di-5-zhang-xwindow-xi-tong/5.4.-an-zhuang-xorg-fu-wu-qi.md) 描述了如何设置 `X Window` 系统。请参考该指南进行初始的 `X Window` 设置，然后查阅 [桌面环境](../di-8-zhang-zhuo-mian-huan-jing/8.1.-gai-shu.md) 以了解如何设置完整的桌面环境。
