...
要配置 PPP 连接，首先要编辑/etc/ppp/ppp.conf，输入 ISP 的拨号信息。该文件的描述如下：
...
为测试连接启用详细记录参数。一旦配置正常工作，应将此行减少到：
...
默认情况下， ppp 必须作为 root 运行。要更改此默认设置，请将应运行 ppp 的用户帐户添加到 /etc/group 中的 network 组。
...
这一行指定了 PAP/CHAP 密码。插入 MyPassword 的正确值。您可能希望添加另一行，比如：
...
可能是一个好主意确保 sendmail_flags 行不包括 -q 选项，否则 sendmail 会不时尝试进行网络查找，可能会导致您的机器拨号。您可以尝试：
...
一旦套接字设置完成，pppctl(8) 程序可用于希望操作正在运行的程序的脚本。

### 配置拨号服务

“拨号服务”提供了使用 getty(8)启用拨号服务的良好描述。
