...
其中 **filename** 可以是任何类型的文件。该命令会产生一些噪音，以确认声卡工作。
...
|         `xxx: can’t open /dev/dsp!`        | 输入 `fstat \| grep dsp` 来检查是否有其他的应用程序在打开设备。值得注意导致问题的是 esound 和 KDE 的声音支持。 |

现代显卡通常都有自己的声卡驱动，以便与 HDMI 一起使用。若这个显卡设备有时被列在声卡之前，意味着声卡不会被用作默认的播放设备。要检查是否是这种情况，可以运行 dmesg 并查找 `pcm`。输出结果看起来像这样：
...
在这个例子中，显卡（Nvidia，英伟达）的排序先于声卡（Realtek，瑞昱 ALC889）。要让声卡成为默认播放设备，将 `hw.snd.default_unit` 更改为播放需使用的设备：
...
要让 `hw.snd.default_unit` 中的变更生效，使用 [audio/pulseaudio](https://cgit.freebsd.org/ports/tree/audio/pulseaudio/pkg-descr) 的程序可能需要重启守护程序[audio/pulseaudio](https://cgit.freebsd.org/ports/tree/audio/pulseaudio/pkg-descr)。另一种方式是实时更改 [audio/pulseaudio](https://cgit.freebsd.org/ports/tree/audio/pulseaudio/pkg-descr) 设置——用 [pacmd(1)](https://www.freebsd.org/cgi/man.cgi?query=pacmd\&sektion=1\&format=html) 定位到守护程序 [audio/pulseaudio](https://cgit.freebsd.org/ports/tree/audio/pulseaudio/pkg-descr)，打开命令行：  
