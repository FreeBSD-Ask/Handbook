...
**当内容被转换到新版本时，包数据库的转换可能会发出错误。一般来说，这些错误可以被安全地忽略掉。然而，在 pkg2ng 完成后，会显示一个未成功转换的软件列表。这些应用程序必须手动重新安装。**
...
一旦一个软件包成为子包并被标记为自动，它就会被 pkg 自动删除选中。
...
作为最后一个例子，要把libglut共享库的来源从graphics/libglut改为graphics/freeglut，运行。
