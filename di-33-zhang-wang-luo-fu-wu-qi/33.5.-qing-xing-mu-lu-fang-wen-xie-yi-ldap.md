...
FreeBSD 不提供内建的 LDAP 服务器。首先，通过安装 [net/openldap-server](https://cgit.freebsd.org/ports/tree/net/openldap-server/) 软件包或 Ports 来进行配置：
...
服务器仍然需要信任。如果以前没有进行过此操作，请按照以下说明进行操作。安装 OpenSSL 软件包或 Ports：
