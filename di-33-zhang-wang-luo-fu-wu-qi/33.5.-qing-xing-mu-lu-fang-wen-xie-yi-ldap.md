...
FreeBSD 不提供内建的 LDAP 服务器。首先，通过安装软件包或 Port [net/openldap-server](https://cgit.freebsd.org/ports/tree/net/openldap-server/) 来进行配置：

```sh
# pkg install openldap-server
```

[软件包](https://docs.freebsd.org/en/articles/linux-users/#software) 中启用了大量的默认选项。通过运行 `pkg info openldap-server` 来查看它们。如果这些选项不够（例如需要 SQL 支持），请考虑使用适当的 [框架](../di-4-zhang-an-zhuang-ying-yong-cheng-xu-ruan-jian-bao-he-ports/4.5.-shi-yong-ports.md) 重新编译 Port。
...
服务器仍然需要信任。如果以前没有进行过此操作，请按照以下说明进行操作。安装软件包或 Port OpenSSL：
...
**slapd** 在引导时不提供调试信息。可以查看 **/var/log/debug.log**、**dmesg -a** 和 **/var/log/messages** 来进行调试。
