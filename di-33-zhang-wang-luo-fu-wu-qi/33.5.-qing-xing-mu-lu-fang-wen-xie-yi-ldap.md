...
轻量级目录访问协议（LDAP）是一种应用层协议，用于访问、修改和验证对象，使用分布式目录信息服务。可以将其看作是一个电话簿或记录本，存储多个层级的同类信息。它用于 Active Directory 和 OpenLDAP 网络，让用户能通过单个账户访问多个层级的内部信息。例如，电子邮件认证、拉取员工联系信息和内部网站认证都可以利用 LDAP 服务器的记录库中的单个用户账户。
