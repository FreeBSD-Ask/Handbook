...
如果某个 Linux 应用程序在配置了 Linux 二进制兼容性后，仍报错缺少共享库，请确定Linux二进制需要哪些共享库，并手动安装它们。
...
然后，将 Linux 系统输出的最后一列中的所有文件复制到 FreeBSD 系统的 **/compat/linux** 中。复制完成后，为第一列中的名称创建符号链接。此示例将在 FreeBSD 系统上生成以下文件：
...
`ldd` 输出显示二进制文件需要更高的版本：

```
libc.so.4 (DLL Jump 4.5pl26) -> libc.so.4.6.29
```

现有的库只迭代了一两个小版本，因此程序仍然可以使用稍旧的版本。但是，用新版本替换现有的 `libc.so` 是安全的：
...
一般来说，只有在 FreeBSD 上安装 Linux 程序的前几次，才需要寻找 Linux 二进制文件所依赖的共享库。一段时间后，系统中就会有足够的 Linux 共享库，不需要额外的工作能够运行新安装的 Linux 二进制文件。

## 10.5.2.品牌化 Linux ELF 二进制文件

FreeBSD 内核使用几种方法来确定要执行的二进制文件是否是 Linux 的：它检查 ELF 文件头中的品牌，寻找已知的 ELF 解释器路径，并检查 ELF 注释；最后，默认情况下，没有品牌的 ELF 可执行文件被认定为是 Linux 的。如果这些方法都失败，尝试执行二进制文件可能会导致错误消息：
...
要安装基于 Linux RPM 的应用程序，首先安装 [archivers/rpm4](https://cgit.freebsd.org/ports/tree/archivers/rpm4/pkg-descr) 包或通过 ports 。安装后，`root` 可以使用此命令安装 **.rpm**：
...
如有必要，可以 **brandelf** 安装的 ELF 二进制文件。请注意，这将妨碍干净地卸载。
...
这规定了首先搜索 **/etc/hosts** ，其次搜索 DNS 。当 **/compat/linux/etc/host.conf** 不存在时， Linux 应用程序会使用 **/etc/host.conf** 并报错不兼容 FreeBSD 语法。如果没有使用 **/etc/resolv.conf** 配置域名服务器，则删除 **bind** 。
