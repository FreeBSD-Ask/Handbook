...
* 在 MITKerberos 中。要让一个委托人的票据寿命超过默认的 10 小时。可以在 [kadmin（8）](https://www.freebsd.org/cgi/man.cgi?query=kadmin\&sektion=8\&format=html) 提示下使用 `modify_principal` 来改变相关委托人和 `krbtgt` 委托人的最大寿命。然后。委托人可以使用 `kinit -l` 来请求一个寿命更长的 ticket。
