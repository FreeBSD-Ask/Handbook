...
- 每个网络上的网关至少有一个外部IP地址。在这个例子中，公司局域网的外部IP地址是172.16.5.4，家庭局域网的外部IP地址是192.168.1.12。
...
首先，必须从Ports Collection安装 [security/ipsec-tools](https://cgit.freebsd.org/ports/tree/security/ipsec-tools/pkg-descr)。该软件提供了许多支持该配置的应用程序。

下一个要求是创建两个 [gif（4）](https://www.freebsd.org/cgi/man.cgi?query=gif&sektion=4&format=html) 伪设备，它们将用于隧道数据包并允许两个网络正确通信。使用root账户，在每个网关上运行以下命令：
...
完成后，两个内部 IP 地址都应该可以使用 [ping（8） 访问](https://www.freebsd.org/cgi/man.cgi?query=ping&sektion=8&format=html)：
...
对于 [pf（4）](https://www.freebsd.org/cgi/man.cgi?query=pf&sektion=4&format=html) 或 [ipf（8） 的用户](https://www.freebsd.org/cgi/man.cgi?query=ipf&sektion=8&format=html)， 以下规则应该可以解决问题：
