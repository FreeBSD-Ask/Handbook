...
| 警告                                                         |
| ------------------------------------------------------------ |
| 许多用户认为钥匙在设计上是安全的，并且会使用没有密码的钥匙。这是很危险的行为。管理员可以通过手动查看私钥来验证一个密钥对是否受到口令的保护。如果私钥文件包含 `ENCRYPTED` 字样，则说明密钥所有者使用了口令。此外，为了更好地保护最终用户，可以在公钥文件中加入 `from`。例如，在 `ssh-rsa` 前缀前面加上 `from="192.168.10.5 "` 将只允许该特定用户从该 IP 地址登录。 |
