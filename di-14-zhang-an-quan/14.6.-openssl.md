...
OpenSSL 的另一个常见用途是提供用于软件应用的证书。证书可以用来验证一个公司或个人的资质。如果一个证书没有经过外部 _*证书授权机构*_（CA）（如 [http://www.verisign.com](http://www.verisign.com)）的签名，使用该证书的应用程序将产生一个警告。获得签名证书是有成本的，但是使用签名证书并不是强制性的，因为证书可以是自签的。然而，使用外部权威机构可防止警告，可以让用户放心。
