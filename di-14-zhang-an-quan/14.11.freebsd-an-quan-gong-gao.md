...
像许多高质量操作系统的开发者一样。FreeBSD 项目有一个安全团队。负责确定每个 FreeBSD 发行版的生命周期结束（EoL）日期。并为尚未达到其 EoL 的受支持发行版提供安全更新。有关 FreeBSD 安全团队和支持的版本的更多信息，请访问 [FreeBSD 安全页面](https://www.freebsd.org/security)。
...
- 每个安全公告都由安全官员的 PGP 密钥签名。安全官的公钥可以在 [OpenPGP 密钥](https://docs.freebsd.org/en/books/handbook/pgpkeys/index.html#pgpkeys) 中验证。
...
- `Corrected` 指出了被纠正的日期、时间、时间偏移和版本。括号中的部分显示了已经合并了修正的每个分支。以及该分支对应的版本号。版本标识符本身包括版本号，如果合适，还包括补丁级别。补丁级别是字母 `p` 后面的数字，表示补丁的序列号，使用户跟踪哪些补丁已经应用到系统中。
- `CVE Name` 字段列出了公共安全漏洞数据库 [cve.mitre.org](http://cve.mitre.org/) 中的公告编号（如果存在）。
