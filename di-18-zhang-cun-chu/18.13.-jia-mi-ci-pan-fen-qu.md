...
>
> 1. 添加新的硬盘驱动器\
>    按照 "Adding Disks "中的解释，将新硬盘安装到系统中。在这个例子中，一个新的硬盘分区被添加为`/dev/ad4s1c`，`/dev/ad0s1*`代表现有的标准 FreeBSD 分区
>
> ```
> # ls /dev/ad*
...
一旦加密分区的工作完成，并且不再需要`/private`分区，谨慎的做法是让设备进入冷存储，将 geli 加密分区从内核中卸载并分离出来。
