...
FreeBSD Ports 提供了几个用于刻录和复制音频和数据光盘的实用程序。这一章演示了几个命令行实用程序的使用。对于带有图形化工具的 CD 刻录软件，可以考虑安装 [sysutils/xcdroast](https://cgit.freebsd.org/ports/tree/sysutils/xcdroast/pkg-descr) 或 [sysutils/k3b](https://cgit.freebsd.org/ports/tree/sysutils/k3b/pkg-descr) 包或 port。
