...
输入 `~sforce=single-char` 后按回车。*single-char* 是任何单个字符。如果省略 *single-char*，则强制字符为空字符，可以通过输入 **Ctrl**+**2** 或 **Ctrl**+**Space** 来访问。*single-char* 的不错选择是 **Shift**+**Ctrl**+**6**，它仅在某些终端服务器上使用。
