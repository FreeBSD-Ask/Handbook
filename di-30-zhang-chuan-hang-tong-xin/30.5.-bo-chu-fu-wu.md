...
以下是一些让主机通过调制解调器连接到另一台计算机的提示。这适用于与远程主机建立终端会话。

如果使用 PPP 时遇到问题，这种连接可以帮助获取 Internet 上的文件。如果 PPP 不工作，使用终端会话通过 FTP 获取所需文件。然后使用 zmodem 将其传输到机器上。

## 30.5.1. 使用标准 Hayes 调制解调器

`tip` 中内置了通用的 Hayes 拨号器。在 **/etc/remote** 中使用 `at=hayes`。
...
在 **/etc/remote** 中创建“直接”条目。例如，如果调制解调器连接到第一个串行端口 **/dev/cuau0**，则使用以下行：
...
**/etc/remote** 中的电话号码功能会告知 `tip` 在 **/etc/phones** 中查找电话号码。但 `@` 符号在 **/etc/remote** 等功能文件中是一个特殊字符，因此需要使用反斜杠转义：
...
## 30.5.6. 通过终端服务器访问多个主机

与其等到连接后每次输入 `CONNECT host`，不如使用 `tip` 的 `cm` 功能。例如，下面的条目会让你输入 `tip pain` 或 `tip muffin` 连接主机 `pain` 或 `muffin`，并通过 `tip deep13` 连接到终端服务器。
...
这通常是大学遇到的问题：有多条调制解调器线路，而成千上万的学生试图使用它们。
...
**Ctrl**+**P** 是默认的“强制”字符，用于告诉 `tip` 后续的字符是字面数据。可以通过 `~s` 转义将强制字符设置为任何其他字符，这意味着“设置变量”。

输入 `~sforce=single-char` 后按回车。*single-char* 是任何单个字符。如果省略 *single-char*，则强制字符为空字符，可以通过输入 **Ctrl**+**2** 或 **Ctrl**+**Space** 访问。*single-char* 的不错选择是 **Shift**+**Ctrl**+**6**，它仅在某些终端服务器上使用。
...
以下是为需要输入 **Ctrl**+**2** 和 **Ctrl**+**A** 的 Emacs 用户准备的 **~/.tiprc** 示例：
...
当与另一个类似 UNIX® 的操作系统通信时，可以使用 `~p`（put）和 `~t`（take）命令发送和接收文件。这些命令在远程系统上运行 `cat` 和 `echo` 来接收和发送文件。语法是：
