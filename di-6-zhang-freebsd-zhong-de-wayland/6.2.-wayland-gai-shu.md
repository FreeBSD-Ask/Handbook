...
由于 Wayland 不是 X 服务器，传统的 X 屏幕连接需要使用其他方法，如 VNC 或 RDP 管理远程桌面。其次，Wayland 可以作为独立实体管理客户端与合成器之间的复合通信，而无需支持 X 协议。
...
> **注意**
>
> 当前的 NVIDIA® 驱动程序应该能与大多数 wlroots 合成器兼容，但可能略显不稳定，且不支持所有功能。目前正在寻求志愿者协助开发 NVIDIA® DRM。
...
对于合成器，必须有支持 [evdev(4)](https://man.freebsd.org/cgi/man.cgi?query=evdev&sektion=4&format=html) 驱动程序的内核，才能利用按键绑定功能。默认情况下，**GENERIC** 内核已构建此支持；但是，如果内核经过定制并且去除了 [evdev(4)](https://man.freebsd.org/cgi/man.cgi?query=evdev&sektion=4&format=html) 支持，则需要加载 [evdev(4)](https://man.freebsd.org/cgi/man.cgi?query=evdev&sektion=4&format=html) 模块。此外，Wayland 用户需要成为 `video` 组的成员。要快速更改此设置，请使用 `pw` 命令：
...
所有此处说明的合成器都应该能与 [graphics/drm-kmod](https://cgit.freebsd.org/ports/tree/graphics/drm-kmod/) 开源驱动程序兼容；但是，使用专有驱动程序时，NVIDIA® 显卡可能会遇到一些问题。首先安装以下软件包：

```sh
# pkg install wayland seatd
```

安装完协议和支持软件包后，需要安装合成器来创建用户界面。以下部分将介绍几种合成器。所有使用 Wayland 的合成器都需要在环境中定义运行时目录。自 FreeBSD 14.1 以来，这已自动创建和定义。对于较早的版本，可以通过以下命令在 Bourne shell 中创建此目录：

```sh
% export XDG_RUNTIME_DIR=/var/run/user/`id -u`
```

需要注意的是，大多数合成器会在 `XDG_RUNTIME_DIR` 目录中搜索配置文件。在此处的示例中，将使用参数来指定配置文件路径为 **~/.config**，以保持临时文件和配置文件的分离。建议为每个合成器配置别名，以加载指定的配置文件。
...
接下来，需要为 GUI 环境安装合成器，它类似于 X11 桌面。以下部分将讨论三种合成器，包括基本配置选项、屏幕锁定设置和更多信息的推荐。
