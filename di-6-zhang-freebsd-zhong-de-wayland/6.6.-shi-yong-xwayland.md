...
在此输出中，显示了默认的 Wayland 显示器和为 `Xwayland` 服务器设置的显示器。验证 `Xwayland` 是否正常工作的另一种方法是安装并测试一个小工具（如 [x11/eyes]），然后观察输出。如果 `xeyes` 应用程序能启动，并且眼睛会跟随鼠标指针，说明 `Xwayland` 工作正常。如果出现如下错误，则表示 `Xwayland` 初始化时出了问题，可能需要重新安装：
...
> **警告**
>
> Wayland 的一个安全特性是，在没有运行 X 服务器的情况下，不会有其他网络监听程序。一旦启用了 `Xwayland`，此安全特性不再适用于系统。
