...
1. 将需要运行 Xorg 的用户添加到 `video` 或 `wheel` 组以在 3D 加速可用时启用 。将用户 `jru` 添加到可用的任何组：
...
1. 在某些旧版本的 FreeBSD 上，必须先将系统控制台设置为 [vt(4)](https://www.freebsd.org/cgi/man.cgi?query=vt\&sektion=4\&format=html)，然后才能正常切换回文本控制台。请参阅 [内核模式设置 ( KMS)](https://docs.freebsd.org/en/books/handbook/x11/#x-config-kms)。
