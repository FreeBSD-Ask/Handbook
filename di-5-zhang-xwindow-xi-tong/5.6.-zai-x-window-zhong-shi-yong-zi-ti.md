...
接着将 TrueType® 目录添加到字体路径中，与 [Type1 Fonts](./5.6.-zai-x-window-zhong-shi-yong-zi-ti.md) 说明的方法相同：
...
此方法有效，但 X 会话关闭后会丢失，除非将其添加到启动文件（普通 `startx` 会话的 **~/.xinitrc** 或图形登录管理器 XDM 的 **~/.xsession**）。第三种方法是使用新的 **/usr/local/etc/fonts/local.conf**，如 [Anti-Aliased Fonts](./5.6.-zai-x-window-zhong-shi-yong-zi-ti.md) 所示。
