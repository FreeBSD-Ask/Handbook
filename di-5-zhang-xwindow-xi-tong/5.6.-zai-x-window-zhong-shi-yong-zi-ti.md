...
FreeBSD Ports 提供丰富且不断增长的免费高质量字体目录，可从 [x11-fonts](https://cgit.freebsd.org/ports/tree/x11-fonts/) 安装。

从 Ports 安装的系统范围字体包位于 **/usr/local/share/fonts/**。单用户字体可以放在 **~/.fonts/** 或 **~/.local/share/fonts/**。
...
Ports 树中有大量免费高质量字体可直接用于 X Window 系统。本章简要介绍这两类字体及 X FreeType 接口的配置。
...
然后为 TrueType® 字体创建目录（例如 **/usr/local/share/fonts/TrueType**），并将所有 TrueType® 字体复制到该目录。注意 TrueType® 字体不能直接从 Apple® Mac® 获取，必须是 UNIX®/MS-DOS®/Windows® 格式才能供 X.org 使用。复制完成后，使用 mkfontscale 创建 **fonts.dir**，以便 X 字体渲染器识别新字体。`mkfontscale` 可作为包安装：
...
接着将 TrueType® 目录添加到字体路径中，与 [Type1 字体](./5.6.-zai-x-window-zhong-shi-yong-zi-ti.md#id-5.6.2.-type1-zi-ti) 说明的方法相同：
...
此方法有效，但 X 会话关闭后会丢失，除非将其添加到启动文件（普通 `startx` 会话的 **~/.xinitrc** 或图形登录管理器 XDM 的 **~/.xsession**）。第三种方法是使用新的 **/usr/local/etc/fonts/local.conf**，如 [字体抗锯齿](./5.6.-zai-x-window-zhong-shi-yong-zi-ti.md#id-5.6.3.-zi-ti-kang-ju-chi) 所示。

## 5.6.3. 字体抗锯齿

X.org 中 **/usr/local/share/fonts/** 和 **~/.fonts/** 的字体会自动对 Xft 兼容应用程序启用抗锯齿处理。大多数现代应用程序都是 Xft 兼容的，包括 KDE、GNOME 和 Firefox。

要控制哪些字体启用抗锯齿处理，或配置抗锯齿属性，可创建或编辑 **/usr/local/etc/fonts/local.conf** 文件。Xft 字体系统的许多高级功能可以在该文件中调整，本节仅说明简单设置。详细信息请参阅 [fonts-conf(5)](https://man.freebsd.org/cgi/man.cgi?query=fonts-conf&sektion=5&format=html)。
...
最后一点：对于 LCD 屏幕，可启用子像素采样（sub-pixel sampling），分别处理水平排列的红、绿、蓝子像素以提高水平分辨率，效果可能非常显著。启用方法是在 **local.conf** 中添加：
