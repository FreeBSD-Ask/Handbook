...
* **直接渲染驱动**，支持 PRIME 卸载（offload）。PRIME 能让多个图形处理提供者共存。有关 PRIME 的详细说明，请参见 [显卡配置](https://docs.freebsd.org/en/books/handbook/x11/#x-config-gpu)。
...
这是直接渲染的 [KMS](https://docs.freebsd.org/en/books/handbook/glossary/#kms-glossary) 驱动。

内核模式设置用于在内核中设置显卡模式。通过以下 [loader.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=loader.conf&sektion=5&format=html) 可调参数可在后续启动时启用：
