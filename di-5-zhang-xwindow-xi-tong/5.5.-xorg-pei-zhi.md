...
**摘要**：如果显示器或输入设备的默认设置不符合要求，[桌面](../di-8-zhang-zhuo-mian-huan-jing/8.1.-gai-shu.md) 提供了图形界面来配置，或者也可以手动配置。

X.org 服务器支持大多数常见的图形处理器、显示器和输入设备。首先应尝试使用默认设置。本小节概述其配置方法。

## 5.5.1. X.org 配置文件

过去，X.org 服务器通过 **/usr/local/etc/X11/** 下的文件配置。这种方式在特殊情况中仍受支持，但可能与动态自动配置冲突。
...
直接渲染能同时无缝使用独立显卡（dGPU）和集成显卡（iGPU），称为 PRIME。驱动程序会在需要时自动将高强度任务卸载到 dGPU，并在可能时关闭其电源。
...
X.org 服务器提供了 [x11/libinput](https://cgit.freebsd.org/ports/tree/x11/libinput/) 库，这是一项跨平台的工作，旨在通过统一的库支持所有触控、指点和键盘设备。默认情况下，该库会自动加载。
...
使用 [libinput(4)](https://man.freebsd.org/cgi/man.cgi?query=libinput&sektion=4&format=html) 支持的设备，可通过桌面提供的图形工具，或在运行时使用 [x11/xinput](https://cgit.freebsd.org/ports/tree/x11/xinput/) 和 [x11/setxkbmap](https://cgit.freebsd.org/ports/tree/x11/setxkbmap/) 原子配置。
