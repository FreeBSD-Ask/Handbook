...
目录也被当作文件处理。它们也有读、写和执行权限。目录的可执行位与文件略有不同。当目录被标记为可执行时，意味着可以使用 [cd(1)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=1&format=html) 进入该目录。同时，在文件本身权限允许的前提下，也能访问目录内的文件。
