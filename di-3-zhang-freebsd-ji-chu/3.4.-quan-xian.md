...
> **注意**
>
> `nosuid` [mount(8)](https://man.freebsd.org/cgi/man.cgi?query=mount&sektion=8&format=html) 挂载选项会导致这类二进制文件静默失败，且不通知用户。该选项并非完全可靠，因为某些 setuid 封装程序可能绕过 `nosuid` 挂载限制。
