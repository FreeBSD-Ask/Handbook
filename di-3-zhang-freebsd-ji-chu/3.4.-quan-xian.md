...
|值|权限说明|目录列表表示||----|----------|----------------||0|无读、无写、无执行权限|`---`||1|无读、无写、有执行权限|`--x`||2|无读、有写、无执行权限|`-w-`||3|无读、有写、有执行权限|`-wx`||4|有读、无写、无执行权限|`r--`||5|有读、无写、有执行权限|`r-x`||6|有读、有写、无执行权限|`rw-`||7|有读、有写、有执行权限|`rwx`|
...
|选项|字母|代表含义||--------------|------|---------------------------||(对象 who)|u|用户（User）||(对象 who)|g|所属组（Group owner）||(对象 who)|o|其他用户（Other）||(对象 who)|a|所有人（All，“world”）||(操作 action)|+|添加权限||(操作 action)|-|移除权限||(操作 action)|=|明确设定权限||(权限 perms)|r|读取权限（Read）||(权限 perms)|w|写入权限（Write）||(权限 perms)|x|执行权限（Execute）||(权限 perms)|t|粘滞位（Sticky bit）||(权限 perms)|s|设置 UID 或 GID（Set UID/GID）|
