...
与其他用户账户不同，超级用户可以无所限制地操作系统，若被误用则可能造成严重后果。普通用户无法因失误而破坏整个操作系统。因此建议用户使用普通账户进行日常操作，仅在命令需要额外权限时再切换为超级用户。
...
FreeBSD 提供了多种不同的命令来管理用户账户。最常用的命令已在 [管理用户账户的工具](./3.3.-yong-hu-he-ji-ben-zhang-hu-guan-li.md#id-3.3.2.-guan-li-zhang-hu) 中总结，后面还附有一些使用示例。更多细节和使用示例请参考每个工具的手册页。
...
一个用户组是一组用户的列表。一个用户组通过其组名和 GID 来标识。在 FreeBSD 中，内核通过进程的 UID 和其所属的用户组列表来判断该进程能执行哪些操作。在大多数情况下，某个用户或进程的 GID 通常指的是该列表中的第一个用户组。
