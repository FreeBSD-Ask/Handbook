...
内建的 [su(1)](https://man.freebsd.org/cgi/man.cgi?query=su&sektion=1&format=html) 框架适合单一系统或由一位管理员管理的小型网络。另一个选择是安装软件包或 Port [security/sudo](https://cgit.freebsd.org/ports/tree/security/sudo/)。该软件支持操作记录，并允许管理员配置哪些用户可以以超级用户身份运行哪些命令。
