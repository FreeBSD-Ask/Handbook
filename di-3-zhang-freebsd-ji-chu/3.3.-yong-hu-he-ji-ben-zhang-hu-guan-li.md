...
在 [以超级用户身份使用 `chpass`](#users-modifying-chpass-su) 一节中，超级用户输入了 `chpass jru`，正在查看可更改的字段。若 `jru` 自己运行该命令，仅会显示后六个字段供编辑。这在 [以普通用户身份使用 `chpass`](#users-modifying-chpass-ru) 中展示。
