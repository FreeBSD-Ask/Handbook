...
一些系统账户的例子包括 `daemon`、`operator`、`bind`、`news` 和 `www`。

`nobody` 是一个通用的非特权系统账户。然而，使用 `nobody` 的服务越多，该用户关联的文件和进程就越多，从而其潜在权限也随之提升。
...
**用户 ID（UID）**  
用户 ID 是一个用于在 FreeBSD 系统中唯一标识该用户的数字。支持用户名参数的命令会首先将其转换为 UID。建议使用小于 65535 的 UID，因部分软件可能对更大的 UID 兼容性不佳。

**组 ID（GID）**  
组 ID 是一个用于唯一标识用户所属主要用户组的数字。组是一种机制，通过 GID（而非 UID）来控制对资源的访问。这可显著减小部分配置文件的体积，并允许用户同时属于多个组。建议使用不超过 65535 的 GID，因更高的 GID 可能会导致某些软件运行异常。
...
**账户过期时间**  
FreeBSD 默认不会使账户过期。如需创建具有使用期限的账户（例如学校中的学生账户），可使用 [pw(8)](https://man.freebsd.org/cgi/man.cgi?query=pw&sektion=8&format=html) 命令指定账户过期时间。一旦过期，该账户将无法登录，但其目录和文件仍会保留。
...
超级用户账户通常称为 `root`，用于以无限制权限管理系统。因此，**不应**将其用于日常任务，例如收发邮件、浏览系统或编程。
...
要从系统中完全删除一个用户，请以超级用户身份运行 [rmuser(8)](https://man.freebsd.org/cgi/man.cgi?query=rmuser&sektion=8&format=html)。该命令会执行以下步骤：

1. 删除该用户的 [crontab(1)](https://man.freebsd.org/cgi/man.cgi?query=crontab&sektion=1&format=html) 项（若存在）。
...
任何用户都可以使用 [chpass(1)](https://man.freebsd.org/cgi/man.cgi?query=chpass&sektion=1&format=html) 更改其默认 shell 和账户的个人信息。超级用户可以用此工具更改任意用户的额外账户信息。
