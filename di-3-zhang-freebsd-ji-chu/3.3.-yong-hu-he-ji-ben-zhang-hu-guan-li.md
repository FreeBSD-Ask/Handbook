...
>4. 从系统的本地密码文件中删除该用户。
>
>5. 可以选择删除用户的主目录，如果该目录是由用户拥有的话。
>
>6. 从 **/var/mail** 中删除属于该用户的接收邮件文件。
>
>7. 从临时文件存储区（如 **/tmp**）中删除属于该用户的所有文件。
>
>8. 最后，从 **/etc/group** 中从所有包含该用户的组删除该用户。如果一个组变成空的，并且组名与用户名相同，那么这个组就会被删除。具体细节请参考 [adduser(8)](https://www.freebsd.org/cgi/man.cgi?query=adduser&sektion=8&format=html) 中，默认为每个用户创建的唯一组。
