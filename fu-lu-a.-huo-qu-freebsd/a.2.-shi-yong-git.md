...
本节介绍如何在 FreeBSD 系统上安装 Git，并用它创建 FreeBSD 源代码存储库的本地副本。
...
Git 使用 URL 来指代存储库。共有三个不同的存储库：`src` 存放 FreeBSD 系统源代码，`doc` 存放文档，`ports` 存放 FreeBSD Ports。三个存储库均可通过两种协议访问：HTTPS 和 SSH。例如，URL `https://git.FreeBSD.org/src.git` 指定的是 `src` 存储库的主分支，使用 `https` 协议。

表 1. FreeBSD Git 存储库 URL 表

| 项目 | Git URL |
| ---- | ------- |
| 通过 HTTPS 访问只读 src 存储库 | [`https://git.FreeBSD.org/src.git`](https://git.freebsd.org/src.git) |
| 通过 anon-ssh 访问只读 src 存储库 | `ssh://anongit@git.FreeBSD.org/src.git` |
| 通过 HTTPS 访问只读 doc 存储库 | [`https://git.FreeBSD.org/doc.git`](https://git.freebsd.org/doc.git) |
| 通过 anon-ssh 访问只读 doc 存储库 | `ssh://anongit@git.FreeBSD.org/doc.git` |
| 通过 HTTPS 访问只读 Ports 存储库 | [`https://git.FreeBSD.org/ports.git`](https://git.freebsd.org/ports.git) |
| 通过 anon-ssh 访问只读 Ports 存储库 | `ssh://anongit@git.FreeBSD.org/ports.git` |
...
`-o freebsd` 选项指定源头；按照 FreeBSD 文档惯例，源头默认为 `freebsd`。初始检出需要下载远程存储库的完整分支，可能耗时较长，请耐心等待。
...
关于存储库写权限，请参阅 [Committer's Guide](https://docs.freebsd.org/en/articles/committers-guide/#git-mini-primer)。

## A.2.6. 外部镜像

这些镜像不托管在 FreeBSD.org 上，但仍由项目成员维护。欢迎用户和开发者从这些镜像拉取或浏览存储库。`doc` 和 `src` 的 GitHub 存储库接受拉取请求；除此以外，这些镜像对应的项目工作流仍在讨论中。
