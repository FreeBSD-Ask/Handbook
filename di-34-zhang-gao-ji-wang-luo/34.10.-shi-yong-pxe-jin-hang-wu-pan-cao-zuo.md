...
Intel® 预启动执行环境（PXE）能通过网络引导操作系统。例如，FreeBSD 系统可以通过网络引导，并在没有本地磁盘的情况下运行，使用从 NFS 服务器挂载的文件系统。PXE 支持通常在 BIOS 中提供。要在机器启动时使用 PXE，请在 BIOS 设置中选择“从网络引导”选项，或在系统初始化过程中按下功能键。
