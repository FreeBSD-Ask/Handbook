...
希望向远程客户端提供 Bluetooth 服务的本地服务器应用程序将服务注册到本地 SDP 守护进程。一个这样的应用程序示例是 [rfcomm\_pppd(8)](https://man.freebsd.org/cgi/man.cgi?query=rfcomm_pppd&sektion=8&format=html)。一旦启动，它将与本地 SDP 守护进程注册 Bluetooth LAN 服务。
...
OBEX 服务器和客户端由 obexapp 实现，可以通过 [comms/obexapp](https://cgit.freebsd.org/ports/tree/comms/obexapp/) 包或端口进行安装。
