...
特定于语言的字符集，如 ISO8859-1、ISO8859-15、KOI8-R 和 CP437，在 multibyte(3)中有描述。字符集的活动列表可以在 IANA 注册表中找到。
...
本节的其余部分描述了在 FreeBSD 系统上配置区域设置的各种方法。下一节将讨论查找和编译具有 i18n 支持的应用程序时需要考虑的事项。
...
对于具有宽字符或多字节字符的语言，请从 FreeBSD 定制集合中安装该语言的控制台。可用的软件包总结在来自 FreeBSD 定制集合的可用控制台中。安装后，请参考软件包的 pkg-message 或 man 页面获取配置和使用说明。

表 3. 来自 FreeBSD 定制集合的可用控制台
...
X 窗口系统描述了如何安装和配置 Xorg。当为本地化配置 Xorg 时，FreeBSD Ports 集合中提供了额外的字体和输入法。应用程序特定的 i18n 设置，如字体和菜单，可以在 ~/.Xresources 中调整，并应允许用户在图形应用程序菜单中查看其选择的语言。

X 输入法 (XIM) 协议是输入非英文字符的 Xorg 标准。可用输入法概述了 FreeBSD Ports 集合中可用的输入法应用程序。还提供了额外的 Fcitx 和 Uim 应用程序。
