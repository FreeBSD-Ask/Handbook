...
引起 error 19 的其他问题需要更多的努力来修复。尽管系统应该从 ada0 引导，但如果 /etc/fstab 不正确，将出现另一个选择 shell 的提示。在引导装载程序提示符处输入 ufs:/dev/ada0s1a 然后按 Enter 键。撤消 /etc/fstab 中的编辑，然后从原始磁盘（ada0）而不是镜像中装载文件系统。重新启动系统，然后再尝试该过程。
