...
如果遇到问题，请参见 [故障排除](./21.3.-raid1-jing-xiang.md)。关闭电源并断开原始的 **ada0** 磁盘后，可以将其保留为离线备份。
...
重新启动系统，从 **ada1** 引导。如果一切正常，系统将从 **mirror/gm0** 引导，且现在包含与之前 **ada0** 相同的数据。若有问题引导，请参阅 [故障排除](./21.3.-raid1-jing-xiang.md)。
...
应该清除新驱动器上任何旧的元数据，方法请参见 [元数据问题](./21.3.-raid1-jing-xiang.md)。然后，将新驱动器（例如 **ada4**）插入镜像中：
