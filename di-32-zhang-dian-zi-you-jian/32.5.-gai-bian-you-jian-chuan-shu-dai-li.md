...
> **警告**
>
> 如果禁用了默认的外发邮件服务，务必使用替代的邮件传递系统进行替换。否则，像 [periodic(8)](https://man.freebsd.org/cgi/man.cgi?query=periodic&sektion=8&format=html) 这样的系统功能将无法通过电子邮件发送结果。系统的许多部分都依赖一个功能正常的 MTA。如果应用程序在禁用后继续使用默认的二进制文件尝试发送邮件，邮件可能会进入一个不活跃的队列并永远无法发送。
...
按照 [配置 DragonFly 邮件代理 (DMA)](./32.3.-dma.md) 中的配置说明进行配置。
...
> **技巧**
>
> 安装应用程序后阅读安装消息是一个好习惯，安装消息中包含了关于设置等方面的有用信息。
