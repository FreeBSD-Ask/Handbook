...
所列的格式，如 YUV2 和 YUV12 ，并不存在于 XVideo 的每个实现中，它们的缺失可能会妨碍一些播放器。
...
得到的 MPEG 文件，**output_vcd.mpg**，已经准备好用 MPlayer 播放了。该文件可以用 [multimedia/vcdimager]() 或 [sysutils/cdrdao]() 等工具刻录在 CD 介质上以创建视频 CD。
