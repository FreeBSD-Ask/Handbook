...
在 **/boot/device.hints** 中使用的语法在 [sound(4)]() 和声卡驱动程式的手册页中有描述。
...
>提示：
>音频光盘有专门的编码，这意味着它们不应该用 [mount(8)]() 来挂载。
...
>注意：
>*/dev/dsp* 设备节点将根据需要自动创建。未使用时，它们不存在，也不会出现在 [ls(1)]() 的输出中。
...
>提示：
>不要使用 `exit` 命令来退出命令行界面。那会杀死 [audio/Pulseaudio]() 守护进程。用  `Ctrl+D` 代替。
