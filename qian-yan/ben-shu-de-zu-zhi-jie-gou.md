...
本书在逻辑上分为五个不同的部分。第一部分，入门，包括 FreeBSD 的安装和基本使用。希望读者能按顺序阅读这些章节，可以跳过您熟悉的章节。第二部分，通用任务，涵盖了 FreeBSD 的一些常用功能。这一节以及后面的所有章节都可以不按顺序阅读。每一章的开头都有一个简洁的提要，描述了这一章的内容和读者应该已经知道的内容。这样做的目的是让普通读者能够跳过这些章节，找到感兴趣的章节。第三部分，系统管理，包括管理主题。第四部分，网络通信，包括网络和服务器主题。第五部分包含参考信息的附录。

## 简介
向新用户介绍 FreeBSD。其描述了 FreeBSD 项目的历史，它的目标和开发模式。
...
## X Window
描述了一般的 X Window 系统，特别是在 FreeBSD 上使用 X11。还介绍了常见的桌面环境，如 KDE 和 GNOME。

## 桌面应用程序
列出了一些常见的桌面应用程序，如网络浏览器和生产力工具，并介绍了如何在 FreeBSD 上安装它们。

## 多媒体
展示了如何为您的系统设置声音和视频播放支持。还描述了一些音频和视频应用的样本。

## 配置 FreeBSD 内核
解释了为什么您可能需要配置一个新的内核，并提供了配置、构建和安装自定义内核的详细说明。

## 打印
描述了在 FreeBSD 上管理打印机的情况，包括关于横幅页面、打印机记账和初始设置的信息。

## Linux® 二进制兼容
描述了 FreeBSD 的 Linux® 兼容性特点。还提供了许多流行的 Linux® 应用程序的详细安装说明，如 Oracle® 和 Mathematica®。

## 配置和调整
描述了系统管理员可以用来调整 FreeBSD 系统以获得最佳性能的参数。还描述了 FreeBSD 中使用的各种配置文件以及在哪里可以找到它们。

## FreeBSD 的启动过程
描述了 FreeBSD 的启动过程，并解释了如何通过配置选项来控制这个过程。

## 安全性
描述了许多不同的工具，包括 Kerberos、IPsec 和 OpenSSH，以帮助保持您的 FreeBSD 系统的安全。

## Jails
描述了jails 框架，以及 jails 相对于 FreeBSD 的传统 chroot 支持的改进。
...
## GEOM: 模块化磁盘转换框架
描述了 FreeBSD 中的 GEOM 框架是什么，以及如何配置各种支持的 RAID 级别。

## 其他文件系统
检查FreeBSD中对非本地文件系统的支持，如Sun™的Z文件系统。
