...
在这个例子中，假设 Sendmail 在 `example.FreeBSD.org` 上正常运行，直接发送到 [yourlogin@example.FreeBSD.org](yourlogin@example.FreeBSD.org) 的邮件应该没有问题。
...
所有发往 [example.FreeBSD.org](yourlogin@example.FreeBSD.org) 的邮件都会被收集到 `hub` 上的同一个用户名下，而不是直接发往你的主机。
...
为了使生活变得更简单，在 MTA 和带有 MUA 的系统上都应该存在一个具有相同用户名的用户账户。使用 [adduser(8)](https://www.freebsd.org/cgi/man.cgi?query=adduser&sektion=8&format=html) 来创建用户账户。
...
`customer1.org` *不* 需要 `A` 记录，以便只处理该域名的电子邮件。然而，除非有 `A` 记录存在，否则对 `customer1.org` 运行 `ping` 将不工作。
