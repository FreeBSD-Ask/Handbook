...
重新启动系统以测试新镜像，并验证所有数据是否已复制。BIOS 会将镜像视为两个独立的磁盘，而不是一个镜像。由于磁盘是相同的，选择哪一个引导都无关紧要。
...
其他导致 `error 19` 的问题需要更多精力来修复。尽管系统应从 **ada0** 引导，如果 **/etc/fstab** 不正确，仍会出现另一个提示符以选择一个 shell。在引导加载程序提示符下输入 `ufs:/dev/ada0s1a`，然后按回车键。撤销 **/etc/fstab** 中的编辑，然后挂载原始磁盘 (**ada0**) 上的文件系统，而不是镜像。重新启动系统并再次尝试该过程。
