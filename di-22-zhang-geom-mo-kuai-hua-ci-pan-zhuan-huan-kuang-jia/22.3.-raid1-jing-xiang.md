...
重新启动系统，从 **ada1** 引导。如果一切正常，系统将从 **mirror/gm0** 引导，且现在包含与之前 **ada0** 相同的数据。如果在引导时遇到问题，请参阅 [故障排除](./22.3.-raid1-jing-xiang.md#id-22.3.4.-gu-zhang-pai-chu)。
...
如果忘记在 **/boot/loader.conf** 中加载 **geom_mirror.ko** 模块，可能会导致此问题。要解决此问题，请从 FreeBSD 安装介质引导，并在第一个提示符处选择 `Shell`。然后加载镜像模块并挂载镜像设备：
...
要更换故障的驱动器，关闭系统并物理更换故障的驱动器，使用相同或更大容量的新驱动器。制造商在以千兆字节为单位评定驱动器容量时使用的值有些是任意的，唯一确保容量正确的方法是通过 `diskinfo -v` 比较总扇区数。容量大于镜像的驱动器可以使用，尽管新驱动器的额外空间不会被使用。
