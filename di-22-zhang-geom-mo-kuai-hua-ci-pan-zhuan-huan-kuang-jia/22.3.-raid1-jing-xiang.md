...
如果遇到问题，请参见 [故障排除](./22.3.-raid1-jing-xiang.md#id-22.3.4.-gu-zhang-pai-chu)。关闭电源并断开原始的 **ada0** 磁盘后，可以将其保留为离线备份。
...
重新启动系统，从 **ada1** 引导。如果一切正常，系统将从 **mirror/gm0** 引导，且现在包含与之前 **ada0** 相同的数据。若有问题引导，请参阅 [故障排除](./22.3.-raid1-jing-xiang.md#id-22.3.4.-gu-zhang-pai-chu)。
...
其他导致 `error 19` 的问题需要更多的努力来修复。尽管系统应从 **ada0** 引导，如果 **/etc/fstab** 不正确，仍会出现另一个提示符以选择一个 shell。在引导加载程序提示符下输入 `ufs:/dev/ada0s1a`，然后按 Enter 键。撤销 **/etc/fstab** 中的编辑，然后挂载原始磁盘 (**ada0**) 上的文件系统，而不是镜像。重新启动系统并再次尝试该过程。
