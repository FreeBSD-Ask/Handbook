...
*路由* 是让系统能找到通往另一个系统的网络路径的机制。*路由* 是一对定义的地址，分别表示“目标”和“网关”。路由表明，当尝试连接到指定的目标时，应该通过指定的网关发送数据包。有三种目标类型：单个主机、子网和“默认”。“默认路由”在没有其他路由适用时使用。网关也有三种类型：单个主机、接口（也叫做链路）和以太网硬件（MAC）地址。已知的路由会存储在路由表中。
...
对于作为外部世界网关的机器，默认路由将是互联网服务提供商（ISP）处的网关机器。
...
**子网**
FreeBSD 会自动为本地子网添加子网路由。在此示例中，`10.20.30.255` 是 `10.20.30` 子网的广播地址，`example.com` 是与该子网关联的域名。`link#1` 表示机器的第一个以太网卡。

本地网络主机和本地子网的路由由名为 [routed(8)](https://man.freebsd.org/cgi/man.cgi?query=routed&sektion=8&format=html) 的守护进程自动配置。如果它未运行，则只有管理员静态定义的路由存在。
...
可以在 `Flags` 列中查看每个路由的各种属性。[常见的路由表标志](./35.2.-wang-guan-he-lu-you.md) 总结了其中部分标志及其含义。
...
为让系统在接口之间转发数据包，FreeBSD 必须被配置为路由器。互联网标准和良好的工程实践不让 FreeBSD 项目默认启用此功能，但可以通过在 **/etc/rc.conf** 中添加以下行来配置在引导时启用此功能：
...
> **注意**
>
> 对于大型网络，静态路由很快变得不可扩展。FreeBSD 提供标准的 BSD 路由守护进程 [routed(8)](https://man.freebsd.org/cgi/man.cgi?query=routed&sektion=8&format=html)，它支持 RIP 协议（版本 1 和 2）和 IRDP 协议。BGP 和 OSPF 路由协议的支持可以通过软件包或 Port [net/quagga](https://cgit.freebsd.org/ports/tree/net/quagga/) 安装。

考虑以下网络配置：

![静态路由](https://docs.freebsd.org/images/books/handbook/advanced-networking/static-routes.png)

在此示例中，`RouterA` 是一台 FreeBSD 机器，充当通向外部互联网的路由器。它的默认路由设置为 **10.0.0.1**，让它能与外部世界连接。`RouterB` 已配置使用 **192.168.1.1** 作为其默认网关。
...
现在，`RouterA` 可以访问 `192.168.2.0/24` 网络上的任何主机。但是，如果 FreeBSD 系统重新引导，路由信息将不会保留。如果需要使静态路由持久化，可以将其添加到 **/etc/rc.conf** 中：
...
有一个系统跟踪所有分配的地址空间，并定义它们与互联网主干网（或承载跨越全国和世界各地的互联网流量的主干线路）连接的节点。每台主干机器都保存一份主路由表，该表将特定网络的流量指向一个特定的主干承运商，从那里通过一系列服务提供商直到到达特定网络。

服务提供商的任务是向主干站点通告他们是连接点，因此也是流量的进入路径。此过程称为路由传播。

有时，路由传播会出现问题，导致某些站点无法连接。查找路由中断位置最有用的命令或许是 `traceroute`，在 `ping` 失败时尤其有用。
...
FreeBSD 本身支持多播应用和多播路由。多播应用在 FreeBSD 上运行时无需特殊配置。要支持多播路由，必须将以下选项编译到定制内核中：

```sh
options MROUTING
```

多播路由守护进程 `mrouted` 可以通过软件包或 Port [net/mrouted](https://cgit.freebsd.org/ports/tree/net/mrouted/) 安装。该守护进程实现了 DVMRP 多播路由协议，并通过编辑 **/usr/local/etc/mrouted.conf** 来配置隧道和 DVMRP。安装 `mrouted` 时，还会安装 `map-mbone` 和 `mrinfo`，以及它们相关的 man 页面。请参考这些文档以获得配置示例。

> **注意**
>
> DVMRP 在许多多播安装中很大程度上已被 PIM 协议取代。有关更多信息，请参阅 [pim(4)](https://man.freebsd.org/cgi/man.cgi?query=pim&sektion=4&format=html)。
