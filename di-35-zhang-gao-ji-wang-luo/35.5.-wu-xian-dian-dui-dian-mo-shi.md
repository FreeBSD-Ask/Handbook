...
在将 FreeBSD 机器配置为接入点之前，必须在内核中配置适当的无线网卡网络支持和所使用的安全协议。有关详细信息，请参见 [基本设置](./34.4.-wu-xian-gao-ji-shen-fen-yan-zheng.md)。
...
本节重点介绍如何使用 WPA2 安全协议设置 FreeBSD 接入点。有关 WPA 和基于 WPA 的无线客户端配置的更多详细信息，请参阅 [WPA with EAP-TLS](./34.4.-wu-xian-gao-ji-shen-fen-yan-zheng.md)。
...
在尝试配置 [hostapd(8)](https://man.freebsd.org/cgi/man.cgi?query=hostapd&sektion=8&format=html) 之前，首先配置 [基本设置](./34.4.-wu-xian-gao-ji-shen-fen-yan-zheng.md) 中介绍的基本设置。
...
- ③ [hostapd(8)](https://man.freebsd.org/cgi/man.cgi?query=hostapd&sektion=8&format=html) 用于存储域套接字文件的目录路径，用于与外部程序（如 [hostapd_cli(8)](https://man.freebsd.org/cgi/man.cgi?query=hostapd_cli&sektion=8&format=html)）通信。此示例使用默认值。
- ④允许访问控制接口文件的组。
...
接入点启动后，客户端可以与其关联。有关更多详细信息，请参阅 [基本设置](./34.4.-wu-xian-gao-ji-shen-fen-yan-zheng.md)。可以使用 `ifconfig wlan0 list sta` 查看与接入点关联的站点。
