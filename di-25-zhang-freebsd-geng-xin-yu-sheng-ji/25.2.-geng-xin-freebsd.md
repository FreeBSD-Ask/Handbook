...
- FreeBSD 从 13.1 升级到 13.2

_主要_ 版本升级会增加主要版本号，例如：

- FreeBSD 从 12.4 升级到 13.2

这两种类型的升级都可以通过包含 `freebsd-update` 支持的 release 版本来完成。

> **警告**
>
> 在每个新版本发布之后， FreeBSD 软件包制作服务器将在一定时期内不使用新版本的操作系统。这为许多在发布后不立即升级的用户提供了连续性。比如说：
>
> - 使用 13.1 和 13.2 版本的用户软件包将在运行 13.1 版本的服务器上构建，直到 13.1 版本的支持结束。
>
> ——凡事都有两面性：
>
> - 一个构建在 13.1 版本上的内核模块可能不适合 13.2版本。
>
...
### 25.2.3.1. 在 FreeBSD 9.X 及以后版本的定制内核
