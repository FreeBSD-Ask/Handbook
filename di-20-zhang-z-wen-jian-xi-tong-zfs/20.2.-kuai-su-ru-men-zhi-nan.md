...
```
这样就完成 RAID-Z 的配置。通过在 **/etc/periodic.conf** 中添加这一行，在每晚的[periodic(8)](https://www.freebsd.org/cgi/man.cgi?query=periodic&sektion=8&format=html) 运行中添加关于已创建文件系统的每日状态更新。
```
