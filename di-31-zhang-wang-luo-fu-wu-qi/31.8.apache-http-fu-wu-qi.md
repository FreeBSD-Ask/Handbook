...
*   _**DocumentRoot "/usr/local/www/apache2\_x\_/data"**_
...
httpd服务可以通过在网页浏览器中输入 `http://localhost`，用运行 `httpd` 的机器的完全限定域名替换 `localhost` 来测试。显示的默认网页是 **/usr/local/www/apache24/data/index.html**。
...
可以使用软件包或 port [www/mod\_perl2](https://cgit.freebsd.org/ports/tree/www/mod\_perl2/pkg-descr) 安装 `mod_perl`。有关使用此模块的文档，请参见 [http://perl.apache.org/docs/2.0/index.html](http://perl.apache.org/docs/2.0/index.html)。
...
通过取消注释 /**usr/local/etc/apache24/httpd.conf** 中的行来启用 http2 模块，然后用 mpm\_event 替换 mpm\_prefork 模块，因为后者不支持 HTTP2，从而启动该过程。
...
> **注意**
>
> 有一个单独的 port  **mod\_http2** 可用。它的存在是为了提供比随 port **apache24** 捆绑安装的模块更快的安全性和错误修复。它不是 HTTP2 支持所必需的，但可用。安装后，应在 Apache 配置中使用 **mod\_h2.so** 代替 **mod\_http2.so**。
