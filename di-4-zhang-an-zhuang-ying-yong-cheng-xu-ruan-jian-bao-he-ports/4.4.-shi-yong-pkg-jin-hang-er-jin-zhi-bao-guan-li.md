...
然而，对于那些从源代码构建的站点，将需要一个单独的[端口管理工具](https://docs.freebsd.org/en/books/handbook/book/#ports-upgrading-tools)。

由于[pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html)仅适用于二进制软件包，它并不是这些工具的替代品。这些工具可用于从二进制软件包和 Ports 集合安装软件，而[pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html)仅安装二进制软件包。
...
`Quarterly` 分支为用户提供了更可预测和稳定的 Ports 和软件包安装和升级体验。这主要通过仅允许非功能更新来实现。季度分支旨在接收安全修复（可能是版本更新，或提交的后退），错误修复和符合或框架更改的端口。Quarterly 分支是每个（年度）季度在一月、四月、七月和十月初始时从 HEAD 切出的。分支的名称根据它们创建的年份（YYYY）和季度（Q1-4）进行命名。例如，2023 年 1 月创建的季度分支被命名为 2023Q1。而 `Latest` 分支为用户提供了软件包的最新版本。
...
然后运行此命令以更新 Latest 分支的本地软件包存储库目录：
...
|     | FreeBSD 不需要具有 `pkg.conf` 文件。许多安装将没有 `pkg.conf` 或只有空的 `pkg.conf`（除了注释行）运行良好。 |
| --- | ----------------------------------------------------------------------------------------------------------- |
...
`pkg prime-list` 是在 `/usr/local/etc/pkg.conf` 中声明的别名命令。还有许多其他可以用来查询系统的软件包数据库的命令。例如，命令 `pkg prime-origins` 可用于获取上述列表的原始端口目录：
