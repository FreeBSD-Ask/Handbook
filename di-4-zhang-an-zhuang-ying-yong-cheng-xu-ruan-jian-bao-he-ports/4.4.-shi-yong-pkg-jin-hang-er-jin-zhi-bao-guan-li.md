...
然而，对于从源代码构建软件的站点，需要一个单独的 [port 管理工具](https://docs.freebsd.org/en/books/handbook/ports/#ports-upgrading-tools)。
...
默认情况下，pkg 会将二进制软件包存储在由 [pkg.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=pkg.conf&sektion=5&format=html) 中的 `PKG_CACHEDIR` 定义的缓存目录中。只保留最新安装的软件包的副本。先前版本的 pkg 会保留所有先前的软件包。要移除这些过时的二进制软件包，可以运行：
