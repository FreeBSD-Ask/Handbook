...
`Quarterly` 分支为用户提供了更可预测和稳定的 port 和软件包安装及升级体验。这主要通过只允许非功能性更新来实现。季度分支旨在接收安全修复（这些可能是版本更新，或是提交的回溯）、错误修复和 ports 合规性或框架更改。季度分支在每年的一月、四月、七月和十月的开始从 HEAD 分支切出。分支名称根据它们创建的年份（YYYY）和季度（Q1-4）命名。例如，2023 年一月创建的季度分支命名为 2023Q1。`Latest` 分支则为用户提供最新版本的软件包。
