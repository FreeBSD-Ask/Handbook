...
一些 ports 包含 **pkg-message** 或其他文件来处理特殊情况。关于这些文件以及一般的 ports 的更多细节，请参考 [FreeBSD Porter 的手册](https://docs.freebsd.org/en/books/porters-handbook/)。

port 中并不包括实际的源代码，也就是所谓的 **distfile**。构建 port 的提取部分会自动将下载的源代码保存到 **/usr/ports/distfiles**。
...
由于 `lsof` 是一个需要提升权限才能运行的程序，所以在安装时将显示一个安全警告。安装完成后，将返回提示。
...
- 主干 ports：没有依赖关系，但其他 ports 依赖它。

- 分支 ports：有依赖关系，其他 ports 依赖于它。

- 子 ports：有依赖关系但没有其他 ports 依赖它。
...
>**注意**
>
>在默认情况下，Portmaster 在删除现有的 port 之前会做一个备份包。如果新版本的安装成功，Portmaster 将删除备份。使用 `-b` 可以指示 Portmaster 不自动删除备份。添加 `-i` 可以在交互式模式下启动 Portmaster，在升级每个 port 之前提示确认。还有许多其他选项可用。请阅读  [portmaster(8https://www.freebsd.org/cgi/man.cgi?query=portmaster&sektion=8&format=html)]() 的手册以了解它们的详细用法。
