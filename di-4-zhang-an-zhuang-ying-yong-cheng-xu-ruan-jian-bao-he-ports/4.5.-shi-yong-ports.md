...
ports 是一组 Makefiles，补丁和描述文件。这些文件集用于在 FreeBSD 上编译和安装单个应用程序，并被称为port。

默认情况下，Ports 本身存储为 /usr/ports 的子目录。

|  | 在安装和使用 Ports 之前，请注意，通常不建议使用 Ports 与通过 pkg 提供的二进制软件包一起安装软件。pkg 默认跟踪 ports 树的季度分支发布，而不是 HEAD。与季度分支发布中的 port 相比，HEAD 中的 port 依赖关系可能不同，这可能导致 pkg 安装的依赖关系与 Ports 中的依赖关系之间发生冲突。如果必须同时使用 Ports 和 pkg，则请确保你的 Ports 和 pkg 处于 ports 树的相同分支发布上。|
| -- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

Ports 包含软件类别的目录。在每个类别内部，有各个应用程序的子目录。每个应用程序子目录包含一组文件，告诉 FreeBSD 如何编译和安装该程序，称为 ports 骨架。每个 port 骨架包括这些文件和目录：
...
在使用port编译应用程序之前，必须先安装ports 。如果在安装 FreeBSD 期间没有安装它，请使用以下方法之一安装它：

** 过程：Git 方法**

如果需要对 ports 树有更多控制，或者需要维护本地更改，或者运行 FreeBSD-CURRENT，可以使用 Git 来获取 Ports 。有关 Git 的详细描述，请参阅 Git Primer。
...
本节提供有关使用ports 安装或删除软件的基本说明。可在ports(7)中获取可用 make 目标和环境变量的详细描述。

|  | 在编译任何port之前，请务必按照前一节中的描述更新ports 。由于安装任何第三方软件可能会引入安全漏洞，建议在安装新port之前先检查 https://vuxml.freebsd.org/ 以获取与port相关的已知安全问题。或者，在安装新port之前运行 pkg audit -F。可以配置此命令在每日安全系统检查期间自动执行安全审计和漏洞数据库更新。有关更多信息，请参阅 pkg-audit(8) 和 periodic(8)。|
| -- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

使用Ports Collection 假设你有工作的互联网连接。它还需要超级用户特权。
...
随着时间的推移，更高版本的软件会在 Ports 中变得可用。本节描述了如何确定应升级哪些软件以及如何执行升级。
