...
Ports 是一组 `Makefile`、补丁和说明文件。每一组文件用于在 FreeBSD 上编译并安装某个单独的应用程序，称为 *Port*。

默认情况下，Ports 本身存储在 **/usr/ports** 的子目录中。

> **警告**
>
> 在安装和使用 Ports 之前，请注意：通常不建议将 Ports 与通过 pkg 提供的二进制软件包混用。pkg 默认跟踪的是 Ports 树的季度分支（quarterly branch），而不是 HEAD。某个 Port 在 HEAD 中的依赖项，可能与其在季度分支中的不同，这可能会导致 pkg 安装的依赖项与 Ports 安装的依赖项之间产生冲突。如果必须同时使用 Ports 和 pkg，请确保它们使用的是同一分支的 Ports 树。

Ports 包含以软件类别划分的目录，每个类别目录中包含若干应用程序的子目录。每个应用程序的子目录包含一组告诉 FreeBSD 如何编译和安装该程序的文件，这组文件称为 *Ports 框架*。每份 Ports 框架包含以下文件和目录：
...
我们在 Git 命令中加入 `--depth 1` 参数，以便在克隆时不获取提交历史，这样能节省时间，对于大多数用户而言已经足够。如果你对 Ports 树有自己的修改，或出于其他原因需要保留历史记录，则可以省略下面命令中的 `--depth 1` 参数。
...
在安装过程中，系统会创建用于临时编译的子目录。删除该目录可以节省磁盘空间，并减少将来升级该 Port 时可能出现的问题：
...
在少数情况下，如组织有本地 distfiles 存储库，可使用 `MASTER_SITES` 变量来覆盖 `Makefile` 中指定的下载路径。使用时，请指定替代位置：
...
将在 **/usr/home/example/ports** 下编译，并将程序安装到 **/usr/local**。

```sh
# make PREFIX=/usr/home/example/local install
```

将在 **/usr/ports** 下编译，并将程序安装到 **/usr/home/example/local**。再如：
...
建议在卸载 Port 时阅读相关提示信息。如果该 Port 有其他依赖它的程序，将显示相关信息，但卸载过程仍将继续。在这种情况下，更好的做法可能是重新安装该程序，以避免依赖损坏。

## 4.5.4. 升级 Ports

随着时间推移，Ports 中的软件可能会有更新版本。本节说明如何判断哪些软件可以升级，以及如何执行升级操作。

要判断已安装的 Ports 是否有新版本可用，首先确保使用 [Git 方法](./4.5.-shi-yong-ports.md#id-4.5.1.-an-zhuang-ports) 中说明的命令更新 Ports 树。然后执行以下命令列出已过期的 Ports：
...
> **重要**
>
> 在尝试升级之前，请阅读 **/usr/ports/UPDATING** 文件，从文件顶部阅读到离上一次升级 Ports 或系统安装时间最近的日期。该文件说明了用户在升级 Port 时可能遇到的各种问题和所需的额外步骤，例如文件格式变化、配置文件路径变更，或与旧版本不兼容等问题。请记录与需升级的 Port 匹配的所有说明，并在执行升级时遵循这些指引。

### 4.5.4.1. 升级和管理 Ports 的工具

Ports 提供了若干实用工具用于执行实际升级操作。每种工具都有其优点和局限。

以前大多数系统使用的是 Portmaster 或 Portupgrade。Synth 是一款较新的可选工具。
...
[ports-mgmt/portmaster](https://cgit.freebsd.org/ports/tree/ports-mgmt/portmaster/) 是非常小巧的工具，用于升级已安装的 Ports。它的设计目标是仅使用 FreeBSD 基本系统中已有的工具，而不依赖其他 Ports 或数据库。要通过 Ports 安装该工具：
...
> **注意**
>
> 默认情况下，Portmaster 会在删除旧版本 Port 之前创建备份软件包。如果新版本安装成功，它会删除该备份。使用 `-b` 可以指示 Portmaster 不自动删除备份。加上 `-i` 可启用交互模式，升级每个 Port 前询问用户确认。Portmaster 提供了许多其他选项，详情请阅读 [portmaster(8)](https://man.freebsd.org/cgi/man.cgi?query=portmaster&sektion=8&format=html) 手册页。
...
Portmaster 也可以用于安装新的 Ports，会在构建和安装新 Port 之前先升级所有依赖项。要使用此功能，请指定 Port 在 Ports 中的位置：
...
加上 `-P` 后，Portupgrade 会在 `PKG_PATH` 中列出的本地目录中查找已有的软件包文件，若找不到，则尝试从远程站点获取。如果本地和远程都无法获取，则会使用 Ports 构建。若希望完全不使用 Ports，请使用 `-PP`，该选项在找不到软件包时会直接中止：

```sh
# portupgrade -PP gnome3
```

若只想获取 Port 的 distfiles 或包文件（若加了 `-P`），不构建或安装，可使用 `-F`。更多选项请参考 `portupgrade` 的手册页。
