...
Ports 是一组 `Makefile`、补丁和描述文件。每一组文件用于在 FreeBSD 上编译并安装某个单独的应用程序，被称为一个 *port*。
...
要判断已安装的 ports 是否有新版本可用，首先确保使用 [Git 方法](./4.1.-gai-shu.md) 中描述的命令更新 ports 树。然后执行以下命令列出已过期的 ports：
