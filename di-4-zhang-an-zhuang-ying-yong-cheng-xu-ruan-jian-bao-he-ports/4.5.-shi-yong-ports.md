...
> **警告**
>
> 在安装和使用 ports 之前，请注意通常不建议将 ports 与通过 pkg 提供的二进制包一起使用来安装软件。RELEASE 版本的 FreeBSD:[pkg.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=pkg.conf&sektion=5&format=html) 默认会跟踪季度分支，而非最新分支。与季度分支中对应的 port 相比（如果有的话），最新分支中的依赖关系可能有所不同，这可能导致 pkg 安装的依赖关系与 ports 中的依赖关系发生冲突。如果必须同时使用 ports 和 pkg，那么请确保你的 ports 和 pkg 在的同一个 ports 分支上。
