...
Ports 是一组 `Makefile`、补丁和说明文件。每一组文件用于在 FreeBSD 上编译并安装某个单独的应用程序，被称为一个 *Port*。
...
> **注意**
>
> 默认情况下，Portmaster 会在删除旧版本 Port 之前创建一个备份软件包。如果新版本安装成功，它会删除该备份。使用 `-b` 可以指示 Portmaster 不自动删除备份。加上 `-i` 可启用交互模式，升级每个 Port 前询问用户确认。Portmaster 提供了许多其他选项，详情请阅读 [portmaster(8)](https://man.freebsd.org/cgi/man.cgi?query=portmaster&sektion=8&format=html) 手册页。
