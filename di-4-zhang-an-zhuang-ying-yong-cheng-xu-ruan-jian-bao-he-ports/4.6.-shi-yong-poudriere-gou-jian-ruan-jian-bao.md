...
虽然在系统上运行 poudriere 不强制使用 ZFS，但使用 ZFS 是有好处的。当使用 ZFS 时，必须在 **/usr/local/etc/poudriere.conf** 中指定  `ZPOOL`，并且 `FREEBSD_HOST` 应该被设置为附近的镜像站。定义 `CCACHE_DIR` 可以使用 [devel/ccache](https://cgit.freebsd.org/ports/tree/devel/ccache/pkg-descr) 来缓存编译，减少经常编译的代码的构建时间。将 poudriere 数据放在一个安装在 **/poudriere** 的隔离树中可能会很方便。其他配置值的默认值是足够的。
...
## 4.6.2.配置 pkg 客户端使用 Poudriere 库
