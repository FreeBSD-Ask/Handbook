...
poudriere 可通过软件包或 Port [ports-mgmt/poudriere](https://cgit.freebsd.org/ports/tree/ports-mgmt/poudriere/) 安装。安装后会附带一个示例配置文件 **/usr/local/etc/poudriere.conf.sample**，可将其复制为 **/usr/local/etc/poudriere.conf** 并根据本地系统进行编辑。
...
要构建的软件包列表写入 `13amd64-local-workstation-pkglist` 文件（带有 [FLAVORS](https://docs.freebsd.org/en/books/porters-handbook/flavors) 的 Ports 使用 @FLAVOR 指定）指定）：
