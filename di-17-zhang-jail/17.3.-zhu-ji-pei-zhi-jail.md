...
- ⑦ `mount.devfs` 将 [devfs(5)](https://man.freebsd.org/cgi/man.cgi?query=devfs&sektion=4&format=html) 文件系统挂载到 jail 的 **/dev** 目录，并应用 `devfs_ruleset` 中定义的规则集来限制 jail 可见的设备。
