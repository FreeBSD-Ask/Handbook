...
要通过 OpenZFS 快照创建瘦 jail，第一步是创建 jail 目录树，按照[设置 Jail 目录树](./17.1.-gai-shu.md)中的说明进行操作。
...
可以使用 [传统 Jail（厚 Jail）](./17.1.-gai-shu.md) 或 [瘦 Jail](./17.1.-gai-shu.md) 的方法。唯一不同的是 **/etc/jail.conf** 文件中的配置。
...
接下来的步骤是按照上述方法创建 jail，例如在 [使用 OpenZFS 快照创建瘦 Jail](./17.1.-gai-shu.md) 中所示，但 **不进行** 配置。FreeBSD Linux jails 需要特定的配置，接下来将详细说明。
