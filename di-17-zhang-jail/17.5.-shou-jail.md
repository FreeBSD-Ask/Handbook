...
### 创建一个薄Jail，使用 OpenZFS 快照

由于 FreeBSD 和 OpenZFS 之间的良好集成，使用 OpenZFS 快照创建新的薄Jails非常容易。

要使用 OpenZFS 快照创建一个薄Jail，第一步是创建一个模板。
...
接下来的步骤是根据上面的指示创建一个jail，例如在使用 OpenZFS 快照创建一个薄的Jail时，但不执行配置。FreeBSD Linux jails需要一个将在下面详细说明的特定配置。
