...
将文件移动到模板中后，接下来的步骤是执行以下命令更新到最新的补丁级别：
...
接下来的步骤是执行以下命令创建指向 `skeleton` 的符号链接：
...
FreeBSD 可在 jail 内运行 Linux，利用 [Linux 二进制兼容性](../di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/12.1.-gai-shu.md) 和 [debootstrap(8)](https://man.freebsd.org/cgi/man.cgi?query=debootstrap&sektion=8&format=html)。jail 没有内核，它们运行在宿主机的内核上。因此，需要在宿主系统中启用 Linux 二进制兼容性。
...
接下来的步骤是按照上述方法创建 jail，例如在 [使用 OpenZFS 快照创建瘦 jail](./17.5.-shou-jail.md#id-17.5.1.-shi-yong-openzfs-kuai-zhao-chuang-jian-shou-jail) 中所示，但 **不做** 配置。FreeBSD Linux jails 需要特定的配置，接下来将详细说明。

创建空的 **/etc/jail.conf**，以防止在尝试停止 jail 时出现失败：

```sh
# touch /etc/jail.conf
```

按照上述说明创建 jail 后，执行以下命令完成所需配置并启动它：
...
服务 jail 完全通过 **/etc/rc.conf** 或 [sysrc(8)](https://man.freebsd.org/cgi/man.cgi?query=sysrc&sektion=8&format=html) 配置。基本系统服务支持服务 jail。它们包含一行配置，启用网络连接或解除其他 jail 限制。不适合在 jail 中运行的基本系统服务，即使在 **/etc/rc.conf** 中启用，也配置为不作为服务 jail 启动。例如，某些服务会在启动或停止方法中挂载或卸载某些内容，或者仅配置诸如路由、防火墙等内容。
