...
要通过 OpenZFS 快照创建瘦 jail，第一步是创建 jail 目录树，按照[设置 Jail 目录树](./17.4.-chuan-tong-jail-hou-jail.md)中的说明进行操作。
...
有关桥接的更多信息，请参阅 [Network Bridging](../di-34-zhang-gao-ji-wang-luo/34.8.-qiao-jie.md)。
...
接下来的步骤是按照上述方法创建 jail，例如在 [使用 OpenZFS 快照创建瘦 Jail](./17.5.-shou-jail.md) 中所示，但 **不进行** 配置。FreeBSD Linux jails 需要特定的配置，接下来将详细说明。
