...
jail 创建完成后，可以执行许多操作，如启动、重启或删除 jail、在其中安装软件等。本节将介绍可以从主机对 jail 执行的不同操作。
...
将 `start` 参数更改为 `restart` 或 `stop` 可以对 jail 执行其他操作。
...
例如，要在 jail 中安装 [www/nginx-lite](https://cgit.freebsd.org/ports/tree/www/nginx-lite/)，可以**从主机**执行以下命令：
