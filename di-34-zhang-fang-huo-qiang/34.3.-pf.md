...
自 FreeBSD 5.3 以来，OpenBSD 的 PF 防火墙的移植版本已作为基本系统的集成部分被包含在内。PF 是一个完整的、功能丰富的防火墙，它可选地支持 ALTQ（Alternate Queuing），提供服务质量（QoS）。
...
要控制 PF，可以使用 `pfctl`。有用的 `pfctl` 选项总结了该命令的一些常用选项。有关所有可用选项的说明，请参阅 [pfctl(8)](https://man.freebsd.org/cgi/man.cgi?query=pfctl&sektion=8&format=html)：
...
为了监控通过 PF 防火墙的流量，可以考虑软件包或 Port [sysutils/pftop](https://cgit.freebsd.org/ports/tree/sysutils/pftop/) 。安装后，可以运行 pftop 查看流量的实时快照，其格式类似于 [top(1)](https://man.freebsd.org/cgi/man.cgi?query=top&sektion=1&format=html)。
...
要通过 PF 和 [ftp-proxy(8)](https://man.freebsd.org/cgi/man.cgi?query=ftp-proxy&sektion=8&format=html) 运行受保护的 FTP 服务器，可以在单独的端口上以反向模式配置一个独立的 `ftp-proxy`，使用 `-R` 选项，并配合其自己的重定向通过规则。
...
PF 能基于所有变种的 ICMP 类型和代码进行过滤。可能的类型和代码列表在 [icmp(4)](https://man.freebsd.org/cgi/man.cgi?query=icmp&sektion=4&format=html) 和 [icmp6(4)](https://man.freebsd.org/cgi/man.cgi?query=icmp6&sektion=4&format=html) 中有文档说明。
...
> **不一定需要阻止所有超载者：**
>
> 超载机制是一种通用技术，不仅适用于 SSH，并且并非总是需要完全阻止所有违反者。
>
