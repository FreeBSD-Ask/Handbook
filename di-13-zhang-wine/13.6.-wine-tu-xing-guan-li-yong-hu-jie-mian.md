...
虽然 WINE 的基本安装包含了名为 `winecfg` 的图形配置工具，其主要作用是配置现有的 WINE 前缀。然而，还有一些更高级的应用程序，可以帮助完成应用程序的初始安装并优化它们的 WINE 环境。以下章节介绍一些最流行的工具。

## 13.6.1. Winetricks

`winetricks` 是跨平台的、通用的 WINE 辅助工具。它并非由 WINE 项目本身开发，而是由一群贡献者在 [GitHub](https://github.com/Winetricks/winetricks) 上维护。它包含一些自动化的“食谱”，帮助让常见的应用程序在 WINE 上运行，这些食谱不仅可以优化设置，还能自动获取一些 DLL 库。

### 13.6.1.1. 安装 winetricks

要在 FreeBSD 上使用二进制包安装 `winetricks`，可使用以下命令（注意：`winetricks` 需要安装 i386-wine 或 i386-wine-devel 包，因此不会与其他依赖项一起自动安装）：
...
注意：这应该在 32 位的前缀中运行 `winetricks`。启动 `winetricks` 后，会显示包含多个选项的窗口，如下所示：

![winetricks run 1](https://docs.freebsd.org/images/books/handbook/wine/winetricks-run-1.png)

选择 *Install an application*、*Install a benchmark* 或 *Install a game* 会显示支持的选项列表，例如下图所示的应用程序列表：
...
将显示 Windows® 风格的对话框，列出已安装的程序和组件。选择要删除的应用程序，然后点击 *Modify/Remove* 按钮。
...
[Mizutamari](https://codeberg.org/Alexander88207/Mizutamari) 是类似于 `winetricks` 的应用程序，灵感来源于 [Lutris](https://lutris.net/) 这个 Linux 上的游戏系统。尽管它主要聚焦于游戏，但也可通过 Mizutamari 安装非游戏类的应用程序。
...
Mizutamari 也可在 FreeBSD Ports 系统中找到。不过，它不在 *emulators* 部分或二进制包中，而是在 *games* 部分。
...
Mizutamari 的使用方式与 `winetricks` 相似。在首次使用时，可通过命令行（或桌面环境的运行小程序）启动它：

```sh
% Mizuma
```

这将显示友好的欢迎消息，点击 *OK* 继续。
...
根据 FreeBSD 机器的设置，Mizutamari 可能会显示提示，建议安装本地图形驱动程序。
...
要选择安装 Mizutamari 支持的某个应用程序，请选择 *Installation*，然后点击 *OK*。这将显示 Mizutamari 可以自动安装的应用程序列表。选择应用程序，并点击 *OK* 开始安装过程。
...
该程序还会为应用程序创建新的前缀，并显示标准的 WINE 对话框，提示正在创建前缀。
...
然而，明智之举是确保它们不会访问相同的 WINE 前缀。每个工具都会根据已知的 WINE 问题应用补丁，并更改注册表，以确保某个应用程序顺利运行。如果让 `winetricks` 和 Mizutamari 访问相同的前缀，可能会导致这些更改被覆盖，结果是某些或所有应用程序无法按预期工作。
