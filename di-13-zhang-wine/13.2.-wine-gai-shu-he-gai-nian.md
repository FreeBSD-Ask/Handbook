...
* **虚拟机**：如本章前面提到的，虚拟机（VM）是模拟完整硬件集的软件进程，可以安装并运行其他操作系统（包括 Windows®）。现代工具使得创建和管理虚拟机变得简单，但这种方法有其代价。虚拟机需要占用大量宿主系统的资源，而这些资源在虚拟机运行时无法被宿主系统回收。一些虚拟机管理器的例子包括开源的 qemu、bhyve 和 VirtualBox。有关更多详细信息，请参见关于 [虚拟化](../di-24-zhang-xu-ni-hua/24.1.-gai-shu.md) 的章节。
