...
1. 首先，在系统上设置一个共享位置以存储文件，例如 **/mnt/windows-drive_d/**。创建新目录的步骤可以参考 [mkdir(1)](https://man.freebsd.org/cgi/man.cgi?query=mkdir&sektion=1&format=html) 手册页。
2. 接着，为这个新目录设置权限，只让希望访问该目录的用户能访问。一种方法是创建一个名为 "windows" 的新组，并将所需的用户添加到该组中（请参见 [Users and Basic Account Management](../di-3-zhang-freebsd-ji-chu/3.3.-yong-hu-he-ji-ben-zhang-hu-guan-li.md) 章节的关于组的部分），然后将目录的权限设置为 `770`（[Permissions](../di-3-zhang-freebsd-ji-chu/3.4.-quan-xian.md) 章节介绍了该过程）。
...
另一方面，如果需要共享许多应用程序，或者这些应用程序需要特定的调整才能正常工作，可能需要采用不同的方法。在这种方法中，专门为存储 WINE 前缀和所有已安装的应用程序创建一个完全独立的用户。然后，单独的用户将被授权使用 [sudo(8)](https://man.freebsd.org/cgi/man.cgi?query=sudo&sektion=8&format=html) 命令以该用户身份运行程序。结果是，这些用户可以像平常一样启动 WINE 应用程序，只不过它会像由新创建的用户启动一样，因此使用的是包含设置和程序的集中维护的前缀。为此，可以执行以下步骤：
...
这些更改的结果是，*User_Alias* 部分列出的用户可以使用 *Cmnd Alias* 部分中列出的程序，并使用 *Defaults* 部分中列出的资源（当前显示器），以文件最后一行中列出的用户身份运行它们，也就是说，指定为 *WINDOWS_USERS* 的用户可以像 *windows* 用户一样运行 WINE 和 `winecfg` 应用程序。作为附加好处，此配置意味着它们无需输入 *windows* 用户的密码。
...
此命令应当添加到登录时或默认图形环境启动时运行的命令列表中。完成上述所有步骤后，在 **sudoers** 中配置为 `WINDOWS_USERS` 的用户可以使用以下命令运行程序，使用共享前缀：
