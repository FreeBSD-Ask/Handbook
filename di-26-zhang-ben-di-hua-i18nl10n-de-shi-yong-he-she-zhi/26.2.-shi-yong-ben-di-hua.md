...
*语言代码* 和 *国家代码* 用于确定国家和特定的语言变体。[常见语言和国家代码](./25.2.-shi-yong-ben-di-hua.md) 提供了一些 *语言代码_国家代码* 的示例：
...
有两种方法可以进行所需的变量分配：推荐的方法是通过 [登录分级](./25.2.-shi-yong-ben-di-hua.md)，另一种是通过 [启动文件](./25.2.-shi-yong-ben-di-hua.md) 方法。接下来的两个小节将演示如何使用这两种方法。
...
接下来，设置正确的控制台终端类型，在 **/etc/ttys** 中为所有虚拟终端条目配置。以下是 [各字符集已定义的终端类型](./25.2.-shi-yong-ben-di-hua.md) 的总结：
...
对于使用宽字符或多字节字符的语言，从 FreeBSD Ports 安装相应的控制台。可用的 Port 总结在 [来自 Ports 的可用控制台](./25.2.-shi-yong-ben-di-hua.md) 中。安装完成后，查看 Port 的 **pkg-message** 或 man 页面以获取配置和使用说明。
...
X 输入法（XIM）协议是 Xorg 标准，用于输入非英文字符。[可用输入法](./25.2.-shi-yong-ben-di-hua.md) 总结了 FreeBSD Ports 中可用的输入法应用程序。额外的 Fcitx 和 Uim 应用程序也可供使用。
