...
这通常由 **/etc/rc.d/linux** 脚本处理，但可以通过执行以下命令在启动时禁用此功能：
...
FreeBSD 内核使用多种方式来判断一个待执行的可执行文件是否为 Linux 程序：它会检查 ELF 文件头中的品牌标记，查找已知的 ELF 解释器路径，并检查 ELF 注记；最后，默认情况下，如果一个 ELF 可执行文件没有品牌标记，也会被认为是 Linux 程序。
