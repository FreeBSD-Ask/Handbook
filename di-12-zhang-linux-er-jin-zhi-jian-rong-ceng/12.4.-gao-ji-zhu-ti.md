...
这通常由 **/etc/rc.d/linux** 脚本处理，但可通过执行以下命令在启动时禁用此功能：

```sh
sysrc linux_mounts_enable="NO"
```

由 rc 脚本挂载的文件系统对于 chroot 或 jail 中的 Linux 进程无效；如有需要，请在 **/etc/fstab** 中配置：
...
由于 Linux 二进制兼容层已经支持运行 32 位与 64 位 Linux 二进制文件，因此无法再将仿真功能静态链接进定制内核中。
...
如果某个 Linux 共享库已经存在，并且其主版本号与 `ldd` 输出中第一列相匹配，则无需复制输出中最后一列所指向的文件，因为已有的库通常可以正常工作。但如果复制的是更新版本的库，建议仍替换。只要符号链接指向新版本，就可以删除旧版本。
...
由于已有的库版本只是在末尾小数字位相差一两个版本，该程序应仍然可以使用这个稍旧的版本。但使用更新版本替换已有的 **libc.so** 是安全的：
...
## 12.4.2. 为 Linux ELF 二进制文件添加品牌标记

FreeBSD 内核使用多种方式判断待执行的可执行文件是否为 Linux 程序：它会检查 ELF 文件头中的品牌标记，查找已知的 ELF 解释器路径，并检查 ELF 注记；最后，默认情况下，如果 ELF 可执行文件没有品牌标记，也会被认为是 Linux 程序。
...
为了帮助 FreeBSD 内核区分 FreeBSD ELF 可执行文件与 Linux 可执行文件，可使用 [brandelf(1)](https://man.freebsd.org/cgi/man.cgi?query=brandelf&sektion=1&format=html) 命令：
...
要安装基于 Linux 的 RPM 应用程序，首先安装软件包或 Port [archivers/rpm4](https://cgit.freebsd.org/ports/tree/archivers/rpm4/)。待安装完成，`root` 用户可使用以下命令安装 **.rpm** 文件：
...
关于 Linux® 二进制兼容的工作机制，更多信息可参见文章 [Linux emulation in FreeBSD](https://docs.freebsd.org/en/articles/linux-emulation/)。
