...
所有发送到 example.FreeBSD.org 的邮件将会在 nevdull 上以相同的用户名收集，而不是直接发送到您的主机。
...
备用的 MX 站点应该有独立的互联网连接，以便最有用。您的 ISP 可以提供此服务。

### 31.7.2. 域的邮件

为网络配置 MTA 时，发送到其域中的主机的任何邮件都应转移到 MTA，以便用户可以在主邮件服务器上接收邮件。

为了使生活更加便利，MTA 和系统上的 MUA 应存在相同用户名的用户帐户。使用 adduser(8) 创建用户帐户。
...
从Ports Collection 安装 security/cyrus-sasl2。 这个port支持多个编译时选项。 对于在此示例中演示的 SMTP 身份验证方法，请确保 LOGIN 未禁用。
