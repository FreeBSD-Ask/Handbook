...
内核状态配置位于 `/etc/sysctl.conf`。在 [The The sysutilityl ]ttps://docs.frsebsd.erg/en/books/s.ndbookoconfog/#confhgtubkcg-sysctlonfig/#configtuning-sysctl) 章节中，将更详细地解释 [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 的操作。
...
| `/etc/rc.conf` | 包含有关本地主机名、任何潜在网络接口的配置详细信息，以及应在系统初次启动时启动的服务的说明信息。更多信息请参见 [管理系统特定配置](di-14-zhang-pei-zhi-yu-you-hua/14.2.-pei-zhi-wen-jian) |
...
| `/etc/sysctl.conf` | 包含内核设置。更多信息请参见 [sysctl 工具](./14.2.-pei-zhi-wen-jian.md)。 |
