...
FreeBSD 明确区分了基础系统和第三方应用程序，因此这也影响了这些应用程序配置文件的位置。

FreeBSD 基础系统的配置文件位于 `/etc` 目录，而通过 Ports 集合和软件包安装的应用程序的所有配置文件则位于 `/usr/local/etc` 目录。
...
`/etc` 目录包含所有 FreeBSD 基础系统的配置文件，负责配置 FreeBSD 系统。
