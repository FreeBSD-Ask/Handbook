...
内核状态配置位于 **/etc/sysctl.conf**。在 [sysctl 工具](./14.2.-pei-zhi-wen-jian.md#id-14.2.2.-sysctl-gong-ju) 章节中，将更详细地解释 [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 的操作。

有关 FreeBSD 文件系统结构的更多信息，请参阅 [hier(7)](https://man.freebsd.org/cgi/man.cgi?query=hier&sektion=7&format=html)。

通常，配置文件在语法上并无统一标准。尽管 `#` 字符通常用于注释行，并且每行通常包含一个配置变量。
...
> **当心**
>
> 在修改 **/etc** 目录中的文件时必须**非常小心**；配置错误可能导致 FreeBSD 无法引导或发生故障。

| 目录 | 说明 |
| ---- | ---- |
| **/etc** | 系统配置文件和脚本。 |
| **/etc/defaults** | 默认的系统配置文件，更多信息请参见 [rc(8)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=8&format=html)。 |
| **/etc/fstab** | [fstab(5)](https://man.freebsd.org/cgi/man.cgi?query=fstab&sektion=5&format=html) 包含有关各种文件系统的说明信息。 |
| **/etc/mail** | 额外的 [sendmail(8)](https://man.freebsd.org/cgi/man.cgi?query=sendmail&sektion=8&format=html) 配置和其他 MTA 配置文件。 |
| **/etc/mtree** | mtree 配置文件，更多信息请参见 [mtree(8)](https://man.freebsd.org/cgi/man.cgi?query=mtree&sektion=8&format=html)。 |
...
| **/etc/rc.conf** | 包含有关本地主机名、任何潜在网络接口的配置详细信息，以及应在系统初次引导时启动的服务的说明信息。更多信息请参见 [管理系统特定的配置](./14.2.-pei-zhi-wen-jian.md#id-14.2.4.-guan-li-xi-tong-te-ding-de-pei-zhi) |
...
| **/etc/sysctl.conf** | 包含内核设置。更多信息请参见 [sysctl 工具](./14.2.-pei-zhi-wen-jian.md#id-14.2.2.-sysctl-gong-ju)。 |

## 14.2.2. sysctl 工具

[sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 工具用于修改正在运行的 FreeBSD 系统。

[sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 工具检索内核状态，并让具有适当权限的进程能设置内核状态。要检索或设置的状态使用“管理信息库”（“MIB”）样式的名称，由一组以点分隔的组件构成。
...
[sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 的核心功能是读取和修改系统设置。
...
管理信息库（MIB）是分层的，因此指定前缀会列出它下面的所有节点：
...
例如，要关闭致命信号退出的日志记录，并防止用户查看其他用户启动的进程，可在 **/etc/sysctl.conf** 中设置以下参数：
...
要获取有关特定 sysctl 功能的更多信息，可执行以下命令：
...
该文件包含广泛的配置信息，并在系统引导时读取以配置系统。它为 **rc\*** 文件提供配置信息。

**/etc/rc.conf** 中的条目将覆盖 **/etc/defaults/rc.conf** 中的默认设置。

> **技巧**
>
> 不应编辑 **/etc/defaults/rc.conf** 文件中包含的默认设置。所有系统特定的更改都应写入 **/etc/rc.conf**。

在集群应用程序中，可应用多种策略，将全站配置与特定系统配置分开，以减少管理开销。
...
> **技巧**
>
> **/etc/rc.conf** 和 **/etc/rc.conf.local** 都由 [sh(1)](https://man.freebsd.org/cgi/man.cgi?query=sh&sektion=1&format=html) 解析，系统操作员可借此创建复杂的配置场景。有关此主题的更多信息，请参见 [rc.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&format=html)。
