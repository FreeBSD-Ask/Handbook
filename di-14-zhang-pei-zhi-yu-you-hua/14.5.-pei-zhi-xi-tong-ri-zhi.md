...
本节介绍如何配置 FreeBSD 系统日志记录器以实现本地和远程日志记录，以及如何进行日志轮替和日志管理。

## 14.5.1. 配置本地日志记录

配置文件 **/etc/syslog.conf** 控制 syslogd 在接收到日志条目时的处理方式。此文件有多个参数来控制如何处理传入的事件。*facility* 说明了哪个子系统生成了消息，例如内核或守护进程，*level* 说明了事件发生的严重性。这使得可以根据日志消息的 *facility* 和 *level* 来配置日志消息的记录位置。还可以根据发送消息的应用程序进行操作，并且在远程日志记录的情况下，可以根据生成日志事件的机器的主机名进行操作。
...
第一个条目在系统启动时启动 syslogd。第二个条目让来自指定客户端的日志条目能被接收。`-v -v` 增加了日志消息的详细程度，这对于调整设施非常有用，管理员可以查看每个设施下记录了哪些类型的消息。

可以指定多个 `-a` 选项，以接收来自多个客户端的日志记录。还可以指定 IP 地址和整个网段。有关完整的选项列表，请参考 [syslogd(8)](https://man.freebsd.org/cgi/man.cgi?query=syslogd&sektion=8&format=html)。
...
本地安全也是一个问题。日志文件在使用过程中或日志轮替后并未加密。本地用户可能访问日志文件以获取有关系统配置的额外信息。因此，对日志文件设置适当的权限至关重要。内置的日志轮替工具 newsyslog 支持对新创建的和已轮替的日志文件设置权限。将日志文件设置为模式 `600` 应该可以防止本地用户的未经授权访问。有关更多信息，请参阅 [newsyslog.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=newsyslog.conf&sektion=5&format=html)。
