...
`<em>CMD RULE_NUMBER set SET_NUMBER ACTION log LOG_AMOUNT PROTO from SRC SRC_PORT to DST DST_PORT OPTIONS</em>`
...
* **SET\_NUMBER**
  每条规则都与一个从 `0` 到 `31` 的集合编号相关联。可以单独禁用或启用集合，因此可以快速添加或删除一组规则。如果未指定 `SET_NUMBER`，则规则将被添加到集合 `0`。
...
要配置端口或地址重定向，可以使用类似于内核态 NAT 的语法。尽管现在，在规则集脚本中指定配置不像内核态 NAT 那样进行，而是最好的做法是在配置文件中配置 [natd(8)](https://man.freebsd.org/cgi/man.cgi?query=natd&sektion=8&format=html)。为此，必须通过 **/etc/rc.conf** 传递一个额外的标志，指定配置文件的路径。
