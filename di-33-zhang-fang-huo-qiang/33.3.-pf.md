...
|命令|作用||-------------------------------------|------------------------------------------||`pfctl -e`|启用 PF。||`pfctl -d`|禁用 PF。||`pfctl -F all -f /etc/pf.conf`|清除所有 NAT、过滤、状态和表规则，并重新加载 **/etc/pf.conf**。||`pfctl -s [ rules \|nat \|states ]`|报告过滤规则、NAT 规则或状态表。||`pfctl -vnf /etc/pf.conf`|检查 **/etc/pf.conf** 中的错误，但不加载规则集。|
>**技巧**
>
