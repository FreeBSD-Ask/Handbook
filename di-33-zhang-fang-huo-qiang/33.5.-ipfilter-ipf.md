...
IPF 已包含在 FreeBSD 基本安装中，作为一个内核可加载模块，这意味着不需要自定义内核即可启用 IPF。

对于那些希望将 IPF 支持静态编译到自定义内核中的用户，请参阅 [Configuring the FreeBSD Kernel](../di-10-zhang-pei-zhi-freebsd-nei-he/10.1.-gai-shu.md) 中的说明。以下是可用的内核选项：
...
IPF 提供了 `ipmon`，可用于以人类可读的格式记录防火墙的日志信息。它要求在自定义内核中首先添加 `options IPFILTER_LOG`，按照 [配置 FreeBSD 内核](../di-10-zhang-pei-zhi-freebsd-nei-he/10.1.-gai-shu.md) 中的说明进行操作。
