...
打印文件到后台的过程称为 *排队打印*（spooling）。排队打印能让用户继续使用计算机上的其他程序，而无需等待打印机缓慢完成打印任务。

FreeBSD 包含排队打印程序，叫做 [lpd(8)](https://man.freebsd.org/cgi/man.cgi?query=lpd&sektion=8&format=html)。打印任务是通过 [lpr(1)](https://man.freebsd.org/cgi/man.cgi?query=lpr&sektion=1&format=html) 提交的。

## 11.5.1. 初始设置

创建存储打印任务的目录，设置目录的所有权，并设置权限以防止其他用户查看这些文件的内容：
...
到目前为止的例子直接将文本文件的内容发送到打印机。只要打印机能理解这些文件的内容，输出就会正确地打印出来。

一些打印机无法打印纯文本，而且输入文件可能甚至不是纯文本。

*过滤器* 能转换或处理文件。典型的用途是将一种类型的输入（如纯文本）转换成打印机可以理解的形式，如 PostScript® 或 `PCL`。过滤器还可用于提供附加功能，比如添加页码或高亮显示源代码以使其更易于阅读。
...
过滤器在 **/etc/printcap** 中通过 `if=` 标识符指定。要使用 **/usr/local/libexec/lf2crlf** 作为过滤器，可以像这样修改 **/etc/printcap**：
...
> **技巧**
>
> **printcap** 条目中行尾的反斜杠 *续行* 字符表明，打印机的条目实际上只是一条长行，各条目之间以冒号分隔。前面的示例可以改写为一条不太易读的单行：
>
> ```sh
...
发送到该脚本的 PostScript® 输入在发送到打印机之前会渲染并转换为 `PCL`。
...
能检测输入类型并自动将其转换为适合打印机的正确格式的过滤器非常方便。PostScript® 文件的前两个字符通常是 `%!`。过滤器可以检测这两个字符。PostScript® 文件可以直接发送到 PostScript® 打印机而不做任何更改。文本文件可以使用 Enscript 转换为 PostScript®，如前所示。创建 **/usr/local/libexec/psif** 文件，并写入以下内容：
...
**/etc/printcap** 中的条目实际上是 *队列* 的定义。一台打印机可以有多个队列。结合过滤器使用时，多个队列可以为用户提供更多的打印作业控制。

例如，考虑办公室中的网络化 PostScript® 激光打印机。大多数用户希望打印纯文本，但一些专业用户希望能直接打印 PostScript® 文件。可以为同一台打印机在 **/etc/printcap** 中创建两个条目：
...
发送到 `textprinter` 的文档将由之前示例中的 **/usr/local/libexec/enscript** 过滤器格式化。专业用户可以在 `psprinter` 上打印 PostScript® 文件，那里不做任何过滤。

这种多个队列技术可用于提供对打印机所有功能的直接访问。例如，一台配有双面打印器的打印机可以使用两个队列，一个用于普通的单面打印，另一个使用过滤器，该过滤器发送命令序列以启用双面打印，然后发送传入的文件。

## 11.5.5. 监控与控制打印

有几种工具可用于监控打印作业、检查和控制打印机操作。
...
企业和学校中的多个用户通常共享打印机。为使共享打印机更加便利，提供了附加功能。
...
对于直接连接的打印机，通过从 **/etc/printcap** 文件中的条目中删除 `:sh:\`（禁止页头）行来启用页头页。这些页头页仅使用换行符换行。一些打印机可能需要 **/usr/share/examples/printing/hpif** 过滤器，以防止阶梯式文本。该过滤器配置 `PCL` 打印机，在接收到换行符时打印回车符和换行符。

对于网络打印机，页头页必须在打印机本身上配置。**/etc/printcap** 文件中的页头页条目将被忽略。设置通常可通过打印机前面板或通过浏览器访问的配置网页调整。
