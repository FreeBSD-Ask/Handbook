...
*过滤器* 能对文件进行转换或处理。典型的用途是将一种类型的输入（如纯文本）转换成打印机可以理解的形式，如 PostScript® 或 `PCL`。过滤器还可以用于提供附加功能，比如添加页码或高亮显示源代码以使其更易于阅读。
...
例如，考虑一个办公室中的网络化 PostScript® 激光打印机。大多数用户希望打印纯文本，但一些专业用户希望能够直接打印 PostScript® 文件。可以为同一台打印机在 **/etc/printcap** 中创建两个条目：
...
发送到 `textprinter` 的文档将由之前示例中的 **/usr/local/libexec/enscript** 过滤器进行格式化。专业用户可以在 `psprinter` 上打印 PostScript® 文件，那里不会进行任何过滤。
