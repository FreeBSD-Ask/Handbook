...
ZFS 提供透明压缩。在块级别压缩写入的数据可以节省空间，并且还提高了磁盘吞吐量。如果数据压缩率为 25%，压缩后的数据写入磁盘的速度与未压缩版本相同，结果是有效写入速度是未压缩时的 125%。压缩也可以是 [去重](https://docs.freebsd.org/en/books/handbook/zfs/#zfs-zfs-deduplication) 的一个很好的替代方案，因为它不需要额外的内存。
