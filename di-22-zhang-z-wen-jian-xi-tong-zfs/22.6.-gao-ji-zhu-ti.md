...
* `vfs.zfs.resilver_delay` — 在重新同步过程中，每次 I/O 操作之间的延迟时间（以 tick 为单位）。该设置限制重新同步产生的 IOPS 上限，有助于确保重新同步操作不会干扰系统上运行的其他工作负载。设置方式与 `vfs.zfs.scrub_delay` 类似。默认值为 `2`。
...
* `vfs.zfs.txg.timeout` — 事务组（transaction group）超时时间（以秒为单位）。事务组用于协调数据写入磁盘的过程。增加此值可能会减少磁盘 I/O，但会占用更多内存。默认值为 `5`。可以随时通过 [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 调整此值。
