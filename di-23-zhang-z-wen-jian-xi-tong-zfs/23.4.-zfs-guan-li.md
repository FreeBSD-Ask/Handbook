...
两个数据集属性控制 ACL 与传统权限处理的交互方式。`aclmode` 属性（`discard`、`groupmask`、`passthrough` 或 `restricted`）控制 **chmod(1)** 更改文件权限位时 ACL 的处理方式。默认值 `discard` 会让 chmod 移除所有不表示新文件模式的 ACL 条目。`aclinherit` 属性（`discard`、`noallow`、`restricted`、`passthrough` 或 `passthrough-x`）控制新文件和目录从父目录继承哪些 ACL 条目。默认值 `restricted` 会让继承的条目失去 `write_acl` 和 `write_owner`