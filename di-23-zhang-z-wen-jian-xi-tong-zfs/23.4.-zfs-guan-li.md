...
[保留空间](./23.8.-zfs-te-xing-he-shu-yu.md) 保证数据集始终有可用空间。保留的空间将不会分配给任何其他数据集。这个有用的功能确保了重要的数据集或日志文件有足够的空间。
...
对于设置 [引用保留空间](./23.8.-zfs-te-xing-he-shu-yu.md) 的 `refreservation` 属性，适用相同的原则，格式为 `refreservation=size`。
