...
[快照](./23.10.-zfs-te-xing-he-shu-yu.md) 是 ZFS 最强大的功能之一。快照提供了数据集的只读、时间点副本。通过写时复制（COW）技术，ZFS 在磁盘上保留旧版本的数据，从而快速创建快照。如果没有快照，ZFS 会在数据被重写或删除时回收空间，为将来使用腾出空间。快照通过记录当前数据集与之前版本之间的差异来节省磁盘空间。快照适用于整个数据集，而不是单独的文件或目录。来自数据集的快照会复制其中包含的所有内容，