...
[书签](./23.10.-zfs-te-xing-he-shu-yu.md) 记录快照创建时的时间点，但不保留任何快照数据。无论数据集之后如何变化，书签几乎不占用空间。与快照一样，书签可以作为增量 `zfs send` 的源。与快照不同的是，书签不会阻止 ZFS 释放旧数据。这种组合使书签适用于增量复制：在将快照发送到另一个