...
创建一个 ZFS 存储池需要做出永久性的决策，因为池的结构在创建后无法更改。最重要的决策是将物理磁盘分组为哪种类型的 vdev。有关可能的选项，请参阅 [vdev 类型](./23.8.-zfs-te-xing-he-shu-yu.md) 列表。创建池后，大多数 vdev 类型不能向 vdev 添加磁盘。例外的是镜像（mirrors），它能向 vdev 添加新磁盘；条带（stripes）也可以通过将新磁盘附加到 vdev 来升级为镜像。尽管添加新的 vdev 会扩展池，但池的布局在创建后无法更改。相反，你需要备份数据，销毁池并重新创建。
...
池也可以使用分区，而不是整个磁盘。将 ZFS 放入单独的分区可以使同一磁盘具有其他分区，用于其他目的。特别是，它能添加具有引导代码和引导所需文件系统的分区。这使得可以从同时作为池成员的磁盘引导。使用分区而不是整个磁盘不会对 FreeBSD 性能造成额外的负担。使用分区还能让管理员进行 *低配* 磁盘操作，即使用比整个磁盘容量少的空间。如果未来替换的磁盘与原始磁盘的名义大小相同，但实际容量略小，则较小的分区仍然可以适应替换磁盘。

使用分区创建一个 [RAID-Z2](./23.8.-zfs-te-xing-he-shu-yu.md) 池：
...
使用单个磁盘创建的池没有冗余。它可以检测到损坏，但无法修复，因为没有数据的其他副本。[复制](./23.8.-zfs-te-xing-he-shu-yu.md) 属性可能能够从小的故障（例如坏道）中恢复，但它不像镜像或 RAID-Z 那样提供相同级别的保护。从一个由单个磁盘 vdev 组成的池开始，使用 `zpool attach` 将新磁盘添加到 vdev，创建一个镜像。也可以使用 `zpool attach` 将新磁盘添加到镜像组中，从而增加冗余和读取性能。在对用于池的磁盘进行分区时，将第一个磁盘的布局复制到第二个磁盘。使用 `gpart backup` 和 `gpart restore` 可以简化这一过程。
