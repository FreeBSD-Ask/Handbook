...
使用分区创建一个 [RAID-Z2](./22.8.-zfs-te-xing-he-shu-yu.md) 池：
...
池的状态很重要。如果某个硬盘脱机或 ZFS 检测到读取、写入或校验和错误，相应的错误计数将增加。`status` 输出显示了池中每个设备的配置和状态，以及整个池的状态。还会显示对上一次 [`scrub`](./22.8.-zfs-te-xing-he-shu-yu.md) 的操作详情。
...
如果 vdev 没有冗余，或者设备出现故障且冗余不足以弥补，池会进入 [faulted](./22.8.-zfs-te-xing-he-shu-yu.md) 状态。除非有足够的设备重新连接池，否则池将无法使用，必须从备份中恢复数据。
