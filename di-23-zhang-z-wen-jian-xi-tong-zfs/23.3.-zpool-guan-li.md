...
建议定期对池进行 [scrub](./23.8.-zfs-te-xing-he-shu-yu.md)，最好每月至少一次。`scrub` 操作会消耗磁盘资源，因此在运行时会降低性能。请避免在高负载时期安排 `scrub` 操作，或者使用 [`vfs.zfs.scrub_delay`](./23.6.-gao-ji-zhu-ti.md) 调整 `scrub` 的相对优先级，以免影响其他工作负载。
