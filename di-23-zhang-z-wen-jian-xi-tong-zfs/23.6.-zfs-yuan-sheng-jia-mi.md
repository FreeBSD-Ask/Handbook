...
`zfskeys` [rc(8)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=8&format=html) 脚本在引导时为 `keylocation` 属性指向 `file://` URL 密钥文件的加密数据集加载加密密钥。该脚本在池导入之后、文件系统挂载之前运行，因此以这种方式处理的数据集会在正常的 ZFS 启动过程中自动挂载。该脚本不处理 `keylocation=prompt` 的数据集；系统引导后需用 `