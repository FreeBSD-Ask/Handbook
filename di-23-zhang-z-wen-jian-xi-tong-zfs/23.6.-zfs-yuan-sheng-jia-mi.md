...
除了[通过 SSH 发送加密备份](./23.4.-zfs-guan-li.md#id-23.4.10.4.-tong-guo-ssh-fa-song-jia-mi-bei-fen)外，在 `zfs send` 中使用 `-w`（raw）选项可将加密数据块传输到目标池。原始发送具有以下优势：
...
`zfskeys` [rc(8)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=8&format=html) 脚本在引导时为 `keylocation` 属性指向 `file://` URL 密钥文件的加密数据集加载加密密钥。该脚本在池导入之后、文件系统挂载之前运行，因此以这种方式处理的数据集会在正常的 ZFS 启动过程中自动挂载。该脚本不处理 `keylocation=prompt` 的数据集；系统引导后需用 `zfs load-key` 手动加载其密钥。
...
> **警告：** 任何能读取该密钥文件的人都可以解密数据集。请将密钥文件所有者保持为 `root`、模式为 `600`，并将其存储在本身受保护的存储上，例如加密的根文件系统或仅在引导时连接的可移动介质。
