...
FreeBSD 安装程序可以直接将系统安装到 ZFS 存储池中，这种配置称为 Root-on-ZFS；参见[引导式 Root-on-ZFS](../di-2-zhang-an-zhuang-freebsd/2.6.-fen-pei-ci-pan-kong-jian.md)。本节介绍如何在已运行的系统上创建和管理额外的 ZFS 存储池与数据集。
...
[periodic(8)](https://man.freebsd.org/cgi/man.cgi?query=periodic&sektion=8&format=html) 还可以执行存储池的定期 scrub；参见[对池进行 Scrub](./23.3.-zpool-guan-li.md)。
...
如果所有池都是 [在线](./23.10.-zfs-te-xing-he-shu-yu.md)，且一切正常，消息会显示：

```sh
all pools are healthy
```

如果出现问题，可能是磁盘处于 [离线](./23.10.-zfs-te-xing-he-shu-yu.md) 状态，池的状态会显示如下：
