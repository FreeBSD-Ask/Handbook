...
加载正确的驱动程序后，即可配置网络适配器。FreeBSD 使用驱动程序名称后跟单元号来命名网络接口适配器。单元号表示适配器在引导时被检测到的顺序，或者稍后发现的顺序。
...
* `em0`：以太网接口。
* `lo0`：环回接口是软件环回机制，可以用于性能分析、软件测试和/或本地通信。更多信息请参考 [lo(4)](https://man.freebsd.org/cgi/man.cgi?query=lo&sektion=4&format=html)。
...
4. 接口具有有效的子网掩码（`netmask`），`0xffffff00`（即 `255.255.255.0`）。
5. 接口具有有效的广播地址 `192.168.1.255`。
6. 接口的 MAC 地址（`ether`）是 `00:1f:16:0f:27:5a`。
7. 物理媒体选择处于自动选择模式（`media: Ethernet autoselect (1000baseT <full-duplex>)`）。
8. 链路状态（`status`）为 `active`，表示检测到载波信号。对于 `em0`，当以太网电缆未插入接口时，`status: no carrier` 状态是正常的。
...
可以通过命令行使用 [ifconfig(8)](https://man.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&format=html) 配置网络接口卡，但除非将配置添加到 **/etc/rc.conf** 中，否则在重启后不会保留。

> **注意**
>
> 如果在安装过程中通过 [bsdinstall(8)](https://man.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&format=html) 配置了网络，可能已有某些网卡（NIC）的条目。在执行 [sysrc(8)](https://man.freebsd.org/cgi/man.cgi?query=sysrc&sektion=8&format=html) 之前，请仔细检查 **/etc/rc.conf**。
...
> **技巧**
>
> [dhclient(8)](https://man.freebsd.org/cgi/man.cgi?query=dhclient&sektion=8&format=html) 客户端可以在后台启动。这可能会对依赖正常网络的应用程序造成问题，但在许多情况下可以提供更快的引导速度。
>
> 要在后台执行 [dhclient(8)](https://man.freebsd.org/cgi/man.cgi?query=dhclient&sektion=8&format=html)，执行以下命令：
>
> ```sh
...
IPv6 是众所周知的 IP 协议（即 IPv4）的新版本。

与 IPv4 相比，IPv6 提供了许多优势和新特性：

* 其 128 位地址空间可提供 340,282,366,920,938,463,463,374,607,431,768,211,456 个地址。这解决了 IPv4 地址短缺和最终耗尽的问题。
* 路由器仅在其路由表中存储网络聚合地址，从而将路由表的平均大小减少到 8192 条目。这解决了与 IPv4 相关的可扩展性问题，IPv4 需要每个分配的地址块在互联网路由器之间交换，导致路由表变得过大，无法有效路由。
...
阅读 IPv6 地址时，规范形式表示为 `x:x:x:x:x:x:x:x`，其中每个 `x` 代表 16 位十六进制值。示例：`FEBC:A574:382B:23C1:AA49:4592:4EFE:9982`。

通常，地址会有很长的零子串。可以使用 `::`（双冒号）来替换地址中的一个子串。另外，每个十六进制值最多可以省略三个前导零。例如，`fe80::1` 等同于规范形式 `fe80:0000:0000:0000:0000:0000:0000:0001`。

第三种形式是使用众所周知的 IPv4 表示法来写最后 32 位。例如，`2002::10.0.0.1` 对应的十六进制规范表示为 `2002:0000:0000:0000:0000:0000:0a00:0001`，这又等同于 `2002::a00:1`。
...
在此示例中，`em0` 接口使用 `fe80::21f:16ff:fe0f:275a%em0`，这是自动配置的链路本地地址，自动从 MAC 地址生成。
...
| `ff00::/8` | 多播 | |
| `2000::/3` | 全局单播 | 所有全局单播地址都从该地址池中分配。前三位是 `001`。 |
...
要使用 [SLAAC](../shu-yu-biao.md) 动态配置接口的 IPv6 地址，请执行以下命令：
...
重要的是要指定 IPv6 路由器通告接口。例如，要告诉 [rtadvd(8)](https://man.freebsd.org/cgi/man.cgi?query=rtadvd&sektion=8&format=html) 使用 `em0`：
...
启用 IPv6 的服务器可能需要启用 IPv4 映射的 IPv6 地址通信。此兼容性选项能将 IPv4 地址表示为 IPv6 地址。能让 IPv6 应用程序与 IPv4 通信，反之亦然，这可能会带来安全问题。
