...
本节提供了一个更详细的无线网卡配置示例。要查看系统中有哪些无线网卡，请参阅章节 [识别网络适配器](./7.2.-she-zhi-wang-luo.md)。
...
从扫描到的无线网络列表中选择了一个网络之后，就需要执行连接和认证。在绝大多数无线网络中，认证是通过路由器中配置的密码进行的。其他方案则要求在数据流量开始之前完成加密握手，可能使用预共享密钥或密钥，或者使用涉及后台服务（如 RADIUS）的更复杂的方案。
...
[wpa_supplicant(8)](https://man.freebsd.org/cgi/man.cgi?query=wpa_supplicant&sektion=8&format=html) 的配置将在 `/etc/wpa_supplicant.conf` 文件中进行。更多信息请参见 [wpa_supplicant.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=wpa_supplicant.conf&sektion=5&format=html)。
...
> **注意**
>
> 有关如何执行更高级认证方法的信息，请参见 [无线高级认证](../di-34-zhang-gao-ji-wang-luo/34.4.-wu-xian-gao-ji-shen-fen-yan-zheng.md)。
...
链路聚合和故障转移的详细内容请参见 [链路聚合和故障转移](../di-34-zhang-gao-ji-wang-luo/34.9.-lian-lu-ju-he-yu-gu-zhang-zhuan-yi.md)，并且可以在 [以太网和无线接口之间的故障转移模式](../di-34-zhang-gao-ji-wang-luo/34.9.-lian-lu-ju-he-yu-gu-zhang-zhuan-yi.md) 中找到使用有线和无线连接的示例。
