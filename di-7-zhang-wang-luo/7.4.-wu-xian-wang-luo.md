...
一个基本的无线网络由多个通过无线电在 2.4GHz 或 5GHz 频段中通信的站点组成，不过这因地区而异，并且也正在改变以支持在 2.3GHz 和 4.9GHz 范围内的通信。
...
如果该文件不存在，请创建它。有关在此文件中格式化条目的详细信息，请参阅 [wpa_supplicant.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=wpa_supplicant.conf&sektion=5&format=html)。
...
* ② 这是无线网络的 PSK。将其替换为无线网络的口令。

第三步是添加网络接口条目，以便在引导时配置网络。使用 `sysctl net.wlan.devices` 获取网络设备接口。在下面的输出中，该 sysctl 输出显示网络设备接口为“iwm0”。
...
在接下来的 `sysrc` 行中，如有必要，将“iwm0”替换为 sysctl 命令的输出：
...
本节提供了一个更详细的无线网络适配器配置示例。要查看系统中有哪些无线网络适配器，请参阅 [识别网络适配器](./7.2.-she-zhi-wang-luo.md) 章节。
...
> **注意**
>
> 由于各地法规不同，需要正确设置适用于所在位置的区域，以获取正确的可用信道信息。
>
> 可用的区域定义可以在 `/etc/regdomain.xml` 中找到。
>
> 要在运行时设置数据，请使用 `ifconfig`：
>
> ```sh
> # ifconfig wlan0 regdomain etsi2 country AT
> ```
>
> 要使设置持久化，在 `/etc/rc.conf` 中添加如下一行：
>
> ```sh
...
3. CAPS 字段标识每个网络的类型及其站点的功能（有关 `list scan` 的更多详细信息，请参见 [ifconfig(8)](https://man.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&format=html) 的定义）。

## 7.4.4. 连接和认证无线网络

从扫描到的无线网络列表中选择了一个网络之后，就需要执行连接和认证。在绝大多数无线网络中，认证是通过路由器中配置的口令进行的。其他方案则要求在数据流量开始之前完成加密握手，可能使用预共享密钥或密钥，或者使用涉及后台服务（如 RADIUS）的更复杂的方案。
...
完成无线网络扫描后，选择了一个网络并获取了口令（PSK），该信息将添加到 `/etc/wpa_supplicant.conf` 文件中，如下所示：
...
> **注意**
>
> 有关如何执行更高级认证方法的信息，请参见 [无线高级认证](../di-35-zhang-gao-ji-wang-luo/35.4.-wu-xian-gao-ji-shen-fen-yan-zheng.md)。
...
有线连接提供更好的性能和可靠性，而无线连接提供灵活性和移动性。笔记本用户通常希望在这两种连接之间无缝切换。

在 FreeBSD 上，可以将两个或更多网络接口结合起来，以“故障转移”方式使用。这种配置使用一组网络接口中最优先和可用的连接，当链路状态发生变化时，操作系统会自动切换。

链路聚合和故障转移的详细内容请参见 [链路聚合和故障转移](../di-35-zhang-gao-ji-wang-luo/35.9.-lian-lu-ju-he-yu-gu-zhang-zhuan-yi.md)，并且可以在 [以太网和无线接口之间的故障转移模式](../di-35-zhang-gao-ji-wang-luo/35.9.-lian-lu-ju-he-yu-gu-zhang-zhuan-yi.md) 中找到使用有线和无线连接的示例。
