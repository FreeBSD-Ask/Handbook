...
基本无线网络由多个站点组成，这些站点通过无线电在 2.4GHz 或 5GHz 频段中通信，不过这因地区而异，并且也正在改变以支持在 2.3GHz 和 4.9GHz 范围内的通信。
...
本节提供更详细的无线网络适配器配置示例。要查看系统中有哪些无线网络适配器，请参阅 [识别网络适配器](./7.2.-she-zhi-wang-luo.md) 章节。
...
从扫描到的无线网络列表中选择网络之后，就需要执行连接和认证。在绝大多数无线网络中，认证通过路由器中配置的口令完成。其他方案则要求在数据流量开始之前完成加密握手，可能使用预共享密钥或密钥，或者使用涉及后台服务（如 RADIUS）的更复杂的方案。
...
完成无线网络扫描后，选择网络并获取口令（PSK），该信息将添加到 **/etc/wpa_supplicant.conf** 文件中，如下所示：
