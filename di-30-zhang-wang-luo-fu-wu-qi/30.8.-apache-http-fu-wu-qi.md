...
在某个时间点，对 Apache 内部 SSL 的支持需要一个名为**mod\_ssl**的辅助模块。现在情况已不再如此，Apache的默认安装带有内置于Web服务器中的SSL。有关如何启用对 SSL 网站支持的示例，请参阅已安装的文件 **httpd-ssl.conf** 中的 **/usr/local/etc/apache24/extra** 目录 在此目录中还有一个名为 **ssl.conf-sample** 的示例文件。建议对这两个文件进行评估，以便在 Apache Web 服务器中正确设置安全网站。
...
可以使用 [www/](https://cgit.freebsd.org/ports/tree/www/mod\_perl2/pkg-descr) **mod\_perl2** 包或 port 安装 mod\_perl。有关使用此模块的文档，请参见 http://perl.apache.org/docs/2.0/index.html。

### 30.8.3.3. **mod\_php**

_PHP：超文本预处理器_（PHP）是一种通用的脚本语言，特别适合 Web 开发。它能够嵌入到 HTML 中，其语法借鉴了 C，Java™ 和 Perl，旨在允许 Web 开发人员快速编写动态生成的网页。

支持 PHP for Apache 和任何其他用该语言编写的功能，可以通过安装适当的端口来添加。
...
除了 mod\_perl和mod\_php 外，还有其他语言可用于创建动态 Web 内容。其中包括 Django 和 Ruby on Rails。
...
Django依赖于**mod\_python**和 SQL 数据库引擎。在 FreeBSD 中，[www/py-django](https://cgit.freebsd.org/ports/tree/www/py-django/pkg-descr) port 会自动安装**mod\_python** 并支持 PostgreSQL、MySQL 或 SQLite 数据库，默认值为 SQLite。要更改数据库引擎，请在 **/usr/ports/www/py-django** 中键入`make config`内容，然后安装port。
...
Ruby on Rails 是另一个开源 Web 框架，它提供了一个完整的开发堆栈。它经过优化，使 Web 开发人员更有效率，并能够快速编写功能强大的应用程序。在 FreeBSD 上，它可以使用 [www/rubygem-rails](https://cgit.freebsd.org/ports/tree/www/rubygem-rails/pkg-descr) 软件包或端口进行安装。
