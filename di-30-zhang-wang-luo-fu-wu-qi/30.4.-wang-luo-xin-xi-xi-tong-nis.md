...
但是，一些非 FreeBSD 操作系统要求 NIS 域名与互联网域名相同。如果网络上的一台或多台计算机具有此限制，*则必须*将 Internet 域名用作 NIS 域名。
...
禁止指定用户登录到单个系统在较大的网络上变得无法扩展，并很快失去NIS的主要好处：*集中管理*。
...
| 注意                                                         |
| ------------------------------------------------------------ |
| 在重建登录功能数据库*后*，每个用户更改其密码之前，不会更新现有用户帐户的密码格式。 |
