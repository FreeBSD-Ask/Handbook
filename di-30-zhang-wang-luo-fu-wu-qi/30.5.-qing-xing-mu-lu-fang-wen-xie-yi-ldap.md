...
提示的条目可能是通用的，但`Common Name` 条目必须与系统主机名*不同*。如果这将是自签名证书，请在主机名前面加上"证书颁发机构"前缀`CA`。
...
对*全局配置*部分的修改只能由全局超级用户执行。例如，假定最初指定了`olcTLSCipherSuite: HIGH:MEDIUM:SSLv3`选项，现在必须将其删除。首先，创建一个包含以下内容的文件：
