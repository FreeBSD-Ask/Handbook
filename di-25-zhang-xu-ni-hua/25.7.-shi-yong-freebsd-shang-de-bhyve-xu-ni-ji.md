...
虽然可以在虚拟机运行时对 ZFS 卷进行快照，但请注意，在虚拟机活跃时，虚拟磁盘的内容可能处于不一致的状态。因此，建议在执行此命令之前，先关闭或暂停虚拟机。默认情况下，不支持暂停虚拟机，需要先启用（参见 [内存和 CPU 快照](./25.7.-shi-yong-freebsd-shang-de-bhyve-xu-ni-ji.md)）
