...
Linux 虚拟机可以像其他常规 [基于 UEFI 的虚拟机](./25.7.-shi-yong-freebsd-shang-de-bhyve-xu-ni-ji.md) 一样引导，或者可以使用 Port [sysutils/grub2-bhyve](https://cgit.freebsd.org/ports/tree/sysutils/grub2-bhyve/) 。
...
虽然可以在虚拟机运行时对 ZFS 卷进行快照，但请注意，在虚拟机活跃时，虚拟磁盘的内容可能处于不一致的状态。因此，建议在执行此命令之前，先关闭或暂停虚拟机。默认情况下，不支持暂停虚拟机，需要先启用（参见 [内存和 CPU 快照](./25.7.-shi-yong-freebsd-shang-de-bhyve-xu-ni-ji.md#id-25.7.8.2.-nei-cun-he-cpu-kuai-zhao-shi-yan-xing-gong-neng)）
...
然而，此功能默认情况下未启用，并且需要从源代码重新编译系统。有关编译内核和自定义选项的详细说明，请参见 [从源代码构建](../di-27-zhang-freebsd-geng-xin-yu-sheng-ji/27.6.-cong-yuan-dai-ma-geng-xin-freebsd.md)。
...
> **注意**
>
> 在执行以下步骤之前，请确保 **/usr/src** 目录是最新的。请参见 [更新源代码](../di-27-zhang-freebsd-geng-xin-yu-sheng-ji/27.6.-cong-yuan-dai-ma-geng-xin-freebsd.md) 以了解详细操作步骤。
...
然后按照 [从源代码更新 FreeBSD 快速入门部分](../di-27-zhang-freebsd-geng-xin-yu-sheng-ji/27.6.-cong-yuan-dai-ma-geng-xin-freebsd.md) 中的步骤构建并安装基本系统和内核。
