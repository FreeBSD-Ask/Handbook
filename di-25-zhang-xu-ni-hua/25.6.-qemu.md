...
4. 使用以下命令为两个虚拟机配置网络。在此示例中，宿主网络接口是 `em0`。如果有必要，请修改为宿主系统的接口。每次宿主机重启后必须执行这些命令，以便让 QEMU 客户机虚拟机能够通信。
...
[安装 Xorg](../di-5-zhang-xwindow-xi-tong/5.4.-an-zhuang-xorg-fu-wu-qi.md) 说明了如何设置 `X Window` 系统。请参考该指南进行初始的 `X Window` 设置，然后查阅 [桌面环境](../di-8-zhang-zhuo-mian-huan-jing/8.1.-gai-shu.md) 以了解如何设置完整的桌面环境。
