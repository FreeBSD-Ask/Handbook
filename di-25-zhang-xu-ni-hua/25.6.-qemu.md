...
* QEMU 还提供了多个独立的命令行工具，例如 [qemu-img(1)](https://man.freebsd.org/cgi/man.cgi?query=qemu-img&sektion=1&format=html) 磁盘镜像工具，允许用户创建、转换和修改磁盘镜像。
...
以下更新后的配置演示了如何在虚拟机上实现串行控制台。运行脚本以引导虚拟机。
...
> **注意**
>
> 一旦 **/boot/loader.conf** 已更新为使用串行控制台，虚拟机将在每次引导时尝试从串行端口引导。请确保如上所示启用串行端口，或者修改 **/boot/loader.conf** 以取消对串行控制台的使用。
...
[图 11](./25.6.-qemu.md) 显示了向虚拟机添加新磁盘所需的监控器命令序列。待使用监控器中的 `device_add` 命令添加了设备，它将在 FreeBSD 系统控制台中显示（图中的下半部分）。此时可以根据需要配置磁盘。
