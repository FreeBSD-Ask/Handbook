...
> **注意**
>
> 本节所举示例基于 QEMU 9.0.1 版本（2024 年夏季）。若有疑问，请始终查阅 [QEMU 文档](https://www.qemu.org/docs/master/)，特别是 [关于 QEMU](https://www.qemu.org/docs/master/about/index.html) 页面，该页面包含支持的构建平台、仿真、已弃用功能和已移除功能的链接。
...
5. 切换到 **~/QEMU/SCRIPTS** 目录，使用以下脚本引导第一个虚拟机“left”。该脚本使用了 QEMU 的原始磁盘格式。
...
QEMU 将在一个单独的窗口中引导虚拟机，并按 [图 2](./25.6.-qemu.md) 中所示引导 FreeBSD ISO。所有命令选项，如 `-cpu` 和 `-boot`，均在 QEMU 的手册页 [qemu(1)](https://man.freebsd.org/cgi/man.cgi?query=qemu&sektion=1&format=html) 中有详细说明。
...
> **注意**
>
> QEMU 支持 `-runas` 选项。为了增加安全性，可以在上述脚本中添加“-runas your_user_name”选项。详情请参见 [qemu(1)](https://man.freebsd.org/cgi/man.cgi?query=qemu&sektion=1&format=html)。
...
在 [图 7](./25.6.-qemu.md) 中，串行端口被重定向到主机系统的 TCP 端口，并且 QEMU 监视器等待（`wait=on`）直到在指定的 localhost 端口发生 [telnet(1)](https://man.freebsd.org/cgi/man.cgi?query=telnet&sektion=1&format=html) 连接。当收到来自另一个会话的连接时，FreeBSD 系统开始引导并查找 **/boot/loader.conf** 中的控制台指令。通过设置“console=comconsole”，FreeBSD 在串行端口上启动一个控制台会话。QEMU 监视器会检测到这一点，并将该串行端口的字符 I/O 导向主机上的 telnet 会话。系统引导完成后，登录提示将在串行端口（`ttyu0`）和控制台（`ttyv0`）上启用。
