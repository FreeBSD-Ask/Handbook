...
这将打开一个窗口，QEMU 会尝试从硬盘、软盘、DVD/CD 和 PXE 引导。由于尚未设置任何引导镜像，因此命令会产生一些错误并以“No bootable device”结束，如 [图 1](./24.6.-qemu.md) 所示。不过，这表明 QEMU 软件已正确安装。
...
QEMU 将在一个单独的窗口中引导虚拟机，并按 [图 2](./24.6.-qemu.md) 中所示引导 FreeBSD ISO。所有命令选项，如 `-cpu` 和 `-boot`，均在 QEMU 的手册页 [qemu(1)](https://man.freebsd.org/cgi/man.cgi?query=qemu&sektion=1&format=html) 中有详细说明。
...
> **技巧**
>
> 如果在 QEMU 控制台窗口中点击鼠标，QEMU 将“抓取”鼠标，如 [图 3](./24.6.-qemu.md) 所示。按 Ctrl+Alt+G 可以释放鼠标。
...
XFCE4 窗口管理器将启动并呈现一款功能齐全的图形桌面，如 [图 4](./24.6.-qemu.md) 所示。首次启动时，可能需要一分钟才能显示桌面。有关使用详细信息，请参阅 [XFCE 网站](https://www.xfce.org/)。
...
要切换到另一个虚拟控制台，单击 QEMU 窗口并输入 **Alt**+**F2** 或 **Alt**+**F3**。FreeBSD 应该会切换到另一个虚拟控制台。[图 5](./24.6.-qemu.md) 显示了“left”虚拟机在 `ttyv3` 上显示虚拟控制台的情况。
...
QEMU 窗口的另一个功能是 `View` 菜单和缩放控制。最有用的是 `Zoom to Fit`。当点击此菜单项时，可以通过点击窗口角部控制并调整窗口大小来调整 QEMU 窗口的大小。[图 6](./24.6.-qemu.md) 显示了在图形模式下调整“left”窗口大小的效果。
...
如果一切设置正确，系统将引导，如 [图 8](./24.6.-qemu.md) 所示。
...
[图 9](./24.6.-qemu.md) 显示了登录已安装系统的 root 用户并运行 [uname(1)](https://man.freebsd.org/cgi/man.cgi?query=uname&sektion=1&format=html)。
...
这将在终端窗口中生成一个新的提示符 `(qemu)`，如 [图 10](./24.6.-qemu.md) 所示。
...
[图 11](./24.6.-qemu.md) 显示了向虚拟机添加新磁盘所需的监控器命令序列。待使用监控器中的 `device_add` 命令添加了设备，它将在 FreeBSD 系统控制台中显示（图中的下半部分）。此时可以根据需要配置磁盘。
...
[图 12](./24.6.-qemu.md) 显示了全新安装的 FreeBSD 系统，尚未安装任何软件包，并且在 ttyv1 上单独显示了编辑会话。当前， [vi(1)](https://man.freebsd.org/cgi/man.cgi?query=vi&sektion=1&format=html) 编辑器处于 `insert` 模式，打字员正在输入“broadcast”一词。
...
[图 13](./24.6.-qemu.md) 显示了上述命令的结果以及 `info snapshots` 命令的输出。
...
默认情况下，QEMU 将快照数据存储在与镜像相同的文件中。使用 [qemu-img(1)](https://man.freebsd.org/cgi/man.cgi?query=qemu-img&sektion=1&format=html) 查看快照列表，如下所示，见 [图 14](./24.6.-qemu.md)。
...
[图 16](./24.6.-qemu.md) 的上部分显示了 QEMU 监视器命令：
...
在 [图 17](./24.6.-qemu.md) 中，展示了如何使用新设备。
