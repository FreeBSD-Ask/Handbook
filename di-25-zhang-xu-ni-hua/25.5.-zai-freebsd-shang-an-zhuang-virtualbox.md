...
## 25.5.1. 安装 VirtualBox™

VirtualBox™ 可通过 FreeBSD 的软件包或 Ports 系统获取，位于 [emulators/virtualbox-ose](https://cgit.freebsd.org/ports/tree/emulators/virtualbox-ose/)。使用以下命令安装该 Port：
...
该 Port 的配置菜单中有一个实用选项 `GuestAdditions`，它提供多项对虚拟机操作系统有用的功能，例如鼠标指针集成（能在宿主机与虚拟机之间共享鼠标，无需按快捷键切换）和更快的视频渲染，尤其是在 Windows® 虚拟机中更为明显。虚拟机系统安装完成后，可在 **Devices** 菜单中使用该增强功能。

在第一次启动 VirtualBox™ 前，需要做一些配置。该 Port 会在 **/boot/modules** 中安装一个内核模块，需将其加载进当前运行的内核中：
...
## 25.5.3. VirtualBox™ 宿主机访问 DVD/CD

虚拟机系统通过共享物理设备访问宿主机 DVD/CD 驱动器。在 VirtualBox™ 中，可在虚拟机设置中的 Storage 窗口配置。如有需要，先创建一个空的 IDECD/DVD 设备。然后在虚拟 CD/DVD 选择菜单中选择 Host Drive。会出现一个名为 `Passthrough` 的复选框，勾选该项后虚拟机会直接访问硬件。例如，音频 CD 或刻录功能仅在该选项启用时可用。

要使用 VirtualBox™ 的 DVD/CD 功能，用户必须具有对 **/dev/xpt0**、**/dev/cdN** 和 **/dev/passN** 的访问权限。通常将用户加入 `operator` 用户组即可。还需在 **/etc/devfs.conf** 中添加以下内容以修正这些设备的权限：
