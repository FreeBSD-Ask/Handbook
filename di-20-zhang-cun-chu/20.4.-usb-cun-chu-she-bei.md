...
USB 存储设备的支持已经内建在 **GENERIC** 内核中。对于定制内核，请确保在内核配置文件中包含以下行：
...
如果设备尚未格式化，请参考 [添加磁盘](./20.2.-tian-jia-ci-pan.md) 了解如何在 USB 驱动器上格式化并创建分区。如果驱动器已包含文件系统，可以由 `root` 按照 [“挂载和卸载文件系统”](../di-3-zhang-freebsd-ji-chu/3.7.-wen-jian-xi-tong-de-gua-zai-yu-xie-zai.md) 中的说明进行挂载。
