...
为实现这一任务，使用了通用地址冗余协议（CARP）来提供 IP 层的自动故障切换。CARP 允许同一网络段上的多个主机共享一个 IP 地址。根据 [“通用地址冗余协议（CARP）”](../di-34-zhang-gao-ji-wang-luo/34.11.-gong-yong-di-zhi-rong-yu-xie-yi-carp.md) 中的文档，在两个节点上设置 CARP。在此示例中，每个节点将有自己的管理 IP 地址，并且共享一个 IP 地址 *172.16.0.254*。集群的主 HAST 节点必须是主 CARP 节点。
