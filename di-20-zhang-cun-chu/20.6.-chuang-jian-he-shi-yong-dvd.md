...
* **DVD-RAM**：这是一个可重写格式，可以视为可移动硬盘。然而，这种介质与大多数 DVD-ROM 驱动器和 DVD-Video 播放器不兼容，因为只有少数 DVD 写入器支持 DVD-RAM 格式。有关 DVD-RAM 使用的更多信息，请参阅 [使用 DVD-RAM](./20.1.-gai-shu.md)。
...
由于 [growisofs(1)](https://man.freebsd.org/cgi/man.cgi?query=growisofs&sektion=1&format=html) 是 [mkisofs](./20.1.-gai-shu.md) 的前端，它将调用 [mkisofs(8)](https://man.freebsd.org/cgi/man.cgi?query=mkisofs&sektion=8&format=html) 来创建文件系统布局并执行 DVD 刻录过程。这意味着在刻录之前不需要创建数据的镜像。
