...
**GENERIC** 内核提供了对 SCSI、USB 和 ATAPI CD 读取器及刻录机的支持。如果使用自定义内核，内核配置文件中需要根据设备类型添加相应的选项。
...
有时，SCSI CD 驱动器可能会因为没有足够的时间响应总线重置而被忽略。为了解决这个问题，可以创建一个自定义内核，增加默认的 SCSI 延迟。在自定义内核配置文件中添加以下选项，并按照 [“构建和安装自定义内核”](../di-10-zhang-pei-zhi-freebsd-nei-he/10.5.-bian-yi-yu-an-zhuang-ding-zhi-nei-he.md) 中的说明重新构建内核：
