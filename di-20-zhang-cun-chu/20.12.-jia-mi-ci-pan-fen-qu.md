...
这将配置 **/dev/da2** 作为具有主密钥 **/root/da2.key** 的 `geli` 提供者。在系统关闭之前，系统将自动将提供者从内核中分离。在引导过程中，脚本将提示输入密码，然后附加提供者。引导过程中的其他内核消息可能会在密码提示之前或之后显示。如果引导过程似乎停滞，请仔细查看其他消息中的密码提示。输入正确的密码后，提供者将被附加。然后，文件系统将挂载，通常通过 **/etc/fstab** 中的条目进行挂载。有关如何配置文件系统以在启动时挂载的说明，请参考 [“挂载和卸载文件系统”](../di-3-zhang-freebsd-ji-chu/3.7.-wen-jian-xi-tong-de-gua-zai-yu-xie-zai.md)。
