...
在这个例子中，假设 Sendmail 在 `example.FreeBSD.org` 上正常运行，直接发送到 [yourlogin@example.FreeBSD.org](../di-29-zhang-dian-zi-you-jian/yourlogin@example.FreeBSD.org) 的邮件应该没有问题。
...
所有发往 [example.FreeBSD.org](../di-29-zhang-dian-zi-you-jian/yourlogin@example.FreeBSD.org) 的邮件都会被收集到 `hub` 上的同一个用户名下，而不是直接发往你的主机。
