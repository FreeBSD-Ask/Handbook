...
`et.inet.icmp.drop_redirect` 和 `net.inet.ip.redirect` 设置有助于防止 *重定向攻击*。重定向攻击是 DoS 的一种类型，它发送大量的 ICMP 类型 5 数据包。由于不需要这些数据包，请将 `net.inet.icmp.drop_redirect` 设置为 `1`，将 `net.inet.ip.redirect` 设置为 `0`。
