...
用户空间的事件审计支持作为 FreeBSD 基本系统的一部分进行安装。内核支持默认包含在 `GENERIC` 内核中，可以通过将以下行添加到 **/etc/rc.conf** 来启用 [auditd(8)](https://man.freebsd.org/cgi/man.cgi?query=auditd&sektion=8&format=html)：
...
选择表达式用于审计配置中的多个地方，以确定应审计哪些事件。表达式包含一个用于匹配的事件类别列表。选择表达式从左到右求值，两个表达式通过将一个附加到另一个来组合。
...
这些审计事件类别可以通过修改 `audit_class` 和 `audit_event` 配置文件进行自定义。

每个审计事件类别都可以与前缀结合，表示是否匹配成功/失败的操作，以及该条目是否添加或移除类别和类型的匹配。[审计事件类别的前缀](./20.3.-shen-ji-pei-zhi.md)总结了可用的前缀：
