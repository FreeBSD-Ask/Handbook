...
选择表达式用于审计配置中的多个地方，以确定应审计哪些事件。表达式包含一个事件类别列表用于匹配。选择表达式从左到右进行评估，两个表达式通过将一个附加到另一个来组合。
...
* `dir` 项用于设置一个或多个存储审计日志的目录。如果出现多个目录条目，它们将按顺序使用，直到填满为止。通常建议将审计日志存储在专用文件系统上，以防止当文件系统填满时审计子系统和其他子系统之间的干扰。
...
* `filesz` 项指定审计日志文件的最大大小，超出该大小后自动终止并轮替日志文件。值为 `0` 时会禁用自动日志轮替。如果请求的文件大小小于 512k，则会被忽略，并生成一条日志消息。
