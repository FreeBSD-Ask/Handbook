...
选择表达式用于审计配置中的多个地方，以确定应审计哪些事件。表达式包含一个用于匹配的事件类别列表。选择表达式从左到右进行评估，两个表达式通过将一个附加到另一个的方式进行组合。
...
* `filesz` 项指定审计跟踪文件的最大大小，超出该大小后自动终止并轮替日志文件。值为 `0` 时会禁用自动日志轮替。如果请求的文件大小小于 512k，则会被忽略，并生成一条日志消息。
